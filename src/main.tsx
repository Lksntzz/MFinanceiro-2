...
async function checkForAppUpdate() {
  ...
  try {
    ...
    if ('serviceWorker' in navigator) {
      try { const registration = await navigator.serviceWorker.getRegistration(); await registration?.update(); } catch { /* reload still proceeds */ }
    }
  ...
  if ('serviceWorker' in navigator) {
    if (import.meta.env.PROD) {
      navigator.serviceWorker.register('/sw.js').then((registration) => registration.update().catch(() => {})).catch(() => {
        void import('./lib/operational-observability').then(({ reportOperationalEvent }) => reportOperationalEvent('runtime.service_worker_registration_failed', 'service-worker', 'warning'));
      });
    } else {
