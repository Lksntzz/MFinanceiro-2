...
export default function ImportarExtratos({
  ...
  const onlyDuplicates = noNewRows && duplicates > 0 && rejected === 0 && ignored === 0;
  const hasWarnings = rejected > 0 || ignored > 0;

  const title = inserted > 0
    ? hasWarnings ? 'Importação concluída com ressalvas' : 'Importação concluída'
    : onlyDuplicates
      ? 'Nenhum lançamento novo'
      : 'Importação sem novos lançamentos';

  const description = inserted > 0
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-5 overflow-y-auto p-4 animate-fade-in">
      <div className={`h-20 w-20 rounded-full flex items-center justify-center ${warningOnly ? 'bg-amber-500/20 text-amber-300' : 'bg-green-500/20 text-green-500'}`}>
      ...
      <section className="grid w-full max-w-xl grid-cols-2 gap-2 sm:grid-cols-4" aria-label="Resumo da importação">
        <ImportMetric label="Inseridos" value={inserted} className="text-green-400" />
        <ImportMetric label="Duplicados" value={duplicates} className="text-yellow-300" />
        <ImportMetric label="Rejeitados" value={rejected} className={rejected > 0 ? 'text-red-300' : ''} />
      ...
      {(rejected > 0 || ignored > 0) && (
        <p className="max-w-xl text-center text-[10px] leading-relaxed text-white/35">
          O histórico detalhado do processamento fica disponível em Movimentações → Lotes e conciliação.
        </p>
      )}
    </div>
  );
...
function ImportMetric({ label, value, className = '' }: { label: string; value: number; className?: string }) {
  return (
    <div className="glass-card !p-3 text-center">
      <span className="block text-[9px] font-bold uppercase tracking-wider text-white/35">{label}</span>
