...
export default function ImportarExtratos({
  ...
  const pdfPasswordResolverRef = useRef<
    ((password: string | null) => void) | null
  >(null);
  const [step, setStep] = useState<
    'upload' | 'processing' | 'review' | 'success'
  >('upload');
  ...
  const [balanceMode, setBalanceMode] = useState<
    'keep' | 'apply_new' | 'statement'
  >('keep');
  const [reviewAcknowledged, setReviewAcknowledged] = useState(false);
  const [ocrExtractionId, setOcrExtractionId] = useState<string | null>(null);
  const [ocrStatementBalance, setOcrStatementBalance] = useState<
    number | undefined
  >(undefined);
  ...
  const readyItemsCount = importedData.filter(
    (i) => i.status === 'ready',
  ).length;
  const canConfirmImport =
    readyItemsCount > 0 && Boolean(selectedAccountId) && reviewAcknowledged;
  ...
  const requestPdfPassword = useCallback(
    (options: { fileName?: string; incorrect?: boolean }) => {
      pdfPasswordResolverRef.current?.(null);
      setPdfPassword('');
  ...
  const balanceValidation = calculateImportBalanceValidation(
    importedData,
    ocrStatementBalance,
  );
  ...
  const resetImportState = useCallback(
    (targetStep: 'upload' | 'processing' = 'upload') => {
  ...
  const processFile = async (selectedFile: File) => {
    ...
    try {
      ...
      let parserDebug: ParserDebugSummary = {
        linesExtracted: 0,
        linesIgnored: 0,
        rejectedLineReasons: [],
      };
      ...
      if (detection.format === 'csv') {
        ...
        const content =
          standardizeBankCsv(rawContent, fileSnapshot.name) || rawContent;
        ...
        const hasValidLocalRows = parsed.some(
          (item) => item.status === 'ready' && Number(item.amount) > 0,
        );
        if (!hasValidLocalRows) {
          try {
            const ocrResult = await extractStatementWithOcr(
              fileSnapshot,
              selectedAccountId,
            );
            ...
            parserDebug = {
              ...
              linesIgnored: parsed.filter((item) => item.status !== 'ready')
                .length,
        ...
        parsed = sheetAnalysis.csv.trim()
          ? parseCsvTransactions(sheetAnalysis.csv, bank)
          : [];
        const rejected = getInvalidLineReasons(parsed);
        parserDebug = {
      ...
      const normalized = ensureMinimumTransactionFields(
        normalizeImportedTransactions(parsed),
      );
      ...
      if (ocrDocumentConfidence !== null) {
        ...
        const preview = pdfDebugInfo.textPreview
          ? ` | Texto extraido: "${pdfDebugInfo.textPreview}"`
          : '';
      ...
      setImportDiagnostics({
        ...
        reason: validFound === 0 ? zeroResultReason : undefined,
        debugNote,
      ...
      setImportDiagnostics({
        ...
        rejectedLineReasons: [],
        reason:
          'Falha ao ler o arquivo. Verifique o formato e tente novamente.',
      });
      setStep('review');
    }
  };
  ...
  const handleRemove = (id: string) => {
    ...
    setImportedData((current) => {
      ...
      if (result.rejectedExtractionItemId) {
        setRejectedOcrItemIds((rejected) =>
          rejected.includes(result.rejectedExtractionItemId!)
            ? rejected
  ...
  async function persistOcrReview() {
      ...
      .map((item) =>
        supabase
          ...
          .update({
            ...
            reviewer_notes:
              item.status === 'ready'
                ? 'Revisado antes da importação.'
                : 'Não selecionado na revisão.',
          })
      ...
      updates.push(
        supabase
          ...
          .update({
            ...
            reviewer_notes: 'Removido durante a revisão humana.',
          })
          .eq('extraction_id', ocrExtractionId)
    ...
    if (failed?.error)
      throw new Error(
        `Não foi possível salvar a revisão do OCR: ${failed.error.message}`,
      );
  }
  ...
  const handleFinalImport = async () => {
    ...
    const calibrationBalance =
      balanceMode === 'statement' && balanceValidation
        ? balanceValidation.statementFinal
        : undefined;
    ...
    try {
      if (!file || !selectedAccountId)
        throw new Error(
          'Selecione um arquivo e a conta financeira de destino.',
        );
      ...
      if (ocrExtractionId) {
        ...
        if (extractionError)
          console.warn(
            'A revisão OCR não pôde ser encerrada após a importação:',
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-6 animate-fade-in">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-white/40 text-sm max-w-xs mx-auto">
            {committedCount} lancamentos foram adicionados ao seu ledger com
            sucesso.
          </p>
        </div>
        <button
    ...
    <div className="flex-1 flex flex-col gap-4 overflow-hidden animate-fade-in">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h2 className="text-xl font-bold flex items-center gap-2">
            <Database className="text-brand-secondary" size={24} /> Importar
            Extratos
          </h2>
          <p className="text-xs text-white/40">
            Traga suas movimentacoes bancarias para o MFinanceiro de forma
            inteligente.
          </p>
        </div>
        <button
          ...
          <div className="grid grid-cols-12 gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="glass-card !p-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-white/40 mt-1">
                      CSV, OFX, Excel, PDF e imagens; OCR/IA exige revisão
                      humana.
                    </p>
                  </div>
                </div>
                <div className="glass-card !p-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-white/40 mt-1">
                      Arquivos de OCR ficam em armazenamento privado e a chave
                      de IA permanece no servidor.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="col-span-4 space-y-4">
              <div className="glass-card !p-5 space-y-4">
                ...
                <div>
                  <label className="text-[10px] text-white/40 uppercase font-bold block mb-1.5">
                    Banco / Origem
                  </label>
                  <select
                    value={bank}
              ...
              <div className="glass-card !p-5 bg-brand-primary/5 border-brand-primary/20">
                ...
                <p className="text-[10px] text-white/60 leading-relaxed">
                  OFX costuma ter mais precisao na importacao por trazer
                  identificadores unicos e valor consolidado por transacao.
                </p>
              </div>
            </div>
          </div>
      ...
      {step === 'processing' && (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 animate-pulse">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/40 uppercase tracking-widest mt-1">
              Normalizando dados e detectando duplicidades
            </p>
          </div>
        </div>
      )}
      ...
      {step === 'review' && (
        <div className="flex-1 flex flex-col gap-4 overflow-hidden">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-3">
              {(
                [
                  {
                    id: 'keep',
                    label: 'Preservar saldo atual',
                    detail: 'Importa o histórico sem alterar o saldo de hoje.',
                  },
                  {
                    id: 'apply_new',
                    label: 'Aplicar movimentações',
                    detail: 'Soma entradas e saídas novas ao saldo.',
                  },
                  {
                    id: 'statement',
                    label: 'Usar saldo do extrato',
                    detail: balanceValidation
                      ? 'Calibra para o saldo final informado.'
                      : 'O arquivo não trouxe saldo final.',
                  },
                ] as const
              ).map((mode) => (
                <button
          ...
          {balanceValidation && (
            <div
              className={`glass-card !p-3 shrink-0 border transition-all ${balanceValidation.isClose ? 'border-green-500/30 bg-green-500/10' : 'border-yellow-500/30 bg-yellow-500/10'}`}
            >
                ...
                <div className="text-right text-[11px] leading-5 shrink-0">
                  <div className="text-white/40">
                    ...
                    <span className="text-white font-medium">
                      R${' '}
                      {balanceValidation.expectedFinal.toLocaleString('pt-BR', {
                  ...
                  <div>
                    ...
                    <span className="font-bold text-white text-sm">
                      R${' '}
                      {balanceValidation.statementFinal.toLocaleString(
                        'pt-BR',
                  ...
                  >
                    Diferença: {balanceValidation.diff >= 0 ? '+' : '-'} R${' '}
                    {Math.abs(balanceValidation.diff).toLocaleString('pt-BR', {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </div>
              ...
              {balanceMode === 'statement' && (
                <div className="mt-2 py-2 px-3 bg-brand-primary/10 rounded-lg border border-brand-primary/20 flex items-center gap-3">
                  ...
                  <p className="text-[10px] text-brand-primary/80 leading-relaxed font-medium">
                    Ao confirmar, seu saldo atual no sistema será ajustado para{' '}
              ...
              <div className="flex flex-wrap items-center gap-6">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-sm font-bold text-red-400">
                    {importedData.filter((i) => i.status === 'error').length}
                  </span>
                </div>
                <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-white/10 px-3 py-2 text-[9px] text-white/60">
                  <input
                    ...
                    disabled={readyItemsCount === 0 || isImporting}
                  />
          ...
          {importDiagnostics && (
            <div className="glass-card !p-3 shrink-0 border border-white/10">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-[11px]">
                <div>
                  <div className="text-white/40 uppercase text-[9px] font-bold">
                    Arquivo atual
                  </div>
                  <div className="font-bold truncate">
                    {importDiagnostics.fileName}
                  </div>
                </div>
                <div>
                  <div className="text-white/40 uppercase text-[9px] font-bold">
                ...
                <div>
                  ...
                  <div className="font-bold">
                    {importDiagnostics.mimeType || 'desconhecido'}
                  </div>
                </div>
                <div>
                  <div className="text-white/40 uppercase text-[9px] font-bold">
                  ...
                  <div
                    className={`font-bold ${importDiagnostics.parserExists ? 'text-green-400' : 'text-red-400'}`}
                  >
              ...
              <div className="mt-2 text-[10px] text-white/60">
                <span className="text-white/40 uppercase text-[9px] font-bold">
                  Tamanho:
                </span>{' '}
                ...
                {new Date(importDiagnostics.fileLastModified).toLocaleString(
                  'pt-BR',
                )}
              </div>
              ...
              <div className="mt-1 text-[11px]">
                ...
                <span className="font-bold">
                  {importDiagnostics.linesExtracted}
                </span>{' '}
                <span className="text-white/30">|</span>{' '}
                <span className="text-white/40 uppercase text-[9px] font-bold">
                  Linhas ignoradas:
                ...
                <span className="font-bold">
                  {importDiagnostics.linesIgnored}
                </span>
              </div>
              ...
              {importDiagnostics.rejectedLineReasons.length > 0 && (
                <div className="mt-2 text-[10px] text-red-300 space-y-1">
                  <div className="text-[9px] uppercase text-red-200/80 font-bold">
                    Motivos de rejeicao (amostra)
                  </div>
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar space-y-2">
              ...
              >
                ...
                <div className="min-w-0 space-y-2">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="min-w-0 flex-1 rounded-lg border border-white/10 bg-black/20 px-2 py-1.5 text-xs font-bold text-white outline-none focus:border-brand-primary/50"
                    />
                    {item.status === 'error' && (
                      <span className="text-[8px] bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded uppercase font-bold">
                        Campo faltando
                      </span>
                    )}
                    {item.confidence < 0.85 && item.status !== 'error' && (
                      <span className="whitespace-nowrap rounded bg-yellow-500/15 px-1.5 py-0.5 text-[8px] font-bold text-yellow-200">
                        REVISAR {Math.round(item.confidence * 100)}%
                      </span>
                    )}
                  </div>
                  <div className="grid gap-2 sm:grid-cols-[130px_minmax(0,1fr)]">
                    <input
                      ...
                      onChange={(event) =>
                        handleUpdateItem(item.id, { date: event.target.value })
                      }
                      className="rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-[10px] text-white"
                    />
                    <input
                      ...
                      className="min-w-0 rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-[10px] text-white"
                    />
                  </div>
                </div>

                <div className="col-start-2 text-right md:col-start-auto">
                  <label className="block text-[8px] font-bold uppercase text-white/35">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        handleUpdateItem(item.id, {
                          amount: Math.abs(Number(event.target.value || 0)),
                        })
                      }
                      className={`mt-1 w-full rounded-lg border border-white/10 bg-black/20 px-2 py-1.5 text-right text-sm font-bold ${item.type === 'income' ? 'text-green-400' : 'text-white'}`}
                    />
                  </label>
                  <div className="mt-1 text-[8px] text-white/40 uppercase font-bold">
                    {item.bank_source} · confiança{' '}
                ...
                <div className="col-start-2 flex items-center justify-end gap-1 md:col-start-auto">
                  <button
                    ...
                    aria-label={
                      item.status === 'ready'
                        ? `Não importar ${item.description}`
                        : `Aprovar ${item.description}`
                    }
            ...
            {importedData.length === 0 && (
              <div className="p-4 rounded-xl border border-yellow-500/20 bg-yellow-500/5 text-xs text-yellow-300">
                {importDiagnostics?.reason ||
                  'Nenhum lancamento valido encontrado. Revise o formato e tente novamente.'}
              </div>
            )}
          </div>
        </div>
      ...
      {pdfPasswordPrompt && (
        <div
          className="fixed inset-0 z-[200] grid place-items-center bg-black/80 p-5 backdrop-blur-md"
          role="dialog"
          aria-modal="true"
          aria-labelledby="mf-pdf-password-title"
        >
          <form
          ...
          >
            <span className="inline-flex rounded-full bg-purple-500/15 px-2.5 py-1 text-[10px] font-black tracking-widest text-purple-300">
              PDF PROTEGIDO
            </span>
            <h3 id="mf-pdf-password-title" className="mt-3 text-xl font-black">
              {pdfPasswordPrompt.incorrect
                ? 'Senha incorreta'
                : 'Este extrato precisa de senha'}
            </h3>
            <p className="mt-2 text-xs leading-5 text-white/55">
              {pdfPasswordPrompt.incorrect
                ? 'A senha informada não abriu o PDF. Confira e tente novamente.'
                : 'A senha será usada somente nesta leitura e não será salva.'}
            </p>
            <div className="mt-4 truncate rounded-xl bg-white/5 px-3 py-2 text-[10px] text-white/45">
              {pdfPasswordPrompt.fileName || 'Arquivo PDF protegido'}
            </div>
            <label className="mt-4 block text-[10px] font-bold uppercase tracking-widest text-white/55">
              Senha do PDF
              <div className="mt-2 flex gap-2">
                <input
                  ...
                  className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/35 px-3 py-3 text-sm text-white outline-none focus:border-purple-400/60"
                />
                <button
                  type="button"
                  onClick={() => setShowPdfPassword((current) => !current)}
                  className="rounded-xl border border-white/10 px-3 text-[10px] font-bold text-white/65"
                >
                  {showPdfPassword ? 'Ocultar' : 'Mostrar'}
                </button>
              </div>
            </label>
            {pdfPasswordPrompt.incorrect && (
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => finishPdfPassword(null)}
                className="rounded-xl border border-white/10 px-4 py-2.5 text-xs font-bold text-white/65"
              >
                Cancelar
              </button>
              <button
                ...
                className="rounded-xl bg-purple-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40"
              >
                Abrir PDF
              </button>
            </div>
            <p className="mt-4 text-[9px] leading-4 text-white/30">
              A senha permanece apenas na memória durante a abertura do arquivo.
            </p>
          </form>
        </div>
      )}
