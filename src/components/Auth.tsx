...
function loginStateMessage(state: ResolvedAuthState) {
  if (state === 'pending')
    return 'Sua solicitação ainda está aguardando aprovação.';
  if (state === 'approved')
    return 'Seu acesso foi aprovado. Volte à solicitação para cadastrar sua senha.';
  if (state === 'confirmation_pending')
    return 'Sua conta foi criada, mas o e-mail ainda precisa ser confirmado.';
...
export default function Auth() {
  ...
  const rememberedEmail =
    readSession(CONFIRMED_EMAIL_STORAGE) || readLocal(STORAGE_EMAIL);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('mf:confirmed-email', syncConfirmedEmail);
  }, [isConfirmed]);
  ...
  function applyResolvedState(state: ResolvedAuthState, automatic = false) {
    ...
    if (state === 'account') {
      setMode('login');
      setInfo(
        automatic ? null : 'Conta encontrada. Digite sua senha para entrar.',
      );
      return;
    }
    ...
    if (state === 'confirmation_pending') {
      ...
      setInfo(
        'Sua conta foi criada. Confirme o e-mail enviado para continuar.',
      );
      return;
    }
    ...
    if (state === 'pending') {
      setMode('pending');
      setInfo(
        'Sua solicitação está em análise. A tela será atualizada automaticamente após a aprovação.',
      );
      return;
    }
    ...
    if (state === 'approved') {
      ...
      setInfo(
        'Seu acesso foi aprovado. Cadastre sua senha para criar a conta.',
      );
      return;
    }
  ...
  }, [email, isRequest, applyResolvedState, rememberIdentity]);
  ...
  }, [email, isPending, applyResolvedState]);
  ...
  async function handleRequestAccess() {
    ...
    try {
      ...
      setError(
        String(err?.message || 'Falha ao enviar a solicitação de acesso.'),
      );
    } finally {
  ...
  async function handleAuth(event: React.FormEvent) {
    ...
    try {
      if (isActivation) {
        ...
        if (signUpError) {
          const mapped = mapSignupErrorMessage(
        ...
        if (data.session) {
          ...
          setMode('confirm-email');
          setInfo(
            'Enviamos um link de confirmação para seu e-mail. Abra a mensagem e confirme seu endereço.',
          );
        }
        return;
      }
      ...
      setError(
        mapSignupErrorMessage(String(err?.message || 'Falha na autenticação.')),
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleAdminGithub() {
    ...
    try {
      ...
      setError(
        String(err?.message || 'Falha no acesso administrativo com GitHub.'),
      );
      setLoading(false);
    }
  }
  ...
  function continueAfterConfirmation() {
    const confirmedEmail =
      readSession(CONFIRMED_EMAIL_STORAGE) || email || readLocal(STORAGE_EMAIL);
    if (confirmedEmail) {
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#050505] overflow-hidden">
      ...
      {!introDone && (
        <div className="mf-auth-intro absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          <p className="mt-3 text-[11px] uppercase font-bold tracking-[0.34em] text-white/35">
        ...
        <div className="mf-auth-card glass-card w-full max-w-md p-8 relative overflow-hidden">
          ...
          {isAdmin ? (
            <div className="space-y-4">
              ...
              <button
                type="button"
                onClick={goToNormalAccess}
                className="w-full text-[10px] uppercase font-bold tracking-widest text-white/25 hover:text-white/55 transition-colors"
              >
            ...
            <div className="space-y-4">
              {isPending && (
                <div className="p-4 rounded-xl border border-brand-primary/20 bg-brand-primary/8 text-sm text-white/65">
                  <strong className="block text-brand-primary mb-1">
                    Solicitação recebida
                  </strong>
                  Seu pedido está aguardando a aprovação do administrador. Você
                  não precisa verificar manualmente.
                </div>
              )}
              {isDenied && (
                <div className="p-4 rounded-xl border border-red-500/20 bg-red-500/8 text-sm text-red-300">
                  Sua solicitação não foi aprovada. Se acreditar que houve um
                  engano, entre em contato com o administrador.
                </div>
              )}
              {isConfirmEmail && (
                <div className="p-4 rounded-xl border border-brand-primary/20 bg-brand-primary/8 text-sm text-white/65">
                  ...
                  O link enviado para{' '}
                  <span className="text-white">{email}</span> confirma seu
                  endereço. Depois da confirmação, você volta ao MFinanceiro.
                </div>
              )}
              {isConfirmed && (
                <>
                  <div className="p-4 rounded-xl border border-green-500/20 bg-green-500/8 text-sm text-green-300">
                    <strong className="block mb-1">E-mail aprovado</strong>Seu
                    endereço foi confirmado. Clique abaixo para acessar sua
                    conta.
                  </div>
                  <button
                    type="button"
                    onClick={continueAfterConfirmation}
                    className="w-full bg-brand-primary text-black font-bold py-3 rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2 shadow-[0_4px_20px_rgba(0,242,255,0.2)]"
                  >
                    <LogIn size={20} />
                    Ir para entrar
                  </button>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              ...
              <div>
                ...
                {isRequest && checkingEmail && (
                  <p className="mt-1.5 text-[10px] text-white/25">
                    Verificando se este e-mail já possui conta...
                  </p>
                )}
              </div>
              {(isLogin || isActivation) && (
                <div>
                  ...
                  <input
                    ...
                    minLength={8}
                    autoComplete={
                      isActivation ? 'new-password' : 'current-password'
                    }
                    value={password}
              ...
              <button
                type="submit"
                disabled={loading || (isRequest && checkingEmail)}
          ...
          {isRequest && (
            <section
              ...
              aria-labelledby="mf-existing-access-title"
            >
              <div
                id="mf-existing-access-title"
                className="mb-2 flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.12em] text-white/30 before:h-px before:flex-1 before:bg-white/10 after:h-px after:flex-1 after:bg-white/10"
              >
                Já possui acesso?
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                <button
                  ...
                  disabled={loading}
                  className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.035] px-3 text-[10px] font-bold text-white/75 transition hover:border-brand-primary/30 hover:bg-brand-primary/[0.07] hover:text-white disabled:opacity-50"
                >
              ...
              <p className="mt-2 min-h-4 text-center text-[9px] leading-relaxed text-white/30">
                GitHub é validado como acesso administrativo.
              </p>
            </section>
          )}
        </div>
