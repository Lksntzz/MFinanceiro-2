...
function routeTitle(pathname: string) {
  return (
    ROUTE_TITLES.find(([pattern]) => pattern.test(pathname))?.[1] ||
    'MF Financeiro'
  );
}
...
function enhanceAccessibleNames(root: ParentNode = document) {
  root
    .querySelectorAll<HTMLElement>('button[title], a[title]')
    .forEach((element) => {
...
export default function AccessibilityLayer() {
  ...
  return (
    <>
      ...
      <div
        className="mf-visually-hidden"
        role="status"
        aria-live="polite"
        aria-atomic="true"
      >
        {inApplication
          ? `Página carregada: ${title}. Atalho de busca: Control ou Command mais K. Privacidade: Alt mais P.`
          : ''}
      </div>
    </>
  );
