...
export default function PreferencesCenter({ userId }: { userId: string }) {
  ...
  function toggleHomeWidget(id: HomeWidgetId) {
    ...
    setPreferences({
      ...preferences,
      homeWidgets: active ? preferences.homeWidgets.filter((item) => item !== id) : [...preferences.homeWidgets, id],
    });
  }
  ...
  async function exportData() {
    ...
    setMessage(null);
    try {
      await exportFinancialData(userId);
      setMessage('Exportação preparada. O arquivo contém os dados estruturados da sua conta.');
    } catch (error) {
      ...
      <section className="mf-preferences-panel">
        ...
        <div className="mf-preferences-scroll">
          ...
          <section className="mf-preference-section">
            ...
            <label className="mf-setting-row"><span><strong>Modo compacto</strong><small>Reduz a altura dos cards na Início.</small></span><input type="checkbox" checked={preferences.compactHome} onChange={(event) => setPreferences({ ...preferences, compactHome: event.target.checked })} /></label>
          </section>

          <section className="mf-preference-section">
            <div className="mf-preference-title"><Bell size={17} /><div><strong>Notificações</strong><small>Menos volume, mais relevância.</small></div></div>
            {notificationRows.map((item) => <label className="mf-setting-row" key={item.key}><span><strong>{item.label}</strong><small>{item.description}</small></span><input type="checkbox" checked={preferences.notifications[item.key]} onChange={(event) => setPreferences({ ...preferences, notifications: { ...preferences.notifications, [item.key]: event.target.checked } })} /></label>)}
          </section>
          ...
          <section className="mf-preference-section">
            <div className="mf-preference-title"><EyeOff size={17} /><div><strong>Privacidade e acessibilidade</strong><small>Controle visual sem alterar os dados.</small></div></div>
            <label className="mf-setting-row"><span><strong>Ocultar valores agora</strong><small>Atalho rápido para uso em público ou compartilhamento de tela.</small></span><input type="checkbox" checked={isPrivate} onChange={(event) => setIsPrivate(event.target.checked)} /></label>
            <label className="mf-setting-row"><span><strong>Privacidade por padrão</strong><small>Preferência usada ao abrir novas sessões.</small></span><input type="checkbox" checked={preferences.privacyDefault} onChange={(event) => setPreferences({ ...preferences, privacyDefault: event.target.checked })} /></label>
            <label className="mf-setting-row"><span><strong>Reduzir movimento</strong><small>Desativa animações não essenciais dentro do MF.</small></span><input type="checkbox" checked={preferences.reducedMotion} onChange={(event) => setPreferences({ ...preferences, reducedMotion: event.target.checked })} /></label>
            <label className="mf-setting-row"><span><strong>Contraste reforçado</strong><small>Aumenta contraste de bordas, textos auxiliares e foco.</small></span><input type="checkbox" checked={preferences.highContrast} onChange={(event) => setPreferences({ ...preferences, highContrast: event.target.checked })} /></label>
          </section>

          <section className="mf-preference-section">
            <div className="mf-preference-title"><ShieldCheck size={17} /><div><strong>Seus dados</strong><small>Portabilidade sem incluir arquivos privados automaticamente.</small></div></div>
            <button type="button" className="mf-preference-primary" onClick={() => void exportData()} disabled={exporting}><Download size={15} />{exporting ? 'Preparando exportação…' : 'Exportar meus dados'}</button>
          </section>

          <section className="mf-preference-section">
            <div className="mf-preference-title"><Activity size={17} /><div><strong>Atividade recente</strong><small>Registro de ações importantes para conferência.</small></div></div>
            <div className="mf-activity-list">
              {activity.length ? activity.slice(0, 12).map((item) => <article key={item.id}><span><strong>{item.summary}</strong><small>{item.entity_type} · {formatActivityDate(item.created_at)}</small></span></article>) : <p>Nenhuma alteração relevante registrada neste dispositivo ainda.</p>}
            </div>
          </section>
        </div>
      </section>
