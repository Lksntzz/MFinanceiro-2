...
export default function TransactionLaunchTool({ user }: { user: User }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [settingsResult, accountsResult, categoriesResult, cardsResult] =
        await Promise.all([
          supabase
      ...
      setForm((current) => ({
        ...
        accountId:
          ...
          nextAccounts.find((account) => account.is_active)?.id ||
          '',
      }));
    } catch (refreshError: any) {
      setError(
        refreshError?.message || 'Não foi possível preparar o lançamento.',
      );
    } finally {
      setLoading(false);
    }
    ...
    () =>
      categories.filter(
        (category) =>
          ...
          category.category_type === form.type,
      ),
  ...
  useEffect(() => {
    if (
      selectableCategories.some((category) => category.id === form.categoryId)
    )
      return;
  ...
  async function save(event: React.FormEvent) {
    ...
    if (!form.accountId) {
      setError('Selecione a conta relacionada ao lançamento.');
      return;
    }
    ...
    setError(null);
    setSuccess(false);
    // biome-ignore format: contract check requires single line tag
    const { error: rpcError } = await supabase.rpc('mf_create_finance_entry_v3',
      {
        ...
        p_source: 'Manual',
        p_card_id:
          form.paymentMethod === 'credit_card' && form.cardId
            ? form.cardId
            : null,
    ...
    <div className="mf-app-shell mf-routed-app">
      ...
      <header className="mf-topbar">
        ...
        <div className="mf-top-actions">
          ...
          <button type="button" onClick={() => navigate('/app')} title="Fechar">
            <X size={17} />
          </button>
          <button
            type="button"
        ...
        <div className="mx-auto w-full max-w-3xl space-y-4 animate-fade-in">
          <div>
            ...
            <p className="text-sm text-white/40">
              Uma única entrada para registrar dinheiro que entrou, saiu ou
              ainda está previsto.
            </p>
          </div>
          ...
          {success && (
            <div className="flex items-center gap-2 rounded-xl border border-green-500/25 bg-green-500/10 px-4 py-3 text-xs text-green-200">
          ...
          {loading ? (
            <div className="mf-loading">Preparando lançamento...</div>
          ) : (
            <form onSubmit={save} className="glass-card space-y-4 !p-5">
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-bold ${form.type === 'expense' ? 'border-red-400/30 bg-red-500/10 text-red-300' : 'border-white/10 bg-white/[0.03] text-white/45'}`}
                >
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-bold ${form.type === 'income' ? 'border-green-400/30 bg-green-500/10 text-green-300' : 'border-white/10 bg-white/[0.03] text-white/45'}`}
                >
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Situação">
                  ...
                  >
                    ...
                    <option value="pending">Previsto / pendente</option>
                  </select>
                </Field>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-4">
                <p className="flex items-center gap-2 text-[10px] text-white/35">
                  ...
                  Lançamentos pendentes ficam previstos; realizados passam a
                  compor o saldo.
                </p>
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => navigate(-1)}
                    className="rounded-xl border border-white/10 px-4 py-2.5 text-xs"
                  >
                    Cancelar
                  </button>
                  <button
                    disabled={saving}
                    className="flex items-center gap-2 rounded-xl bg-brand-primary px-5 py-2.5 text-xs font-black text-black disabled:opacity-50"
                  >
                    <Save size={14} />
  ...
  return (
    <label className="block text-[10px] font-bold uppercase tracking-wider text-white/40">
      ...
      {React.cloneElement(children, {
        className:
          'mt-1 w-full rounded-xl border border-white/10 bg-[#121212] px-3 py-3 text-sm text-white outline-none focus:border-brand-primary',
      })}
    </label>
  );
}
