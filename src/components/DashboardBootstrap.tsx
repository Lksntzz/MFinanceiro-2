...
export default function DashboardBootstrap({ user, isMaintenanceBypass }: { user: User; isMaintenanceBypass?: boolean }) {
    ...
    const prepareDashboard = async () => {
      ...
      try {
        for (let attempt = 0; attempt < 6; attempt += 1) {
          ...
          const { data: existing, error: selectError } = await supabase.from('mf_user_settings').select('id,user_id').eq('user_id', user.id).maybeSingle();
          if (selectError) throw selectError;
          if (existing) { if (active) setReady(true); return; }
          const { error: upsertError } = await supabase.from('mf_user_settings').upsert(DEFAULT_USER_SETTINGS(user.id), { onConflict: 'user_id', ignoreDuplicates: true });
  ...
  if (ready && shareRoute) return <Suspense fallback={<div className="mf-mobile-loading"><span>Recebendo conteúdo no MF...</span></div>}><MobileShareReceiver userId={user.id} /></Suspense>;
  if (ready && voiceRoute) return <Suspense fallback={<div className="mf-mobile-loading"><span>Carregando MF Voice...</span></div>}><MobileVoice userId={user.id} /></Suspense>;
  if (ready && recurrenceRoute) return <Suspense fallback={<div className="mf-mobile-loading"><span>Procurando recorrências...</span></div>}><MobileRecurrences userId={user.id} /></Suspense>;
  if (ready && mobileExperience && launchRoute) return <Navigate to="/quick" replace />;
  if (ready && mobileExperience) return <Suspense fallback={<div className="mf-mobile-loading"><span>Carregando MF Mobile...</span></div>}><MobileApp user={user} /></Suspense>;
  ...
  if (ready) {
    ...
    return <>
      <Suspense fallback={<div className="flex h-screen items-center justify-center bg-[#050505] text-sm text-white/55">Carregando ferramenta…</div>}>{tool}</Suspense>
  ...
  return <div className="flex h-screen w-full items-center justify-center bg-[#050505] p-6 text-white"><div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/5 p-6 text-center">{error ? <><AlertCircle className="mx-auto mb-3 text-red-400" size={30} /><h1 className="mb-2 text-lg font-bold">Não foi possível carregar sua conta</h1><p className="mb-5 text-sm text-white/50">{error}</p><button type="button" onClick={() => setRetryKey((value) => value + 1)} className="mx-auto flex items-center gap-2 rounded-xl bg-[#00f2ff] px-4 py-2 text-sm font-bold text-black"><RefreshCw size={16} />Tentar novamente</button></> : <><div className="mx-auto mb-4 h-10 w-10 animate-spin rounded-full border-4 border-[#00f2ff] border-t-transparent" /><p className="text-sm text-white/60">Carregando seus dados financeiros...</p></>}</div></div>;
}
