...
import UserExperiencePreferences from './UserExperiencePreferences';

const Dashboard = lazy(() => import('./Dashboard'));
const FinancialAccountsTool = lazy(() => import('./FinancialAccountsTool'));
...
const TransactionLaunchTool = lazy(() => import('./TransactionLaunchTool'));

const MobileApp = lazy(() => import('../mobile/MobileApp'));
const MobileShareReceiver = lazy(
  () => import('../mobile/pages/MobileShareReceiver'),
);
const MobileVoice = lazy(() => import('../mobile/pages/MobileVoice'));
const MobileRecurrences = lazy(
  () => import('../mobile/pages/MobileRecurrences'),
);
...
export default function DashboardBootstrap({
  ...
  const legacyInvestmentRoute = location.pathname.startsWith(
    '/app/planejamento/investimentos',
  );
  const legacyAutomationRoute = location.pathname.startsWith(
    '/app/planejamento/automacoes',
  );
  const legacyGoalsRoute = location.pathname.startsWith('/app/analises/metas');
  const legacyFixedRoute = location.pathname.startsWith(
    '/app/planejamento/contas-fixas',
  );
  const legacySubscriptionsRoute = location.pathname.startsWith(
    '/app/planejamento/assinaturas',
  );
  const legacyCommitmentsRoute = location.pathname.startsWith(
    '/app/planejamento/compromissos',
  );
  const legacyIncomeRoute =
    location.pathname.startsWith('/app/planejamento/receitas') ||
    location.pathname.startsWith('/app/planejamento/renda');
  const legacyCalendarRoute = location.pathname.startsWith(
    '/app/planejamento/calendario',
  );
  const legacyPreferencesRoute =
    location.pathname.startsWith('/app/preferencias');
  const legacyAgendaFixedRoute = location.pathname.startsWith(
    '/app/agenda/contas-fixas',
  );
  const legacyAgendaSubscriptionsRoute = location.pathname.startsWith(
    '/app/agenda/assinaturas',
  );
  const legacyAnalysisOverviewRoute =
    location.pathname === '/app/analises' ||
    location.pathname.startsWith('/app/analises/resumo');
  const legacyFinancialHealthRoute = location.pathname.startsWith(
    '/app/analises/saude',
  );
  ...
  const betaInvestmentRoute = location.pathname.startsWith(
    '/app/beta/investimentos',
  );
  ...
  const accountManagementRoute =
    location.pathname === '/app/planejamento/contas';
  const categoryManagementRoute =
    location.pathname === '/app/planejamento/categorias';
  const planningStrategyRoute =
    location.pathname.startsWith('/app/planejamento/metas') ||
    location.pathname.startsWith('/app/planejamento/projecoes');
  const planningRoute =
    location.pathname.startsWith('/app/planejamento') &&
      ...
      try {
        for (let attempt = 0; attempt < 6; attempt += 1) {
          ...
          const { error: upsertError } = await supabase
            .from('mf_user_settings')
            .upsert(DEFAULT_USER_SETTINGS(user.id), {
              onConflict: 'user_id',
        ...
        throw new Error(
          'A sessão ainda não conseguiu acessar suas configurações.',
        );
      } catch (err: any) {
        console.error('Dashboard bootstrap error:', err);
        if (active)
          setError(
            err?.message || 'Não foi possível preparar seus dados financeiros.',
          );
      }
    };
      ...
      <Suspense
        fallback={
          <div className="mf-mobile-loading">
            <span>Recebendo conteúdo no MF...</span>
          </div>
        }
      >
  ...
  if (ready && recurrenceRoute)
    return (
      <Suspense
        fallback={
          <div className="mf-mobile-loading">
            <span>Procurando recorrências...</span>
          </div>
        }
      >
  ...
  if (ready && mobileExperience && launchRoute)
  ...
  if (ready && mobileExperience)
    return (
      <Suspense
        fallback={
          <div className="mf-mobile-loading">
            <span>Carregando MF Mobile...</span>
          </div>
        }
      >
        <MobileApp user={user} />
  ...
  if (
    investmentRoute &&
    new URLSearchParams(location.search).get('section') === 'income'
  )
    return <Navigate to="/app/investimentos" replace />;
  if (legacyAutomationRoute) return <Navigate to="/app/integracoes" replace />;
  if (legacyGoalsRoute)
    return <Navigate to="/app/planejamento/metas" replace />;
  if (legacyFixedRoute)
    return <Navigate to="/app/agenda/recorrencias?tipo=fixas" replace />;
  if (legacySubscriptionsRoute)
    return <Navigate to="/app/agenda/recorrencias?tipo=assinaturas" replace />;
      ...
      <Navigate
        to={
          requestedKind === 'assinaturas'
            ? '/app/agenda/recorrencias?tipo=assinaturas'
            : '/app/agenda/recorrencias?tipo=fixas'
        }
        replace
      />
    );
  ...
  if (legacyPreferencesRoute) return <Navigate to="/app" replace />;
  if (legacyAgendaFixedRoute)
    return <Navigate to="/app/agenda/recorrencias?tipo=fixas" replace />;
  if (legacyAgendaSubscriptionsRoute)
    return <Navigate to="/app/agenda/recorrencias?tipo=assinaturas" replace />;
  if (legacyAnalysisOverviewRoute) return <Navigate to="/app" replace />;
  if (legacyFinancialHealthRoute)
  ...
  if (ready) {
    ...
    else if (accountManagementRoute)
      tool = <FinancialAccountsTool user={user} />;
    else if (categoryManagementRoute)
      tool = <FinancialCategoriesTool user={user} />;
    ...
    return (
      <>
        <Suspense
          fallback={
            <div className="flex h-screen items-center justify-center bg-[#050505] text-sm text-white/55">
        ...
        <UserExperiencePreferences userId={user.id} />
