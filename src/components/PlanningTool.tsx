...
function getSection(pathname: string): PlanningSection {
  ...
  if (
    path.endsWith('/compromissos') ||
    path.endsWith('/contas-fixas') ||
    path.endsWith('/assinaturas')
  )
    return 'commitments';
...
export default function PlanningTool({ user }: { user: User }) {
  ...
  const commitmentKind: CommitmentKind =
    pathname.endsWith('/assinaturas') || search.get('tipo') === 'assinaturas'
      ? 'subscriptions'
      : 'fixed';
  ...
  const [editingInstallment, setEditingInstallment] =
    useState<CardInstallment | null>(null);
    ...
    try {
      ...
      const [
        ...
        supabase
          ...
          .select('id,user_id,amount,category,description,date,type,status')
          .eq('user_id', user.id)
          .gte('date', monthStart)
          .lte('date', monthEnd),
      ]);
      ...
      const balance = nextAccounts.reduce(
      ...
      setBudgets(
        (budgetsResult.data || []).map((item: any) => ({
          ...item,
          limit_amount: Number(item.limit_amount || 0),
        })) as BudgetRow[],
      );
      setMonthTransactions(
        (transactionsResult.data || []).map((item: any) => ({
          ...item,
          amount: Number(item.amount || 0),
        })) as Transaction[],
      );
      ...
      setError(
        refreshError?.message || 'Não foi possível carregar o planejamento.',
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (pathname.endsWith('/calendario'))
      window.history.replaceState({}, '', '/app/planejamento');
    if (pathname.endsWith('/renda'))
      window.history.replaceState({}, '', '/app/planejamento/receitas');
    if (pathname.endsWith('/contas-fixas'))
      window.history.replaceState(
        {},
        '',
        '/app/planejamento/compromissos?tipo=fixas',
      );
    if (pathname.endsWith('/assinaturas'))
      window.history.replaceState(
        {},
        '',
        '/app/planejamento/compromissos?tipo=assinaturas',
      );
  }, [pathname]);
  ...
  const projectedBalance =
    balance + expectedIncome - pendingFixed - subscriptionMonthly - cardsUsed;
  const hasPlanningAccount = accounts.some(
    (account) => account.is_active !== false,
  );
  const hasPlanningIncome = expectedIncome > 0;
  const hasPlanningCommitments =
    fixedBills.some((bill: any) => bill.active !== false) ||
  ...
  const expenseCategories = categories.filter(
    (item) => item.category_type === 'expense' || item.category_type === 'both',
  );
  ...
  async function payInstallment(item: CardInstallment) {
    const result = await supabase.rpc('mf_pay_card_installment', {
  ...
  async function payCardBill(card: CreditCard) {
    const result = await supabase.rpc('mf_pay_credit_card_bill_v2', {
      p_card_id: card.id,
    });
    if (result.error) setError(result.error.message);
    else await refresh();
              ...
              <div className="space-y-4 animate-fade-in">
                <div>
                  ...
                  <p className="text-sm text-white/40">
                    O que está previsto para acontecer com seu dinheiro neste
                    mês.
                  </p>
                </div>
                <PlanningJourney
                ...
                <section className="grid grid-cols-2 gap-3 xl:grid-cols-5">
                  <Metric
                    label="Saldo atual"
                    value={formatCurrency(balance, isPrivate)}
                    icon={<Wallet size={16} />}
                  />
                  <Metric
                    label="Receitas previstas"
                    value={formatCurrency(expectedIncome, isPrivate)}
                    detail="Baseadas na renda cadastrada"
                    icon={<TrendingUp size={16} />}
                  />
                  <Metric
                    label="Contas recorrentes"
                    value={formatCurrency(
                      pendingFixed + subscriptionMonthly,
                      isPrivate,
                    )}
                    icon={<ReceiptText size={16} />}
                  />
                  <Metric
                    label="Cartões em aberto"
                    value={formatCurrency(cardsUsed, isPrivate)}
                    icon={<CreditCardIcon size={16} />}
                  />
                  <Metric
                    label="Projeção do mês"
                    value={formatCurrency(projectedBalance, isPrivate)}
                    detail="Saldo + receitas - compromissos"
                ...
                <div className="grid gap-4 xl:grid-cols-3">
                  <article className="mf-card xl:col-span-1">
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between">
                        ...
                        <strong>
                          {formatCurrency(pendingFixed, isPrivate)}
                        </strong>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/45">Assinaturas/mês</span>
                        <strong>
                          {formatCurrency(subscriptionMonthly, isPrivate)}
                        </strong>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/45">
                        ...
                        <strong>{formatCurrency(cardsUsed, isPrivate)}</strong>
                      </div>
                  ...
                  <article className="mf-card xl:col-span-2">
                    ...
                    <p className="mb-3 text-xs text-white/35">
                      Calendário integrado ao planejamento: vencimentos e
                      entradas previstas.
                    </p>
                    <FinancialCalendar
            ...
            {section === 'commitments' && (
              <div className="space-y-4">
                <div className="mf-subnav">
                  <button
                    className={commitmentKind === 'fixed' ? 'active' : ''}
                    onClick={() =>
                      navigate('/app/planejamento/compromissos?tipo=fixas')
                    }
                  >
                    Contas fixas
                  </button>
                  <button
                    className={
                      commitmentKind === 'subscriptions' ? 'active' : ''
                    }
                    onClick={() =>
                      navigate(
                        '/app/planejamento/compromissos?tipo=assinaturas',
            ...
            {section === 'budget' && (
              <div className="space-y-4 animate-fade-in">
                <div>
                  ...
                  <p className="text-sm text-white/40">
                    Defina quanto pretende gastar por categoria e acompanhe o
                    consumo do mês.
                  </p>
                </div>
                <form
                  onSubmit={saveBudget}
                  className="mf-card grid gap-3 md:grid-cols-[1fr_180px_auto] md:items-end"
                >
                  ...
                  <button
                    className="primary rounded-xl px-4 py-3 text-sm font-bold"
                ...
                <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {budgets.length === 0 ? (
                    <div className="mf-card text-sm text-white/40">
                    ...
                    budgets.map((item) => {
                      ...
                      const percentage =
                        item.limit_amount > 0
                          ? Math.min(100, (spent / item.limit_amount) * 100)
                          : 0;
                      ...
                      return (
                        <article key={item.id} className="mf-card">
                          ...
                          <div className="mt-4 flex justify-between text-sm">
                            <span>Gasto</span>
                            <strong>{formatCurrency(spent, isPrivate)}</strong>
                          </div>
                          ...
                          <div className="mt-3 flex justify-between text-xs text-white/45">
                            <span>
                              Limite{' '}
                              {formatCurrency(item.limit_amount, isPrivate)}
                            </span>
                            <span
                              className={
                                remaining < 0
                            ...
                            >
                              ...
                              {formatCurrency(Math.abs(remaining), isPrivate)}
                            </span>
                          </div>
                          <button
                            type="button"
                            onClick={() => {
                              ...
                              setBudgetLimit(String(item.limit_amount));
                            }}
                            className="mt-3 flex items-center gap-1 text-xs text-white/45 hover:text-white"
                          >
                            <Pencil size={12} />
                            Editar limite
                          </button>
          ...
          <form className="mf-modal compact" onSubmit={saveCard}>
            ...
            <label>
              ...
              <input
                ...
                value={cardForm.used}
                onChange={(event) =>
                  setCardForm({ ...cardForm, used: event.target.value })
                }
              />
            </label>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setInstallmentForm({
                      ...installmentForm,
                      current_installment: event.target.value,
                    })
                  }
                />
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setInstallmentForm({
                      ...installmentForm,
                      total_installments: event.target.value,
                    })
                  }
                />
              </label>
