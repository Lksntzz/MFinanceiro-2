...
export default function ProfileCenter({
  ...
  const friendlyName =
    ...
    'Perfil';
  const activeAccount =
    accounts.find((account) => account.is_default && account.is_active) ||
  ...
  async function saveProfile(event: React.FormEvent) {
    ...
    try {
      ...
      if (authUpdateError)
        console.warn(
          'Não foi possível sincronizar o nome no metadata de autenticação:',
          authUpdateError,
        );
    ...
    open && typeof document !== 'undefined'
      ? createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="mf-profile-title"
          >
            <form className="mf-modal" onSubmit={saveProfile}>
              <div className="mf-modal-title">
                <div>
                  ...
                  <p className="mt-1 text-[10px] text-white/35">
                    Foto, nome e informações úteis da sua conta.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <section className="grid grid-cols-2 gap-2">
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                  ...
                  <strong className="mt-1 block truncate text-[11px] text-white/75">
                ...
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                  ...
                  <strong className="mt-1 block truncate text-[11px] text-white/75">
                    {activeAccount?.name || 'Não definida'}
                  </strong>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                  <p className="text-[8px] font-black uppercase tracking-[0.16em] text-white/30">
                  ...
                  <strong className="mt-1 block text-sm text-white/75">
                    {activeAccountsCount}
                  </strong>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                  ...
                  <strong className="mt-1 block text-[11px] text-white/75">
                    {memberSince}
                  </strong>
                </div>
              </section>
              ...
              <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-3">
                <p className="mb-2 text-[9px] font-black uppercase tracking-[0.18em] text-white/35">
                  Atalhos da conta
                </p>
                <div className="grid gap-2 sm:grid-cols-2">
                  <button
                    type="button"
                    onClick={openIncome}
                    className="flex items-center justify-between rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-left transition hover:border-brand-primary/25 hover:bg-brand-primary/[0.05]"
                  >
                    <span className="flex items-center gap-2">
                      <Banknote size={15} className="text-brand-primary" />
                      <span>
                        <strong className="block text-xs text-white">
                          Renda e Folha
                        </strong>
                        <small className="text-[9px] text-white/35">
                          Holerite, salário e ciclo
                        </small>
                      </span>
                    </span>
                  ...
                  >
                    <span className="flex items-center gap-2">
                      <WalletCards size={15} className="text-brand-primary" />
                      <span>
                        <strong className="block text-xs text-white">
                          Contas financeiras
                        </strong>
                        <small className="text-[9px] text-white/35">
                          Saldos e conta principal
                        </small>
                      </span>
                    </span>
                    <ChevronRight size={14} className="text-white/25" />
              ...
              {isAdmin && (
                <section className="rounded-2xl border border-amber-400/20 bg-amber-400/[0.06] p-3">
                  <p className="mb-2 text-[9px] font-black uppercase tracking-[0.18em] text-amber-300/80">
                    Administração
                  </p>
                  <button
                    type="button"
                    ...
                    <span className="flex items-center gap-3">
                      ...
                      <span>
                        ...
                        <small className="mt-0.5 block text-[9px] text-white/35">
                          Solicitações de acesso e controle de manutenção
                        </small>
                      </span>
                    </span>
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 p-1 pr-3 text-[10px] font-bold text-white/70"
        title="Perfil"
      >
