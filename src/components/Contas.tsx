    ...
    <div className="flex-1 flex flex-col gap-4 overflow-y-auto no-scrollbar pb-8 animate-fade-in">
      ...
      <div className="glass-card !p-4 flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-xs text-white/40">Controle das suas despesas e compromissos recorrentes.</p>
        </div>
        <div className="flex gap-6">
          <div className="text-right">
            <span className="text-[10px] text-white/40 uppercase font-bold">Total Comprometido</span>
            <div className="text-lg font-bold text-brand-primary">R$ {totalCommitted.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
          </div>
          <div className="text-right">
            <span className="text-[10px] text-white/40 uppercase font-bold">Contas Fixas</span>
            <div className="text-lg font-bold">R$ {totalFixed.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
          </div>
          <div className="text-right">
            <span className="text-[10px] text-white/40 uppercase font-bold">Dia a Dia (Est.)</span>
                ...
                <div key={bill.id} className="p-3 bg-white/5 rounded-xl border border-white/5 flex items-center justify-between group hover:border-white/10 transition-all">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <div className={`text-[10px] uppercase font-bold ${bill.status === 'paid' ? 'text-green-400' : 'text-brand-primary'}`}>
              ...
              {dailyBills.map(bill => (
                <div key={bill.id} className="p-3 bg-white/5 rounded-xl border border-white/5 flex items-center justify-between group hover:border-white/10 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-sm">{bill.name}</div>
                      <div className="text-[10px] text-white/40 uppercase">{bill.category} • {bill.frequency === 'weekly' ? 'Semanal' : 'Mensal'}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="font-bold text-sm">R$ {bill.average_amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
                      <div className="text-[10px] text-white/40 uppercase">Média Est.</div>
                    </div>
      ...
      <div className="grid grid-cols-12 gap-4 shrink-0">
        <div className="col-span-8 glass-card !p-4 flex items-center gap-8">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-full border-4 border-brand-primary border-t-brand-secondary flex items-center justify-center text-[10px] font-bold">
              {Math.round((totalCommitted / summary.currentBalance) * 100)}%
            </div>
            <div>
              <h4 className="font-bold text-sm">Impacto no Orçamento</h4>
              <p className="text-xs text-white/40">Comprometimento do saldo atual com contas.</p>
            </div>
          </div>
          
          <div className="flex-1 flex gap-4">
            <div className="flex-1 p-3 bg-white/5 rounded-xl border border-white/5">
              <span className="text-[10px] text-white/40 uppercase font-bold">Sobra Estimada</span>
            ...
            <div className="flex-1 p-3 bg-white/5 rounded-xl border border-white/5">
              <span className="text-[10px] text-white/40 uppercase font-bold">Peso Fixo</span>
              <div className="text-sm font-bold text-brand-primary">{Math.round((totalFixed / totalCommitted) * 100)}% do total</div>
            </div>
          </div>
        </div>

        <div className="col-span-4 glass-card !p-4 flex flex-col justify-center gap-3">
          <h4 className="font-bold text-xs uppercase tracking-widest text-white/40 flex items-center gap-2">
            <AlertCircle size={14} /> Alertas de Contas
          </h4>
          <div className="space-y-2">
            {fixedBills.filter(b => b.status === 'pending' && b.due_day <= new Date().getDate() + 3).map(b => (
              <div key={b.id} className="flex items-center gap-2 text-[10px] text-brand-primary font-bold animate-pulse">
                <Clock size={12} />
                <span>{b.name} vence em breve (Dia {b.due_day})</span>
              </div>
            ))}
            {fixedBills.filter(b => b.status === 'pending' && b.due_day < new Date().getDate()).map(b => (
              <div key={b.id} className="flex items-center gap-2 text-[10px] text-red-400 font-bold">
                <AlertCircle size={12} />
