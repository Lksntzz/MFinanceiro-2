...
export default function AdminMfaSecurity() {
  ...
  useEffect(() => {
    ...
    void refreshSecurity()
      .catch((loadError: any) => {
        if (active)
          setError(
            loadError?.message || 'Não foi possível verificar a proteção MFA.',
          );
      })
      .finally(() => {
  ...
  async function startEnrollment() {
    ...
    try {
      ...
      setError(
        enrollError?.message || 'Não foi possível iniciar a configuração MFA.',
      );
    } finally {
      setBusy(false);
    }
  ...
  async function challengeAndVerify(factorId: string) {
    ...
    try {
      ...
      await refreshSecurity();
      setNotice(
        'Segundo fator confirmado. Esta sessão administrativa está protegida com AAL2.',
      );
    } catch (verifyError: any) {
      setError(
        verifyError?.message ||
          'Não foi possível confirmar o segundo fator. Confira o código e tente novamente.',
      );
    } finally {
      setBusy(false);
    }
  ...
  if (loading) {
    return (
      <section className="glass-card flex items-center gap-3 p-5 text-sm text-white/50">
        <Loader2 className="animate-spin" size={18} />
        Verificando segurança administrativa...
      </section>
    );
  }
    ...
    <section className="glass-card space-y-4 p-5">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <div
            className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${protectedSession ? 'bg-emerald-500/10 text-emerald-300' : 'bg-amber-500/10 text-amber-300'}`}
          >
            <ShieldCheck size={21} />
          </div>
          <div>
            ...
            <p className="mt-1 max-w-2xl text-xs leading-relaxed text-white/45">
              Alterações globais e decisões de acesso devem usar uma sessão com
              segundo fator confirmado.
            </p>
          </div>
        </div>
        <span
          className={`rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-wider ${protectedSession ? 'border-emerald-400/20 bg-emerald-500/10 text-emerald-300' : 'border-amber-400/20 bg-amber-500/10 text-amber-300'}`}
        >
          {protectedSession
            ? 'AAL2 ativo'
            : enrolled
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-emerald-400/20 bg-emerald-500/5 p-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-white/45">
              O segundo fator foi confirmado nesta sessão. Operações
              administrativas protegidas podem prosseguir.
            </p>
          </div>
        </div>
      ) : enrollment ? (
        <div className="grid gap-4 rounded-2xl border border-white/10 bg-black/20 p-4 md:grid-cols-[180px_minmax(0,1fr)]">
          ...
          <div className="min-w-0">
            <div className="flex items-start justify-between gap-3">
              <div>
                <strong className="text-sm">
                  Escaneie no seu autenticador
                </strong>
                <p className="mt-1 text-xs leading-relaxed text-white/45">
                  Depois digite o código temporário gerado pelo aplicativo para
            ...
            <details className="mt-3 rounded-xl border border-white/10 bg-white/[0.025] p-3">
              <summary className="cursor-pointer text-[10px] font-bold text-white/50">
                Não consigo ler o QR Code
              </summary>
              <code className="mt-2 block break-all text-[10px] text-white/60">
                {enrollment.secret}
              </code>
            </details>
            <div className="mt-3 flex gap-2">
              ...
              <button
                type="button"
                onClick={() => void challengeAndVerify(enrollment.factorId)}
                disabled={busy}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <strong className="text-xs text-amber-200">
                Confirme o segundo fator nesta sessão
              </strong>
              <p className="mt-1 text-xs text-white/45">
                Abra seu aplicativo autenticador e informe o código atual antes
                de executar operações administrativas protegidas.
              </p>
            </div>
          </div>
          <div className="mt-3 flex gap-2">
            ...
            <button
              type="button"
              onClick={() => void challengeAndVerify(verifiedFactor.id)}
              disabled={busy}
              className="flex items-center gap-2 rounded-xl bg-amber-300 px-4 py-2 text-xs font-black text-black disabled:opacity-50"
            >
              {busy ? (
                <Loader2 className="animate-spin" size={15} />
              ) : (
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-amber-400/20 bg-amber-500/5 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-white/45">
                Configure um aplicativo autenticador antes da publicação das
                políticas que exigem AAL2.
              </p>
            </div>
          </div>
          <button
            ...
            disabled={busy}
            className="flex shrink-0 items-center justify-center gap-2 rounded-xl border border-amber-300/20 bg-amber-300/10 px-4 py-2 text-xs font-black text-amber-200 hover:bg-amber-300/15 disabled:opacity-50"
          >
            {busy ? (
              <Loader2 className="animate-spin" size={15} />
            ) : (
