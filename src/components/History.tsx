...
} from '../mobile/lib/adaptive-category';
...
export default function History({
  ...
  const balanceState = useMemo(() => {
    ...
    const allNet = roundMoney(
      realized.reduce((sum, transaction) => sum + signedAmount(transaction), 0),
    );
    ...
    realized.forEach((transaction) => {
      ...
      byDay.set(
        key,
        roundMoney((byDay.get(key) || 0) + signedAmount(transaction)),
      );
    });
      ...
      .map(([key, items]) => {
        ...
        return {
          ...
          expense: items
            ...
            .reduce((sum, item) => sum + Math.abs(Number(item.amount) || 0), 0),
          closingBalance:
            key === 'sem-data' ? undefined : balanceState.dayClosing.get(key),
        };
      });
  ...
  async function organizeGenericCategories() {
    ...
    try {
      const { data, error: loadError } = await supabase
        ...
        .select(
          'id,user_id,date,description,source,type,amount,account_id,category,category_id',
        )
        .eq('user_id', userId)
      ...
      const candidates = (data || []).filter((entry: any) =>
        isGenericCategory(entry.category),
      );
      if (!candidates.length) {
        setOrganizerMessage(
          'Não há lançamentos genéricos para organizar agora.',
        );
        return;
      }

      const { data: previewData, error: previewError } = await supabase.rpc(
        'mf_preview_categorization_rules',
        {
      ...
      const matches = ((previewData || []) as CategorizationPreview[]).filter(
        (item) =>
          item.entry_id &&
          item.rule_id &&
          item.category_id &&
      ...
      const matchedEntryIds = new Set(
        matches.map((item) => String(item.entry_id)),
      );
      const remainingCandidates = candidates.filter(
        (candidate) => !matchedEntryIds.has(String(candidate.id)),
      );
      const historyEntries = (data || []).filter(
        (entry: any) => !isGenericCategory(entry.category),
      );
      ...
      if (!allMatches.length) {
        setOrganizerMessage(
          `Encontrei ${candidates.length} lançamento${candidates.length === 1 ? '' : 's'} genérico${candidates.length === 1 ? '' : 's'}, mas nenhuma regra automática segura ou similaridade histórica se aplica ainda. Crie uma regra para o MF poder organizar em lote.`,
        );
        return;
      }
      ...
      if (adaptiveMatches.length > 0) {
        const { data: existingRules } = await supabase
          ...
          .select('match_value,category_id,transaction_type')
          .eq('user_id', userId);

        const existingRuleSet = new Set(
          (existingRules || []).map(
            (r: any) =>
              `${r.transaction_type}:${r.category_id}:${String(r.match_value).toLowerCase().trim()}`,
          ),
        );
        ...
        for (const match of adaptiveMatches) {
          const candidate = candidates.find(
            (c: any) => c.id === match.entry_id,
          );
          if (!candidate) continue;

          const rawValue = candidate.description || candidate.source || '';
          const value =
            adaptiveMerchantKey(rawValue) || rawValue.trim().toLowerCase();
          if (value.length < 2) continue;

          const key = `${candidate.type}:${match.category_id}:${value}`;
          if (seenRuleKeys.has(key)) continue;
          seenRuleKeys.add(key);

          if (
            !existingRuleSet.has(
              `${candidate.type}:${match.category_id}:${value.toLowerCase().trim()}`,
            )
          ) {
            rulesToInsert.push({
              user_id: userId,
              name: `Auto · ${match.category_name} · ${value.slice(0, 50)}`,
              priority: 60,
              match_field: 'description',
              match_operator: 'contains',
              match_value: value.slice(0, 160),
        ...
        if (rulesToInsert.length > 0) {
          ...
          if (insertRulesError) {
            console.warn(
              'Falha ao salvar regras aprendidas:',
              insertRulesError,
            );
          }
        }
      ...
      const byCategory = new Map<
        string,
        { categoryId: string; categoryName: string; ids: string[] }
      >();
      ...
      setOrganizerMessage(
        `${updated} lançamento${updated === 1 ? '' : 's'} organizado${updated === 1 ? '' : 's'}${detailsText}.${pending > 0 ? ` ${pending} continuam genéricos porque ainda não há regra segura para eles.` : ''}`,
      );
    } catch (error) {
      setOrganizerMessage(
        error instanceof Error
          ? error.message
          : 'Não foi possível organizar os lançamentos genéricos.',
      );
    } finally {
      setOrganizing(false);
    }
  ...
  async function exportHistory() {
    ...
    await downloadXlsx(
      `MFinanceiro_Historico_${new Date().toISOString().slice(0, 10)}.xlsx`,
      [
        {
          name: 'Lançamentos',
          rows: [
  ...
  return (
    <section className="history-shell flex h-full min-h-0 flex-col gap-3 overflow-hidden animate-fade-in">
      ...
      {assistantOpen && (
        <div className="shrink-0 rounded-xl border border-amber-400/20 bg-amber-400/[0.055] px-4 py-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div>
                <strong className="text-xs text-white/85">
                  Organizar lançamentos genéricos
                </strong>
                <p className="mt-1 text-[10px] leading-relaxed text-white/45">
                  O MF testa suas regras de categorização e aplica em lote
                  somente correspondências determinísticas. Nenhum lançamento é
                  classificado por palpite.
                </p>
                {organizerMessage && (
                  <p
                    className="mt-2 text-[10px] font-semibold text-amber-100/80"
                    role="status"
                  >
                    {organizerMessage}
                  </p>
            ...
            <div className="flex shrink-0 gap-2">
              <button
                ...
                className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-white/60"
              >
                Criar regras
              </button>
              <button
                type="button"
                onClick={() => void organizeGenericCategories()}
                disabled={organizing}
                className="flex items-center gap-2 rounded-lg bg-amber-300 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-black disabled:opacity-50"
              >
                {organizing && <Loader2 size={12} className="animate-spin" />}
                {organizing ? 'Organizando…' : 'Aplicar regras'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="grid shrink-0 grid-cols-2 gap-2 lg:grid-cols-4">
        <div className="glass-card !p-3">
          ...
          <div className="truncate text-sm font-bold text-green-400">
        ...
        <div className="glass-card !p-3">
          ...
          <div
            className={`truncate text-sm font-bold ${summary.net >= 0 ? 'text-brand-primary' : 'text-red-400'}`}
          >
        ...
        <div className="glass-card !p-3">
          ...
          <div className="truncate text-sm font-bold text-brand-primary">
      ...
      <div className="shrink-0 rounded-xl border border-brand-primary/15 bg-brand-primary/[0.055] px-3 py-2 text-[10px] leading-relaxed text-white/55">
        O saldo atual é derivado das contas e está{' '}
        {balanceConfirmed
          ? 'confirmado pelo usuário'
          : 'aguardando conferência'}
        . Base anterior aos {transactions.length} lançamentos carregados:{' '}
        <strong className="text-white/85">
          {formatMoney(balanceState.openingBase)}
        </strong>
        .
      </div>
      ...
      <div className="history-scroll min-h-0 flex-1 overflow-y-auto pr-1 no-scrollbar">
        <div className="space-y-2">
              ...
              >
                ...
                >
                  ...
                  <div className="flex shrink-0 flex-wrap justify-end gap-3 text-[10px]">
                    ...
                    {group.expense > 0 && (
                      <span>-{formatMoney(group.expense)}</span>
                    )}
                    {group.closingBalance !== undefined && (
                      <span className="text-white/45">
                        Saldo: {formatMoney(group.closingBalance)}
                      </span>
                    )}
                  </div>
                </button>
                    ...
                    {group.items.map((transaction) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div
                            className={`min-w-[96px] text-right text-xs font-bold ${transaction.type === 'income' ? 'text-green-400' : ''}`}
                          >
          ...
          <div className="flex flex-col items-center gap-2 py-3 text-[10px] text-white/35">
            <span>
              {transactions.length} de {totalCount} lançamentos carregados
            </span>
            {hasMore && (
              <button
                type="button"
