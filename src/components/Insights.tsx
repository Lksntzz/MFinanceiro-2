...
function alertClass(alert: IntelligenceAlert) {
  ...
  if (alert.severity === 'positive')
    return 'border-green-500/25 bg-green-500/[0.07] text-green-200';
...
export default function Insights({
        ...
        setSupport({
          ...
          installments: (installmentsResult.data || []).map((item: any) => ({
            ...
            monthly_amount: Number(
              item.monthly_amount ?? item.valor_mensal ?? 0,
            ),
        ...
        if (active)
          setLoadError(
            error instanceof Error
              ? error.message
              : 'Não foi possível carregar os dados da inteligência financeira.',
          );
      } finally {
  ...
  if (loading && !intelligence) {
    return (
      <div className="flex flex-1 items-center justify-center text-xs text-white/35">
        Calculando cenários financeiros...
      </div>
    );
  }
  ...
  if (!intelligence) {
    return (
      <div className="glass-card m-4 !p-5 text-sm text-white/60">
        Não foi possível gerar os cenários financeiros agora.{' '}
        {loadError || 'Atualize a página e tente novamente.'}
      </div>
    );
  }

  const criticalCount = intelligence.alerts.filter(
    (alert) => alert.severity === 'critical',
  ).length;
  const warningCount = intelligence.alerts.filter(
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto pr-1 animate-fade-in [&>*]:shrink-0">
      <section className="glass-card relative overflow-hidden !p-5 border-brand-primary/20 bg-brand-primary/[0.05]">
        <BrainCircuit
          className="absolute -right-3 -top-3 text-brand-primary/10"
          size={110}
          aria-hidden="true"
        />
        <div className="relative z-10 flex flex-wrap items-start justify-between gap-4">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full border border-brand-primary/20 bg-brand-primary/10 px-2 py-1 text-[8px] font-black uppercase tracking-wider text-brand-primary">
                  projeção v2
                </span>
              </div>
              <p className="mt-1 max-w-2xl text-xs leading-relaxed text-white/50">
                Cenários determinísticos usando saldo consolidado, histórico
                real, renda, compromissos, cartões, parcelas, assinaturas e
                metas cadastradas.
              </p>
            </div>
          </div>
          <div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-right">
            ...
            <strong className="mt-1 block text-lg text-brand-primary">
              {Math.round(intelligence.confidence * 100)}%
            </strong>
            <span className="text-[9px] uppercase text-white/35">
              {intelligence.confidenceLabel} · {intelligence.historyDays} dias
            </span>
          </div>
        </div>
        ...
        <div className="relative z-10 mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <Metric
            icon={WalletCards}
            label="Folga mensal"
            value={formatCurrency(intelligence.monthlyFreeCash, isPrivate)}
            state={intelligence.monthlyFreeCash >= 0 ? 'positive' : 'danger'}
          />
          <Metric
            icon={Gauge}
            label="Gasto diário seguro"
            value={formatCurrency(intelligence.safeDailySpend, isPrivate)}
          />
          <Metric
            icon={ShieldCheck}
            label="Reserva alvo (3 meses)"
            value={formatCurrency(
              intelligence.emergencyReserveTarget,
              isPrivate,
            )}
          />
          <Metric
            icon={Target}
            label="Metas pedem / mês"
            value={formatCurrency(intelligence.goalMonthlyNeed, isPrivate)}
          />
        </div>
      </section>
      ...
      {loadError && (
        <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/[0.06] px-4 py-3 text-xs text-yellow-100">
          Alguns dados auxiliares não puderam ser atualizados: {loadError}. A
          projeção usa o que já está disponível.
        </div>
      )}

      <section className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-white/45">
            <CalendarClock size={15} /> Cenários 30 / 60 / 90 dias
          </h3>
          <span className="text-[9px] text-white/30">
            Conservador: +20% em gasto variável e -5% em renda prevista
          </span>
        </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <h4 className="mt-1 text-lg font-black">
                    {projection.horizonDays} dias
                  </h4>
                </div>
                <div
                  className={`rounded-xl px-2.5 py-1 text-[9px] font-black ${projection.base >= 0 ? 'bg-green-500/10 text-green-300' : 'bg-red-500/10 text-red-300'}`}
                >
                  {projection.base >= 0 ? 'POSITIVO' : 'RISCO'}
                </div>
              </div>
              <div className="mt-4 space-y-2 text-xs">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-white/40">Conservador</span>
                  <ScenarioValue
                    value={projection.conservative}
                    privateMode={isPrivate}
                  />
                </div>
                <div className="flex items-center justify-between gap-3 rounded-lg bg-white/[0.04] px-2 py-2">
                  <span className="font-bold text-white/65">Base</span>
                  <ScenarioValue
                ...
                <div className="flex items-center justify-between gap-3">
                  <span className="text-white/40">Otimista</span>
                  <ScenarioValue
                    value={projection.optimistic}
                    privateMode={isPrivate}
                  />
                </div>
              </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]">
        <section className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-white/45">
              <AlertTriangle size={15} /> Alertas inteligentes
            </h3>
            <span
              className={`rounded-full px-2 py-1 text-[9px] font-bold ${criticalCount ? 'bg-red-500/10 text-red-300' : warningCount ? 'bg-yellow-500/10 text-yellow-200' : 'bg-green-500/10 text-green-300'}`}
            >
              {criticalCount
          ...
          <div className="grid gap-2.5">
            {intelligence.alerts.map((alert) => (
              <article
                key={alert.id}
                className={`rounded-2xl border p-4 ${alertClass(alert)}`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {alert.action && (
                      <p className="mt-2 text-[9px] font-bold uppercase tracking-wider text-white/40">
                        Próxima ação: {alert.action}
                      </p>
                    )}
                  </div>
                </div>
        ...
        <section className="space-y-3">
          <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-white/45">
            <Lightbulb size={15} /> Leitura do fluxo
          </h3>
          <div className="glass-card !p-4 space-y-3">
            {intelligence.narrative.map((line) => (
              <p key={line} className="text-xs leading-relaxed text-white/60">
                {line}
              </p>
            ))}
          </div>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
            <MiniMetric
              label="Compromissos / renda"
              value={
                intelligence.commitmentIncomeShare == null
                  ? 'Renda não informada'
                  : `${(intelligence.commitmentIncomeShare * 100).toFixed(1)}%`
              }
              danger={(intelligence.commitmentIncomeShare || 0) >= 0.7}
            />
            <MiniMetric
              label="Assinaturas / renda"
              value={
                intelligence.subscriptionIncomeShare == null
                  ? 'Renda não informada'
                  : `${(intelligence.subscriptionIncomeShare * 100).toFixed(1)}%`
              }
              danger={(intelligence.subscriptionIncomeShare || 0) >= 0.15}
            />
            <MiniMetric
              label="Tendência de gastos"
              value={
                intelligence.expenseTrendPercent == null
                  ? 'Base insuficiente'
                  : `${intelligence.expenseTrendPercent >= 0 ? '+' : ''}${intelligence.expenseTrendPercent.toFixed(1)}%`
              }
              danger={(intelligence.expenseTrendPercent || 0) >= 15}
            />
            <MiniMetric
              label="Fôlego pelo ritmo variável"
              value={
                intelligence.cashRunwayDays == null
                  ? 'Sem base suficiente'
                  : `${intelligence.cashRunwayDays} dias`
              }
              danger={
                intelligence.cashRunwayDays != null &&
                intelligence.cashRunwayDays < 15
              }
            />
          </div>
        </section>
      ...
      <section className="glass-card !p-4 border-white/5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="flex items-center gap-2 text-sm font-bold">
              <TrendingUp size={16} /> Visão de 90 dias
            </h3>
            <p className="mt-1 text-[10px] text-white/40">
              A projeção é uma estimativa baseada nos dados cadastrados e no
              comportamento observado; não é garantia de saldo futuro.
            </p>
          </div>
        ...
        {intelligence.earliestRiskDate && (
          <div className="mt-3 flex items-center gap-2 rounded-xl border border-red-500/20 bg-red-500/[0.05] px-3 py-2 text-xs text-red-200">
            <TrendingDown size={14} /> Primeiro ponto de saldo negativo no
            cenário base:{' '}
            {new Date(
              `${intelligence.earliestRiskDate}T12:00:00`,
      ...
      <section className="space-y-2 pb-2">
        <h3 className="text-[10px] font-black uppercase tracking-[0.16em] text-white/30">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          ...
          {(summary.priorities || []).length === 0 && (
            <div className="text-xs text-white/35">
              Nenhuma prioridade pendente no ciclo atual.
            </div>
          )}
        </div>
      </section>
...
function Metric({
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-black/15 p-3">
      ...
      <strong
        className={`mt-2 block truncate text-sm ${state === 'positive' ? 'text-green-400' : state === 'danger' ? 'text-red-400' : 'text-white'}`}
      >
        {value}
      </strong>
    </div>
...
function MiniMetric({
  ...
  return (
    <div className="glass-card !p-3">
      ...
      <strong
        className={`mt-1 block text-sm ${danger ? 'text-red-300' : 'text-white/75'}`}
      >
        {value}
      </strong>
    </div>
