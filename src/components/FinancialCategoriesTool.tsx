...
export default function FinancialCategoriesTool({ user }: { user: User }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [settingsResult, accountsResult, categoriesResult] =
        await Promise.all([
          supabase
      ...
      const firstError =
        settingsResult.error || accountsResult.error || categoriesResult.error;
      ...
      setError(
        refreshError?.message || 'Não foi possível carregar suas categorias.',
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="space-y-4 animate-fade-in">
          <div>
            <h2 className="text-xl font-black">
              Como seu dinheiro é organizado
            </h2>
            <p className="text-sm text-white/40">
              Categorias descrevem a finalidade de uma entrada ou saída. Elas
              não representam contas nem saldos.
            </p>
          </div>
          ...
          {loading ? (
            <div className="mf-loading">Carregando categorias...</div>
          ) : (
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]">
              <section className="glass-card mf-tool-surface space-y-4">
                <div className="flex items-center gap-3">
                  <Tags size={18} className="text-brand-primary" />
                  <div>
                    <h3 className="text-sm font-bold">Categorias ativas</h3>
                    <p className="text-[10px] text-white/40">
                      Use poucas categorias claras; detalhes podem ficar na
                ...
                <div className="flex max-h-[430px] flex-wrap content-start gap-2 overflow-y-auto no-scrollbar">
                  {categories.map((category) => (
                    <button
                      ...
                      disabled={
                        category.is_system ||
                        busyKey === `category-${category.id}`
                      }
                      className={`rounded-lg border px-3 py-2 text-[10px] transition ${category.is_active ? 'border-white/10 bg-white/5 text-white/75' : 'border-white/5 bg-black/20 text-white/25 line-through'} ${category.is_system ? 'cursor-default' : 'hover:border-brand-primary/40'}`}
                      title={
                        category.is_system
                    ...
                    >
                      {busyKey === `category-${category.id}` ? (
                        ...
                        category.is_active && (
                          <CheckCircle2
                            size={12}
                            className="mr-1 inline text-brand-primary"
                          />
                        )
                      )}
                      {category.name} ·{' '}
                ...
                <p className="text-[10px] text-white/35">
                  {activeCategories.length} categoria(s) ativa(s). Categorias do
                  sistema ficam protegidas para manter relatórios consistentes.
                </p>
              </section>

              <form
                onSubmit={createCategory}
                className="glass-card mf-tool-surface space-y-3"
              >
                <div>
                  <h3 className="text-sm font-bold">Nova categoria</h3>
                  <p className="mt-1 text-[10px] text-white/40">
                    Crie apenas quando nenhuma categoria existente representar
                    bem o lançamento.
                  </p>
                </div>
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-brand-primary px-3 py-2 text-xs font-bold text-black disabled:opacity-50"
                >
