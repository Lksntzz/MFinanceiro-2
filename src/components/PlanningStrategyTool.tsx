...
type SubscriptionRow = { amount: number; billing_cycle?: string | null; status?: string | null };
type FixedRow = { amount: number; status?: string | null; active?: boolean | null };
...
export default function PlanningStrategyTool({ user }: { user: User }) {
  ...
  const refresh = useCallback(async () => {
    setError(null);
    try {
      const [settingsResult, accountsResult, fixedResult, subscriptionsResult, cardsResult] = await Promise.all([
        ...
        supabase.from('mf_fixed_bills').select('amount,status,active').eq('user_id', user.id),
        supabase.from('mf_subscriptions').select('amount,billing_cycle,status').eq('user_id', user.id),
        supabase.from('mf_credit_cards').select('used').eq('user_id', user.id),
      ]);

      const firstError = settingsResult.error || accountsResult.error || fixedResult.error || subscriptionsResult.error || cardsResult.error;
      ...
      setFixedBills((fixedResult.data || []).map((row: any) => ({ ...row, amount: Number(row.amount || 0) })) as FixedRow[]);
      setSubscriptions((subscriptionsResult.data || []).map((row: any) => ({ ...row, amount: Number(row.amount || 0) })) as SubscriptionRow[]);
      setCards((cardsResult.data || []).map((row: any) => ({ used: Number(row.used || 0) })) as CardRow[]);
    } catch (refreshError: any) {
      console.error('Falha ao carregar estratégia de planejamento:', refreshError);
      setError(refreshError?.message || 'Não foi possível carregar os dados de planejamento.');
    } finally {
      setLoading(false);
    }
  }, [user.id]);
  ...
  const projection = useMemo(() => {
    ...
    const fixed = fixedBills
      .filter((bill) => bill.active !== false && String(bill.status || 'pending') !== 'paid')
    ...
    const base = balance + expectedIncome - fixed - subscriptionsMonthly - cardsUsed;
    const adjusted = base + Number(extraIncome || 0) - Number(extraExpense || 0);
    return { balance, expectedIncome, fixed, subscriptionsMonthly, cardsUsed, base, adjusted };
  }, [accounts, cards, extraExpense, extraIncome, fixedBills, settings, subscriptions]);
  ...
  return (
    <div className="mf-app-shell mf-routed-app">
      ...
      <section className="mf-content">
        {loading ? <div className="mf-loading">Carregando planejamento...</div> : showGoals ? (
          <FinancialGoals />
        ) : (
          <div className="space-y-4 animate-fade-in">
            ...
            <section className="grid gap-4 xl:grid-cols-[360px_1fr]">
              <article className="mf-card space-y-4">
                ...
                <label className="block text-xs text-white/45">Gasto extra
                  <div className="mt-1 flex items-center gap-2 rounded-xl border border-white/10 bg-black/30 px-3"><TrendingDown size={15} className="text-red-400" /><input className="w-full bg-transparent py-3 text-sm text-white outline-none" type="number" step="0.01" value={extraExpense} onChange={(event) => setExtraExpense(event.target.value)} /></div>
                </label>
              </article>
