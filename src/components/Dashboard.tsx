...
export default function Dashboard({
  ...
  const [dismissedAlerts, setDismissedAlerts] = useState<string[]>(() =>
    loadDismissedAlerts(user.id, window.localStorage),
  );

  const path = location.pathname.replace(/\/+$/, '') || '/app';
  const historySubTab: 'list' | 'import' | 'batches' =
    path === '/app/movimentacoes/importar'
      ? 'import'
  ...
  const qualityIssues = useMemo(
    ...
    [
      ...
      workspace.financeTransactions,
      workspace.fixedBills,
    ],
  );
  ...
  const notifications = useMemo(() => {
    ...
    preferences.notifications.commitments,
    preferences.notifications.quality,
    qualityIssues,
    workspace.cards,
    workspace.fixedBills,
  ]);
  ...
  const recent = useMemo(
    () => sortTransactionsByDateDesc(workspace.transactions),
  ...
  async function deleteWithUndo(id: string) {
    const snapshot = workspace.transactions.find(
      (transaction) => transaction.id === id,
    ) as any;
    const { error } = await supabase.rpc('mf_delete_finance_entry', {
      p_entry_id: id,
    });
    if (error) {
    ...
    offerUndo('Lançamento excluído.', async () => {
      const { error: restoreError } = await supabase.rpc(
        'mf_create_finance_entry_v3',
        {
          ...
          p_date: String(snapshot.date || new Date().toISOString()).slice(
            0,
            10,
          ),
          p_description:
      ...
      if (restoreError) {
        workspace.setError(restoreError.message);
        return;
      }
      await workspace.refresh();
      void recordUserActivity({
        ...
        entity_type: 'movimentacao',
        summary: 'Lançamento restaurado após exclusão',
      });
    });
  }

  async function importWithAudit(
    imported: ImportedTransaction[],
    newBalance: number | undefined,
    options: StatementImportOptions,
  ) {
    const result = await workspace.importTransactions(
      imported,
      newBalance,
      options,
    );
    void recordUserActivity({
      ...
      summary: `Importação concluída com ${result.inserted_count} lançamento${result.inserted_count === 1 ? '' : 's'} novo${result.inserted_count === 1 ? '' : 's'}`,
      metadata: {
  ...
  return (
    <div className="mf-app-shell mf-routed-app">
      ...
      <header className="mf-topbar">
        <div className="mf-brand">
          ...
          <div>
            ...
            <span>
              {workspace.settings?.display_name
                ? `Olá, ${workspace.settings.display_name.split(/\s+/)[0]}`
                : 'Dashboard'}
            </span>
          </div>
        </div>
        <div className="mf-top-actions">
          ...
          >
            ...
            {notifications.length > 0 && (
              <span className="absolute -right-1.5 -top-1.5 grid h-4 min-w-4 place-items-center rounded-full bg-red-500 px-1 text-[8px] font-black text-white">
                {notifications.length > 99 ? '99+' : notifications.length}
              </span>
            )}
          </button>
          <button className="primary" onClick={() => navigate('/app/lancar')}>
            <Plus size={16} />
            Lançar
          </button>
          <button
      ...
      <section
        className={`mf-content ${path.startsWith('/app/movimentacoes') ? 'history-active' : ''}`}
      >
        <Suspense
          fallback={<div className="mf-loading">Carregando módulo...</div>}
        >
          <Routes>
            <Route
              path="/app"
              element={
                <>
                  <OnboardingChecklist
                    ...
                    hasCommitment={
                      workspace.fixedBills.length > 0 ||
                      workspace.cards.length > 0
                    }
                    hasAccount={workspace.accounts.some(
                  ...
                  <DashboardHome
                    ...
                    isPrivate={isPrivate}
                    visibleWidgets={preferences.homeWidgets}
                    qualityIssues={qualityIssues}
                  />
                  {(workspace.loading || workspace.analyticsLoading) && (
                  ...
                  {workspace.analyticsIncomplete && (
                    <div className="mf-error" role="status">
                      <AlertCircle size={16} />
                      As análises estão usando apenas os lançamentos recentes
                      porque o histórico completo não pôde ser consolidado.
                    </div>
                  )}
                </>
              }
              ...
              element={
                <div className="mf-tab-shell history-shell">
                  <div className="mf-subnav">
                    <button
                      className={historySubTab === 'list' ? 'active' : ''}
                      onClick={() => navigate('/app/movimentacoes')}
                    >
                      Movimentações
                    </button>
                    <button
                      className={historySubTab === 'import' ? 'active' : ''}
                      onClick={() => navigate('/app/movimentacoes/importar')}
                    >
                      Importar extrato
                    </button>
                    <button
                      className={historySubTab === 'batches' ? 'active' : ''}
                      onClick={() => navigate('/app/movimentacoes/lotes')}
                    >
                  ...
                  {historySubTab === 'list' ? (
                    <>
                      {/* biome-ignore format: contract check */}
                      <History userId={user.id}
                        ...
                        balanceConfirmed={
                          workspace.settings?.balance_confirmed === true
                        }
            ...
            />
            <Route
              path="/app/analises/insights"
              element={
                <div className="mf-tab-shell">
                  {workspace.analyticsLoading && (
                    <div className="mf-loading">
                      Consolidando histórico completo...
                    </div>
                  )}
                  {workspace.analyticsIncomplete && (
                    <div className="mf-error" role="status">
                      <AlertCircle size={16} />O histórico completo ainda não
                      está disponível; os Insights podem estar temporariamente
                      limitados.
                    </div>
                  )}
                  <Insights
      ...
      <NotificationCenter
        ...
        onPay={(item: any) =>
          item.type === 'fixed'
            ? workspace.payFixedBill(item.originalData)
            : undefined
        }
        onNavigate={(nextPath) => {
