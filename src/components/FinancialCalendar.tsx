...
export default function FinancialCalendar({
  ...
  useEffect(() => {
    ...
    async function loadOccurrences() {
      ...
      if (ensureError) {
        console.warn(
          'Não foi possível preparar as ocorrências do calendário:',
          ensureError,
        );
      ...
      if (error) {
        console.warn(
          'Não foi possível carregar as ocorrências do calendário:',
          error,
        );
        setOccurrencesLoaded(false);
        return;
      ...
      setOccurrences(
        (data || []).map((row: any) => ({
          ...
          fixed_bill: Array.isArray(row.fixed_bill)
            ? row.fixed_bill[0]
            : row.fixed_bill,
        })) as BillOccurrence[],
      );
  ...
  const annualSubscriptionsWithoutMonth = subscriptions.filter(
    (item) =>
  ...
  const events = useMemo<CalendarEvent[]>(() => {
    const billEvents: CalendarEvent[] = occurrencesLoaded
      ? occurrences
          ...
          .map((row) => {
            const dueDate = new Date(
              `${String(row.due_date).slice(0, 10)}T12:00:00`,
            );
            return {
      ...
      : fixedBills
          ...
          .map((bill) => ({
            id: `bill-${bill.id}`,
            day: safeRecurringDay(
              bill.default_due_day ?? bill.due_day,
              currentDate,
            ),
    ...
    installments
      ...
      .forEach((item) => {
        const includedInCardBill =
          Boolean(item.card_id) && cardIds.has(String(item.card_id));
        list.push({
    ...
    const secondPayday =
      settings?.payday_cycle === 'biweekly'
        ? Number(settings?.payday_2 || 0)
        : 0;
    const firstPercentage = Number(
      settings?.payday_1_percentage ?? (secondPayday ? 50 : 100),
    );
    const secondPercentage = Number(
      settings?.payday_2_percentage ?? (secondPayday ? 50 : 0),
    );
  ...
  const summary = useMemo(() => {
    const pendingExpenses = events.filter(
      (event) =>
        event.type === 'expense' &&
        event.status !== 'paid' &&
        event.affectsSummary !== false,
    );
    ...
    const nextIncome =
      incomes.find((event) => event.day >= referenceDay) || incomes[0] || null;
    const nextExpense =
      pendingExpenses.find((event) => event.day >= referenceDay) ||
      pendingExpenses[0] ||
      null;
    const incomeTotal = incomes.reduce((sum, event) => sum + event.amount, 0);
    const expenseTotal = pendingExpenses.reduce(
    ...
    <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden animate-fade-in">
      <div className="flex shrink-0 items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-bold uppercase tracking-widest text-white/35">
              Agenda de entradas e compromissos
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="rounded-lg bg-white/5 px-3 py-2 text-[10px] font-bold text-white/50"
          >
            Hoje
          </button>
          <button
      ...
      <section className="flex-1 min-h-0 grid grid-cols-7 grid-rows-[auto_repeat(6,minmax(0,1fr))] gap-1.5 overflow-hidden">
        {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map((day) => (
          <div
            key={day}
            className="py-1 text-center text-[9px] font-bold uppercase text-white/25"
          >
            {day}
          </div>
        ))}
        ...
        {days.map((date) => {
          ...
          return (
            <article
              ...
              className={`min-h-0 rounded-xl border p-1.5 ${isToday(date) ? 'border-brand-primary/30 bg-brand-primary/5' : 'border-white/5 bg-white/[0.03]'}`}
            >
              <div
                className={`text-[10px] font-bold ${isToday(date) ? 'text-brand-primary' : 'text-white/35'}`}
              >
                {date.getDate()}
              </div>
              <div className="mt-1 space-y-1">
                {visible.map((event) => (
                  <div
                    key={event.id}
                    title={`${event.name}: ${money(event.amount)}${event.informational ? ' · já incluído na fatura' : ''}`}
                    className={`truncate rounded-md px-1.5 py-1 text-[8px] font-bold ${event.type === 'income' ? 'bg-green-500/10 text-green-400' : event.informational ? 'bg-violet-500/10 text-violet-300' : event.status === 'paid' ? 'bg-white/5 text-white/30 line-through' : 'bg-red-500/10 text-red-400'}`}
                  >
                    {event.type === 'income'
                      ? '+'
                      : event.informational
      ...
      <section className="grid shrink-0 grid-cols-1 md:grid-cols-3 gap-3">
        <Summary
          ...
          value={
            summary.nextIncome
              ? `Dia ${summary.nextIncome.day} • ${money(summary.nextIncome.amount)}`
        ...
        <Summary
          ...
          label="Próxima saída pendente"
          value={
            summary.nextExpense
              ? `Dia ${summary.nextExpense.day} • ${money(summary.nextExpense.amount)}`
        ...
        <Summary
          icon={DollarSign}
          label="Fluxo previsto do mês"
          value={money(summary.balance)}
          tone={summary.balance >= 0 ? 'brand' : 'negative'}
        />
      </section>

      {(annualSubscriptionsWithoutMonth > 0 ||
        installments.some((item) => item.card_id)) && (
        <p className="shrink-0 text-[9px] leading-relaxed text-white/30">
          Parcelas vinculadas a cartões aparecem como referência, mas não são
          somadas novamente à fatura.{' '}
          {annualSubscriptionsWithoutMonth > 0
            ? `${annualSubscriptionsWithoutMonth} assinatura(s) anual(is) não entram no calendário porque o mês da renovação ainda não é armazenado.`
            : ''}
        </p>
      )}
    </div>
...
function Summary({
  ...
  return (
    <div className="glass-card !p-3 flex min-w-0 items-center gap-3">
      <div
      ...
      <div className="min-w-0">
        <span className="block truncate text-[9px] font-bold uppercase tracking-widest text-white/35">
          {label}
        </span>
