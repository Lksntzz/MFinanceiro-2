...
export default function FinancialAgendaTool({ user }: { user: User }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setInstallments((installmentsResult.data || []).map((item: any) => ({
        ...item,
          ...
          <div className="mf-tab-shell">
            {section === 'calendar' && (
              <div className="space-y-4 animate-fade-in">
                <div><h2 className="text-xl font-black">Agenda Financeira</h2><p className="text-sm text-white/40">Tudo que tem data e pode mexer com seu dinheiro: receitas, recorrências, faturas e parcelas.</p></div>
                <FinancialCalendar fixedBills={fixedBills} settings={settings} subscriptions={subscriptions} cards={cards} installments={installments} />
              </div>
            )}
            ...
            {section === 'recurrences' && (
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-white/40">Compromissos que voltam com frequência. Contas fixas e assinaturas ficam juntas, sem perder suas diferenças.</p>
                </div>
                <div className="mf-subnav">
                  <button className={recurrenceKind === 'fixed' ? 'active' : ''} onClick={() => navigate('/app/agenda/recorrencias?tipo=fixas')}>Contas fixas</button>
                  <button className={recurrenceKind === 'subscriptions' ? 'active' : ''} onClick={() => navigate('/app/agenda/recorrencias?tipo=assinaturas')}>Assinaturas</button>
                </div>
