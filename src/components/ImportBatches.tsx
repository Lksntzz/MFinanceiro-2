...
import { AlertCircle, CheckCircle2, ChevronRight, FileClock, Loader2, RefreshCw, RotateCcw } from 'lucide-react';
...
export default function ImportBatches({ userId, accounts }: ImportBatchesProps) {
  ...
  async function loadBatches() {
    ...
    const { data, error: queryError } = await supabase
      ...
      .select('id,user_id,account_id,status,source_format,file_name,parser_name,period_start,period_end,balance_mode,balance_before,balance_after,net_amount,requested_count,inserted_count,duplicate_count,rejected_count,ignored_count,error_message,created_at,completed_at,reverted_at,revert_reason')
      .eq('user_id', userId)
  ...
  async function revertBatch(batch: StatementImportBatch) {
    ...
    setError(null);
    const { error: rpcError } = await supabase.rpc('mf_revert_statement_import', {
      p_batch_id: batch.id,
        ...
        <div className="max-h-[560px] space-y-1 overflow-y-auto p-2 no-scrollbar">
          {batches.map((batch) => (
            <button key={batch.id} type="button" onClick={() => void selectBatch(batch.id)} className={`w-full rounded-xl border p-3 text-left transition ${selectedBatchId === batch.id ? 'border-brand-primary/40 bg-brand-primary/10' : 'border-transparent hover:border-white/10 hover:bg-white/[0.03]'}`}>
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-[9px]"><span className={batch.status === 'reverted' ? 'text-white/30 line-through' : 'text-green-400'}>{batch.inserted_count} importados</span><span className="text-yellow-300">{batch.duplicate_count} duplicados</span>{batch.rejected_count > 0 && <span className="text-red-300">{batch.rejected_count} rejeitados</span>}{batch.status === 'reverted' && <span className="text-blue-300">lote desfeito</span>}</div>
            </button>
          ))}
        ...
        {!selectedBatch ? (
          ...
          <>
            <div className="border-b border-white/10 p-4">
              <div className="flex items-start justify-between gap-4"><div><h3 className="text-sm font-bold">{selectedBatch.file_name || 'Importação sem arquivo'}</h3><p className="mt-1 text-[10px] text-white/40">{selectedBatch.period_start || 'Período não identificado'} até {selectedBatch.period_end || '—'} · saldo {money(selectedBatch.balance_before)} → {money(selectedBatch.balance_after)}</p>{selectedBatch.status === 'reverted' && <p className="mt-1 text-[9px] text-blue-300">Desfeito em {selectedBatch.reverted_at ? new Date(selectedBatch.reverted_at).toLocaleString('pt-BR') : 'data não informada'}{selectedBatch.revert_reason ? ` · ${selectedBatch.revert_reason}` : ''}</p>}</div><div className="flex items-center gap-2">{selectedBatch.status === 'completed' && <button type="button" disabled={Boolean(revertingBatchId)} onClick={() => void revertBatch(selectedBatch)} className="flex items-center gap-1 rounded-lg border border-blue-400/20 bg-blue-500/10 px-3 py-2 text-[9px] font-bold text-blue-200 disabled:opacity-40">{revertingBatchId === selectedBatch.id ? <Loader2 size={12} className="animate-spin" /> : <RotateCcw size={12} />} Desfazer lote</button>}{selectedBatch.status === 'completed' ? <CheckCircle2 size={18} className="text-green-400" /> : <AlertCircle size={18} className={selectedBatch.status === 'reverted' ? 'text-blue-300' : 'text-yellow-300'} />}</div></div>
