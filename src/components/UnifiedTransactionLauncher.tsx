...
export default function UnifiedTransactionLauncher({
  ...
  const [normalizedCategories, setNormalizedCategories] = useState<
    TransactionCategory[]
  >([]);
  ...
  async function loadSupportingData() {
    ...
    const [cardResult, pendingResult, accountResult, categoryResult] =
      await Promise.all([
        supabase
          .from('mf_credit_cards')
          .select('id,name,brand,used,limit')
          .eq('user_id', userId)
          .order('name'),
        supabase
          .from('mf_finance_ledger_entries')
          .select(
            'id,description,category,amount,type,date,due_date,payment_method',
          )
          .eq('user_id', userId)
          .eq('status', 'pending')
    ...
    setPending(
      (pendingResult.data || []).map((item: any) => ({
        ...item,
    ...
    setForm((current) => {
      const account =
        nextAccounts.find((item) => item.id === current.accountId) ||
        nextAccounts.find((item) => item.is_default) ||
        nextAccounts[0];
      const compatibleCategories = nextCategories.filter(
        (item) =>
          item.category_type === 'both' || item.category_type === current.type,
      );
      const category =
        compatibleCategories.find((item) => item.id === current.categoryId) ||
  ...
  const categoryOptions = normalizedCategories.filter(
    (category) =>
  ...
  const balanceEffect = affectsBalance
    ? form.type === 'expense'
      ? -Math.abs(amount)
      : Math.abs(amount)
    : 0;
  ...
  const preview = useMemo(() => {
    if (!Number.isFinite(amount) || amount <= 0)
      return 'Informe o valor para visualizar o efeito.';
    if (form.status === 'pending')
      return `Será salvo como pendente, sem alterar o saldo agora.`;
  ...
  function changeType(type: EntryType) {
    const nextCategory = normalizedCategories.find(
      (category) =>
        category.is_active &&
    ...
    setForm((current) => ({
      ...
      category:
        nextCategory?.name ||
        (type === 'income' ? INCOME_CATEGORIES[0] : CATEGORIES[0] || 'Geral'),
      paymentMethod:
        type === 'income'
          ? 'pix'
          : current.paymentMethod === 'benefit'
            ? 'pix'
  ...
  function selectMethod(method: PaymentMethod) {
    setForm((current) => ({
      ...
      cardId:
        method === 'credit_card' ? current.cardId || cards[0]?.id || '' : '',
      installmentCount:
        method === 'credit_card' ? current.installmentCount : '1',
    }));
    setAdvanced(true);
  }
  ...
  async function refreshDashboard() {
    try {
      ...
      console.warn(
        'Lançamento confirmado, mas o painel não pôde ser atualizado imediatamente:',
        refreshError,
      );
    }
  }
  ...
  async function save(keepOpen: boolean) {
    ...
    try {
      ...
      if (!form.accountId)
        throw new Error('Selecione a conta financeira do lançamento.');
      ...
      const installmentCount = Math.max(
        1,
        Math.min(48, Number(form.installmentCount || 1)),
      );
      ...
      const { error: rpcError } = await supabase.rpc(
        'mf_create_finance_entry_v3',
        {
          ...
          p_card_id:
            form.paymentMethod === 'credit_card' && form.type === 'expense'
              ? form.cardId
              : null,
          p_installment_count: installmentCount,
          p_due_date:
            form.status === 'pending' && form.dueDate ? form.dueDate : null,
      ...
      setMessage(
        form.status === 'pending'
          ? 'Lançamento pendente registrado.'
          : 'Lançamento registrado e valores atualizados.',
      );
      ...
      setError(
        saveError?.message || 'Não foi possível registrar o lançamento.',
      );
    } finally {
      setSaving(false);
    }
  ...
  async function markPaid(item: PendingRow) {
    ...
    try {
      ...
      setMessage(
        item.type === 'income'
          ? 'Entrada recebida e saldo atualizado.'
          : 'Saída paga e saldo atualizado.',
      );
      await loadSupportingData();
      await refreshDashboard();
    } catch (payError: any) {
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="mf-transaction-launcher-title"
        className="flex max-h-[94vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl border border-white/10 bg-[#090909] shadow-2xl"
      >
        <header className="flex shrink-0 items-center justify-between border-b border-white/10 px-4 py-3 md:px-5">
          <div>
            <h2
              id="mf-transaction-launcher-title"
              className="flex items-center gap-2 text-base font-black"
            >
              <ListPlus size={19} className="text-brand-primary" /> Central de
              lançamentos
            </h2>
            <p className="mt-1 text-[9px] uppercase tracking-[0.17em] text-white/35">
              Entrada, saída, forma de pagamento e pendências no mesmo lugar
            </p>
          </div>
          <button
            type="button"
            aria-label="Fechar central de lançamentos"
            disabled={saving}
            onClick={() => onOpenChange(false)}
            className="rounded-xl bg-white/5 p-2 text-white/45 hover:text-white disabled:opacity-40"
          >
            <X size={18} />
          </button>
        </header>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 md:p-5">
          {(error || message) && (
            <div
              className={`mb-4 flex items-center justify-between rounded-xl border px-3 py-2 text-xs ${error ? 'border-red-500/30 bg-red-500/10 text-red-300' : 'border-green-500/30 bg-green-500/10 text-green-300'}`}
            >
          ...
          <div className="grid grid-cols-2 gap-2 rounded-2xl bg-white/[0.03] p-1.5">
            <button
              ...
              className={`rounded-xl py-3 text-sm font-black transition ${form.type === 'expense' ? 'bg-red-500/15 text-red-300' : 'text-white/35'}`}
            >
              Saída
            </button>
            <button
              ...
              className={`rounded-xl py-3 text-sm font-black transition ${form.type === 'income' ? 'bg-green-500/15 text-green-300' : 'text-white/35'}`}
            >
              Entrada
            </button>
          </div>
          ...
          <section className="mt-5">
            <div className="mb-2 text-[9px] font-bold uppercase tracking-wider text-white/35">
              {form.type === 'expense'
                ? 'Como foi pago?'
                : 'Como foi recebido?'}
            </div>
              ...
              {PAYMENT_OPTIONS[form.type].map((option) => {
                ...
                return (
                  <button
                    ...
                    className={`flex min-h-16 flex-col items-center justify-center gap-1.5 rounded-xl border px-2 text-[10px] font-bold transition ${form.paymentMethod === option.id ? 'border-brand-primary/45 bg-brand-primary/10 text-brand-primary' : 'border-white/10 bg-white/[0.025] text-white/45 hover:text-white'}`}
                  >
                    <Icon size={16} />
                    {option.label}
                  </button>
          ...
          <button
            ...
            className="mt-4 flex w-full items-center justify-between rounded-xl border border-white/10 bg-white/[0.025] px-3 py-2.5 text-xs font-bold text-white/55"
          >
          ...
          {advanced && (
            <section className="mt-3 grid gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4 md:grid-cols-2">
              <Field label="Situação">
                ...
                >
                  <option value="paid">
                    {form.type === 'expense' ? 'Pago agora' : 'Recebido agora'}
                  </option>
              ...
              <Field label="Observações" wide>
                <textarea
                  ...
                  placeholder="Informações opcionais sobre o lançamento"
                />
              </Field>
            </section>
          )}
          ...
          {form.type === 'expense' &&
            ...
            cards.length === 0 && (
              <div className="mt-3 rounded-xl border border-amber-500/25 bg-amber-500/10 p-3 text-xs text-amber-200">
                Cadastre um cartão em Compromissos → Cartões antes de lançar uma
                compra no crédito.
              </div>
            )}
            ...
            >
              <span>
                <strong className="flex items-center gap-2 text-sm">
                  <Clock3 size={15} /> Pendências
                </strong>
                <small className="mt-1 block text-[9px] uppercase text-white/30">
                  {pending.length} lançamento(s) aguardando conclusão
                </small>
              </span>
              <ChevronDown
                size={15}
                className={`transition ${pendingOpen ? 'rotate-180' : ''}`}
              />
            </button>
            {pendingOpen && (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            className="rounded-lg bg-brand-primary/15 px-3 py-2 text-[10px] font-bold text-brand-primary disabled:opacity-50"
                          >
                            {payingId === item.id
                              ? 'Atualizando...'
                              : item.type === 'income'
        ...
        <footer className="grid shrink-0 gap-2 border-t border-white/10 p-4 sm:grid-cols-[auto_1fr_1fr]">
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-xs font-bold text-white/45"
          >
          ...
          <button
            ...
            disabled={saving}
            onClick={() => save(true)}
            className="flex items-center justify-center gap-2 rounded-xl border border-brand-primary/30 bg-brand-primary/10 px-4 py-3 text-xs font-black text-brand-primary disabled:opacity-50"
          >
            <Plus size={14} /> Salvar e lançar outro
          </button>
          <button
            type="button"
            disabled={saving}
            onClick={() => save(false)}
            className="flex items-center justify-center gap-2 rounded-xl bg-brand-primary px-4 py-3 text-xs font-black text-black disabled:opacity-50"
          >
...
function Field({
  ...
  return (
    <label className={wide ? 'block md:col-span-2' : 'block'}>
      <span className="mb-1.5 block text-[9px] font-bold uppercase tracking-wider text-white/35">
        {label}
      </span>
      {React.cloneElement(children, {
        className:
          'w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2.5 text-sm text-white outline-none transition focus:border-brand-primary/55',
      })}
    </label>
  );
}
