...
export default function IntegrationTool({ user }: { user: User }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(
        refreshError?.message || 'Não foi possível carregar suas conexões.',
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="mf-app-shell mf-routed-app">
      ...
      <header className="mf-topbar">
        ...
        <div className="mf-top-actions">
          ...
          <button
            type="button"
            className="primary"
            onClick={() => window.location.assign('/app/lancar')}
          >
            <Plus size={16} />
            Lançar
          </button>
      ...
      <section className="mf-content">
        {loading ? (
          <div className="mf-loading">Carregando conexões...</div>
        ) : (
          <div className="space-y-4 animate-fade-in">
            <div>
              ...
              <p className="text-sm text-white/40">
                Use regras para reduzir trabalho repetitivo. As conexões
                bancárias serão liberadas quando a experiência estiver pronta
                para uso contínuo.
              </p>
            </div>
            <div className="mf-open-finance-coming-soon" role="status">
              <div className="flex min-w-0 gap-3">
                ...
                />
                <div>
                  <strong>Conexão bancária via Open Finance</strong>
                  <p>
                    Em breve você poderá conectar bancos e trazer contas e
                    movimentações para o MF com sincronização. Até lá, o acesso
                    fica bloqueado para evitar fluxos incompletos.
                  </p>
                </div>
              </div>
              <span>Em breve</span>
            </div>
            <details className="mf-automation-disclosure">
              <summary>
                <span>
                  <strong>Regras e automações</strong>
                  <small>
                    Use somente quando quiser reduzir trabalho repetitivo dentro
                    das suas conexões.
                  </small>
                </span>
                <ChevronDown size={16} />
              </summary>
              <div className="mf-automation-content">
                <AutomationCenter
                  userId={user.id}
                  accounts={accounts}
