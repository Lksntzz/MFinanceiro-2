...
export default function CommandPalette({ userId }: { userId: string }) {
  ...
  return createPortal(
    <div className="mf-command-backdrop" role="dialog" aria-modal="true" aria-label="Busca rápida do MF" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="mf-command-palette">
        <div className="mf-command-search"><Search size={18} /><input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar ferramenta ou ação…" aria-label="Buscar no MF" /><kbd>Esc</kbd></div>
        <div className="mf-command-results" role="listbox">
          {commands.map((item, index) => {
            const Icon = item.icon;
            return <button key={item.id} type="button" onClick={() => choose(item)} autoFocus={index === 0 && query.length > 0} role="option">
        ...
        <footer><span><kbd>Ctrl</kbd>/<kbd>⌘</kbd> + <kbd>K</kbd> abre esta busca</span><span>Enter ou clique para abrir</span></footer>
      </section>
    </div>,
    document.body,
  );
