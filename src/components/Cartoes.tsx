...
import type { CardInstallment, CreditCard } from '../types';
...
export default function Cartoes(props: CartoesProps) {
  ...
  const pressureCard = [...normalizedCards].sort(
    (a, b) => percent(b.used, b.limit) - percent(a.used, a.limit),
  )[0];
  ...
  const cardPages = Math.max(
    1,
    Math.ceil(normalizedCards.length / cardsPerPage),
  );
  const installmentPages = Math.max(
    1,
    Math.ceil(normalizedInstallments.length / installmentsPerPage),
  );
  const visibleCards = normalizedCards.slice(
    (cardPage - 1) * cardsPerPage,
    cardPage * cardsPerPage,
  );
  const visibleInstallments = normalizedInstallments.slice(
    (installmentPage - 1) * installmentsPerPage,
    installmentPage * installmentsPerPage,
  );
  ...
  useEffect(() => {
    if (installmentPage > installmentPages)
      setInstallmentPage(installmentPages);
  }, [installmentPage, installmentPages]);
    ...
    <div className="flex-1 min-h-0 flex flex-col gap-4 overflow-hidden animate-fade-in">
      ...
      <section className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-12 gap-4 overflow-hidden">
        <div className="xl:col-span-8 glass-card !p-4 flex flex-col min-h-0 overflow-hidden">
          <Header
            title="Meus cartões"
            subtitle="Valores cadastrados manualmente"
            action={onAddCard}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 content-start">
              {visibleCards.map((card) => {
                ...
                return (
                  <article
                    key={card.id}
                    className={`rounded-2xl border p-4 ${usage >= 80 ? 'border-red-500/30 bg-red-500/5' : 'border-white/10 bg-white/[0.03]'}`}
                  >
                    ...
                    <div className="mt-4 flex justify-between text-xs">
                      <span>
                        Usado <strong>{money(card.used)}</strong>
                      </span>
                      <span>
                        Disponível{' '}
                        <strong
                    ...
                    <div className="mt-3 flex justify-between border-t border-white/5 pt-3 text-[9px] text-white/40">
                      <span className="flex gap-1">
                        <Calendar size={11} /> Fecha {card.closing_day}
                      </span>
                      <span>Vence {card.due_day}</span>
                    </div>
                    {onPayCardBill && card.used > 0 && (
                      <button
                        onClick={() => onPayCardBill(card)}
                        className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-brand-primary/15 py-2 text-[10px] font-bold text-brand-primary"
                      >
                        <CheckCircle2 size={13} /> Registrar pagamento da fatura
                      </button>
                    )}
                  </article>
                );
          ...
          {cardPages > 1 && (
            <Pager page={cardPage} pages={cardPages} onChange={setCardPage} />
          )}
        </div>

        <aside className="xl:col-span-4 glass-card !p-4 flex flex-col gap-4">
          <h2 className="flex items-center gap-2 text-sm font-bold">
            <AlertTriangle size={16} /> Saúde do crédito
          </h2>
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <div
              className={`mt-1 text-3xl font-black ${globalUsage >= 80 ? 'text-red-400' : 'text-brand-secondary'}`}
            >
              {globalUsage.toFixed(0)}%
            </div>
            <p className="mt-2 text-xs text-white/40">
              {totalLimit <= 0
                ? 'Cadastre limites para acompanhar o uso.'
                : globalUsage >= 80
                  ? 'Uso elevado. Evite novas compras.'
                  : globalUsage >= 50
                    ? 'Uso moderado. Acompanhe as faturas.'
                    : 'Uso em faixa controlada.'}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <div className="text-sm font-bold">Maior pressão</div>
            {pressureCard ? (
              <p className="mt-2 text-xs text-white/50">
                <strong>{pressureCard.name}</strong>:{' '}
                {percent(pressureCard.used, pressureCard.limit).toFixed(0)}% do
                limite.
              </p>
            ) : (
          ...
          <div className="mt-auto rounded-2xl border border-brand-primary/20 bg-brand-primary/5 p-4">
            <div className="text-[9px] uppercase text-white/30">
              Restante consolidado
            </div>
            <div
              className={`mt-1 text-xl font-black ${available >= 0 ? 'text-brand-primary' : 'text-red-400'}`}
            >
              {money(available)}
            </div>
          </div>
      ...
      <section className="glass-card !p-4 shrink-0">
        <Header
          ...
          action={onAddInstallment}
          actionLabel="Novo parcelamento"
        />
        {visibleInstallments.length === 0 ? (
          <div className="flex h-12 items-center justify-center rounded-xl border border-dashed border-white/10 text-xs text-white/30">
              ...
              return (
                ...
                >
                  <div className="flex justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-[9px] text-white/30">
                        ...
                        /{item.total_installments} • dia {item.due_day}
                      </p>
                    </div>
                    <div className="flex gap-1">
                      {onEditInstallment && (
                      ...
                      {onDeleteInstallment && (
                        <button
                          onClick={() => onDeleteInstallment(item)}
                  ...
                  <div className="mt-2 text-sm font-bold">
                    {money(item.monthly_amount)}
                  </div>
                  {onPayInstallment && !completed && (
                    <button
                      onClick={() => onPayInstallment(item)}
                      className="mt-2 w-full rounded-lg bg-green-500/10 py-1.5 text-[9px] font-bold text-green-400"
                    >
        ...
        {installmentPages > 1 && (
          <Pager
            ...
            pages={installmentPages}
            onChange={setInstallmentPage}
            compact
          />
        )}
      </section>
...
function Metric({
  ...
  return (
    <div className="glass-card !p-3 min-w-0">
      <div className="truncate text-[9px] font-bold uppercase text-white/35">
        {label}
      </div>
      <div
  ...
  return (
    <div className="mb-3 flex items-center justify-between gap-3">
      <div>
        ...
        <p className="text-[9px] uppercase text-white/30">{subtitle}</p>
      </div>
      {action && (
        <button
          onClick={action}
...
function Empty({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-white/10 text-white/30">
      {icon}
...
function Pager({
  ...
  return (
    <div
      className={`${compact ? 'mt-2' : 'mt-auto pt-3'} flex items-center justify-center gap-3 text-[10px]`}
    >
      <button
