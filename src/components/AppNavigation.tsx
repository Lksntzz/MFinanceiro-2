...
import {
  Beaker, Bot, CalendarDays, ChevronDown, CircleDollarSign, CreditCard, FileText, Landmark,
  LayoutDashboard, Lightbulb, ListChecks, PieChart, Plus, Receipt, Search, Sparkles,
  Tags, Target, TrendingUp, Upload, Wallet,
} from 'lucide-react';
import { NavLink, useLocation, useNavigate } from 'react-router';

type NavigationItem = { to: string; label: string; icon: React.ComponentType<{ size?: number }>; end?: boolean };
...
const investmentItems: Array<{ section: InvestmentSection; label: string; icon: React.ComponentType<{ size?: number }> }> = [
...
function NavigationLink({ item, compact = false }: { item: NavigationItem; compact?: boolean }) {
  ...
  return <NavLink to={item.to} end={item.end} className={({ isActive }) => `mf-side-item ${compact ? 'compact' : ''} ${isActive ? 'active' : ''}`} title={item.label} aria-label={item.label}><Icon size={compact ? 14 : 16} aria-hidden="true" /><span>{item.label}</span></NavLink>;
}

function InvestmentSectionLink({ section, label, icon: Icon, activeSection }: { section: InvestmentSection; label: string; icon: React.ComponentType<{ size?: number }>; activeSection: InvestmentSection }) {
  const search = section === 'portfolio' ? '' : `?section=${section}`;
  const active = activeSection === section;
  return <NavLink to={{ pathname: '/app/investimentos', search }} className={`mf-side-item compact ${active ? 'active' : ''}`} title={label} aria-label={label} aria-current={active ? 'page' : undefined}><Icon size={14} aria-hidden="true" /><span>{label}</span></NavLink>;
}

function ToolHeader({ to, label, icon: Icon, active, expanded, onToggle }: { to: string; label: string; icon: React.ComponentType<{ size?: number }>; active: boolean; expanded: boolean; onToggle: () => void }) {
  return <div className="mf-side-tool-row"><NavLink to={to} className={`mf-side-item ${active ? 'active' : ''}`} title={label} aria-label={label} aria-current={active ? 'page' : undefined}><Icon size={16} aria-hidden="true" /><span>{label}</span></NavLink><button type="button" className={`mf-side-chevron ${expanded ? 'expanded' : ''}`} onClick={onToggle} aria-label={`${expanded ? 'Recolher' : 'Expandir'} ${label}`} aria-expanded={expanded}><ChevronDown size={14} aria-hidden="true" /></button></div>;
}
...
function betaInvestEnabled(pathname: string) {
  ...
  try { return window.localStorage.getItem('mf-beta:investments-v2:enabled') === '1'; } catch { return false; }
}
...
export default function AppNavigation({ onLaunch }: { onLaunch: () => void }) {
  ...
  function toggleGroup(group: ToolGroup) { setExpandedGroup((current) => current === group ? null : group); }
  function launch() { if (isHomeRoute) onLaunch(); else navigate('/app/lancar'); }

  return <aside className="mf-side-panel" aria-label="Navegação financeira">
    <style>{`
    ...
    `}</style>
    <div className="mf-side-brand" aria-label="MF Financeiro"><div className="mf-side-brand-mark" aria-hidden="true">M</div><div className="mf-side-brand-copy"><strong>MF Financeiro</strong><small>Controle financeiro inteligente</small></div></div>
    <nav className="mf-side-primary" aria-label="Seções principais">
      ...
      <section className="mf-side-tool" aria-label="Movimentações"><ToolHeader to="/app/movimentacoes" label="Movimentações" icon={Receipt} active={activeGroup === 'movements'} expanded={expandedGroup === 'movements'} onToggle={() => toggleGroup('movements')} />{expandedGroup === 'movements' && <div className="mf-side-primary-children">{movementItems.map((item) => <NavigationLink key={item.to} item={item} compact />)}</div>}</section>
      {!betaEnabled && <section className="mf-side-tool" aria-label="Investimentos"><ToolHeader to="/app/investimentos" label="Investimentos" icon={Landmark} active={activeGroup === 'investments'} expanded={expandedGroup === 'investments'} onToggle={() => toggleGroup('investments')} />{expandedGroup === 'investments' && <div className="mf-side-primary-children">{investmentItems.map((item) => <InvestmentSectionLink key={item.section} section={item.section} label={item.label} icon={item.icon} activeSection={investmentSection} />)}</div>}</section>}
      <section className="mf-side-tool" aria-label="Planejamento"><ToolHeader to="/app/planejamento" label="Planejamento" icon={Wallet} active={activeGroup === 'planning'} expanded={expandedGroup === 'planning'} onToggle={() => toggleGroup('planning')} />{expandedGroup === 'planning' && <div className="mf-side-primary-children">{planningItems.map((item) => <NavigationLink key={item.to} item={item} compact />)}</div>}</section>
      <NavigationLink item={{ to: '/app/analises/insights', label: 'Insights', icon: Lightbulb }} />
      <section className="mf-side-tool" aria-label="Agenda Financeira"><ToolHeader to="/app/agenda" label="Agenda Financeira" icon={CalendarDays} active={activeGroup === 'agenda'} expanded={expandedGroup === 'agenda'} onToggle={() => toggleGroup('agenda')} />{expandedGroup === 'agenda' && <div className="mf-side-primary-children">{agendaItems.map((item) => <NavigationLink key={item.to} item={item} compact />)}</div>}</section>
      <NavigationLink item={{ to: '/app/integracoes', label: 'Conexões', icon: Bot }} />
      {betaEnabled && <div className="mf-side-beta-zone"><NavLink to="/app/beta/investimentos" className={`mf-side-item mf-side-beta-link ${betaActive ? 'active' : ''}`} title="MF Invest Beta" aria-label="MF Invest Beta"><Beaker size={16} aria-hidden="true" /><span>MF Invest</span><small className="mf-beta-badge">BETA</small></NavLink></div>}
    </nav>
    ...
    <nav className="mf-side-shortcuts" aria-label="Atalhos rápidos">
      <button type="button" className="mf-side-shortcut mf-side-search-shortcut" title="Buscar no MF (Ctrl/⌘ K)" aria-label="Buscar no MF" onClick={() => window.dispatchEvent(new Event('mf:open-command-palette'))}><Search size={14} aria-hidden="true" /></button>
