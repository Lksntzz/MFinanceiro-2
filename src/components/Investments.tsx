...
const SECTION_COPY: Record<PortfolioTab, { title: string; description: string }> = {
  portfolio: {
  ...
  planning: {
    ...
    description: 'Calcule uma margem de aporte usando sua situação financeira cadastrada.',
  },
};
...
export default function Investments({ user, settings, onRefresh }: InvestmentsProps) {
  ...
  const stats = useMemo(() => {
    ...
    const invested = investments.reduce((sum, item) => sum + safeNumber(item.initial_amount || item.amount), 0);
    const dividends = investments.reduce((sum, item) => sum + safeNumber(item.dividends_received), 0);
  ...
  async function recalculatePortfolio() {
    ...
    try {
      ...
      await fetchInvestments();
      onRefresh();
      setMessage('Carteira recalculada usando somente os preços atuais informados por você.');
    } catch (syncError: any) {
  ...
  if (loading) {
    return (
      <div className="flex flex-1 items-center justify-center" role="status" aria-live="polite">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-brand-primary border-t-transparent" aria-hidden="true" />
    ...
    <div className="flex flex-1 min-h-0 flex-col gap-4 overflow-hidden animate-fade-in">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-white/5 pb-3">
        <div className="min-w-0">
          <div className="text-[9px] font-black uppercase tracking-[0.22em] text-brand-primary/70">Investimentos</div>
          <h2 className="mt-1 text-xl font-black">{sectionCopy.title}</h2>
          <p className="mt-1 text-xs text-white/40">{sectionCopy.description}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {activeTab === 'planning' && (
            ...
            >
              <Calculator size={15} aria-hidden="true" /> {saving ? 'Calculando...' : 'Calcular aporte'}
            </button>
          )}
        </div>
      </header>
      ...
      {error && (
        <div className="flex shrink-0 items-center gap-2 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-2 text-xs text-red-400" role="alert">
          <AlertCircle size={15} aria-hidden="true" /> {error}
          <button type="button" onClick={() => setError(null)} className="ml-auto" aria-label="Fechar erro"><X size={14} aria-hidden="true" /></button>
        </div>
      )}
      {message && (
        <div className="shrink-0 rounded-xl border border-green-500/20 bg-green-500/10 px-4 py-2 text-xs text-green-400" role="status" aria-live="polite">
          {message}
        </div>
      )}
      ...
      {activeTab === 'portfolio' && (
        <div className="grid flex-1 min-h-0 grid-cols-1 gap-4 overflow-hidden lg:grid-cols-12">
          <section className="glass-card flex min-h-0 flex-col overflow-hidden !p-4 lg:col-span-8" aria-labelledby="investment-assets-title">
            <div className="mb-3 flex shrink-0 items-center justify-between gap-3">
              <h3 id="investment-assets-title" className="flex items-center gap-2 text-sm font-bold"><Briefcase size={16} aria-hidden="true" /> Ativos cadastrados</h3>
              <select
                value={filter}
                onChange={(event) => setFilter(event.target.value as 'all' | Investment['type'])}
                    ...
                    <article key={investment.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex gap-1">
                          <button type="button" onClick={() => analyzeInvestment(investment)} className="p-1.5 text-white/30 hover:text-brand-secondary" aria-label={`Analisar ${investment.name}`} title="Análise local"><BarChart2 size={14} aria-hidden="true" /></button>
                          <button type="button" onClick={() => openEdit(investment)} className="p-1.5 text-white/30 hover:text-white" aria-label={`Editar ${investment.name}`} title="Editar"><Pencil size={14} aria-hidden="true" /></button>
                          <button type="button" onClick={() => deleteInvestment(investment)} className="p-1.5 text-white/30 hover:text-red-400" aria-label={`Excluir ${investment.name}`} title="Excluir"><Trash2 size={14} aria-hidden="true" /></button>
                        </div>
                      </div>
          ...
          <section className="glass-card flex min-h-0 flex-col gap-4 !p-4 lg:col-span-4" aria-labelledby="investment-allocation-title">
            <h3 id="investment-allocation-title" className="flex items-center gap-2 text-sm font-bold"><PieChart size={16} aria-hidden="true" /> Distribuição da carteira</h3>
            <div className="space-y-4 overflow-y-auto pr-1">
              {stats.byType.map((item) => (
                <div key={item.type}>
                  ...
                  <div className="mt-1 h-2 overflow-hidden rounded-full bg-white/5" aria-hidden="true">
            ...
            <div className={`mt-auto rounded-xl border p-3 text-[10px] ${stats.targetTotal > 100 ? 'border-orange-400/20 bg-orange-400/5 text-orange-200/70' : 'border-white/10 bg-white/[0.03] text-white/40'}`}>
      ...
      {activeTab === 'income' && (
        <div className="grid flex-1 min-h-0 grid-cols-1 gap-4 overflow-hidden lg:grid-cols-12">
          <section className="glass-card min-h-0 overflow-y-auto !p-4 lg:col-span-7" aria-labelledby="investment-income-title">
            <h3 id="investment-income-title" className="mb-4 flex items-center gap-2 text-sm font-bold"><TrendingUp size={16} aria-hidden="true" /> Proventos por ativo</h3>
            <div className="grid gap-2">
              {investments.filter((item) => safeNumber(item.dividends_received) > 0).map((item) => (
                <div key={item.id} className="flex items-center justify-between gap-4 rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3">
                  <div className="min-w-0">
                    <div className="truncate text-sm font-bold">{item.name}</div>
                    <div className="text-[9px] uppercase text-white/30">Acumulado informado</div>
                  </div>
                  <strong className="shrink-0 text-green-400">{formatCurrency(item.dividends_received || 0, isPrivate)}</strong>
                </div>
              ))}
          ...
          <section className="glass-card flex flex-col justify-center gap-3 !p-5 lg:col-span-5">
            <Target className="text-brand-primary" size={28} aria-hidden="true" />
            <h3 className="text-lg font-bold">Proventos sem estimativa artificial</h3>
            <p className="text-xs leading-relaxed text-white/50">
              Esta seção usa exatamente o valor acumulado informado em cada investimento. Para alterar um valor, abra a Carteira e edite o ativo correspondente.
            </p>
            <div className="text-2xl font-black text-green-400">{formatCurrency(stats.dividends, isPrivate)}</div>
            <button type="button" onClick={() => goToSection('portfolio')} className="mt-2 w-fit rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-white/60">
              Ir para a carteira
            </button>
          </section>
        </div>
        ...
        <div className="grid flex-1 min-h-0 grid-cols-1 gap-4 overflow-hidden lg:grid-cols-12">
          <section className="glass-card flex flex-col justify-center gap-4 !p-5 lg:col-span-7">
            <div className="flex items-center gap-3">
              <div className="rounded-2xl bg-brand-primary/15 p-3 text-brand-primary"><Lightbulb size={22} aria-hidden="true" /></div>
              <div>
                <h3 className="font-bold">Margem de aporte</h3>
                <p className="text-[10px] uppercase tracking-widest text-white/30">Cálculo local, sem recomendação de ativo</p>
              </div>
            </div>
            ...
            {advice ? (
              <>
                <div className="rounded-2xl border border-brand-primary/20 bg-brand-primary/5 p-5">
                  <div className="text-[10px] font-bold uppercase text-white/40">Valor sugerido pelo cálculo</div>
                ...
                <p className="text-[10px] text-white/30">Ferramenta educacional de organização financeira. Não é recomendação de compra ou venda de ativos.</p>
              </>
            ) : (
              <div className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-xs text-white/40">
                Use “Calcular aporte” para considerar saldo, contas pendentes, metas e orçamentos cadastrados.
              </div>
            )}
          </section>

          <section className="glass-card !p-5 lg:col-span-5">
            <h3 className="mb-4 text-sm font-bold">Checklist antes de investir</h3>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm" role="presentation">
          <form onSubmit={saveInvestment} className="glass-card max-h-[90vh] w-full max-w-3xl overflow-y-auto no-scrollbar !p-6" role="dialog" aria-modal="true" aria-labelledby="investment-form-title">
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                <h2 id="investment-form-title" className="text-xl font-bold">{editing ? 'Editar investimento' : 'Novo investimento'}</h2>
                <p className="text-xs text-white/40">Use valores reais informados por você.</p>
              </div>
              <button type="button" onClick={() => setShowModal(false)} aria-label="Fechar formulário"><X size={20} aria-hidden="true" /></button>
            </div>
      ...
      {analysis && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm" role="presentation">
          <div className="glass-card w-full max-w-lg !p-6" role="dialog" aria-modal="true" aria-labelledby="investment-analysis-title">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 id="investment-analysis-title" className="text-xl font-bold">Análise local: {analysis.investment.name}</h2>
                <p className="text-xs text-white/40">Somente indicadores cadastrados.</p>
              </div>
              <button type="button" onClick={() => setAnalysis(null)} aria-label="Fechar análise"><X size={20} aria-hidden="true" /></button>
            </div>
...
function ChecklistItem({ done, text }: { done: boolean; text: string }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-white/5 bg-white/[0.03] p-3">
      <div className={`h-3 w-3 rounded-full ${done ? 'bg-green-400' : 'bg-orange-400'}`} aria-hidden="true" />
      <span>{text}</span>
    </div>
  );
}
