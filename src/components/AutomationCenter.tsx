...
function isAuthorizationPending(error: PluggyConnectError) {
  ...
  return status === 'USER_AUTHORIZATION_PENDING' || status === 'WAITING_USER_INPUT';
}
...
export default function AutomationCenter({ userId, accounts, categories }: AutomationCenterProps) {
  ...
  async function saveRule(event: React.FormEvent) {
    ...
    try {
      ...
      setMessage('Regra criada. Ela será aplicada conforme a prioridade configurada.');
      await load();
    } catch (saveError) {
  ...
  async function syncConnection(connectionId: string, source: 'initial' | 'manual' = 'manual') {
    setBusyConnectionId(connectionId);
    setError(null);
    try {
      const { data, error: syncError } = await supabase.functions.invoke('open-finance-sync', {
        body: { connectionId, triggerSource: source },
      });
      if (syncError) throw syncError;
      if (data?.error) throw new Error(String(data.error));
      if (data?.alreadySyncing) {
        setMessage('Essa instituição já está sincronizando. O resultado aparecerá automaticamente.');
      } else {
        const imported = Number(data?.importedCount || 0);
        const duplicates = Number(data?.duplicateCount || 0);
        setMessage(`Sincronização concluída: ${imported} novo(s) lançamento(s) e ${duplicates} já existente(s).`);
      }
      await load();
    } catch (syncError) {
      setError(syncError instanceof Error ? syncError.message : 'Não foi possível sincronizar a instituição.');
    } finally {
      setBusyConnectionId(null);
    }
  }

  async function bindAndSync(itemId: string, connector: PluggyConnector | null) {
    const institutionName = String(connector?.name || 'Instituição conectada').trim();
    const institutionId = connector?.id == null ? null : String(connector.id);
    const { data: bindData, error: bindError } = await supabase.functions.invoke('open-finance-session', {
      body: {
        action: 'bind',
        itemId,
        institutionId,
    ...
    if (!connectionId) throw new Error('O MF Financeiro não recebeu a referência da conexão.');

    setMessage(`${institutionName} conectada. Importando contas e movimentações...`);
    await load();
  ...
  async function launchOpenFinance(connection?: ProviderConnection) {
    ...
    if (connection && !itemId) {
      setError('Essa conexão antiga não possui a referência necessária para renovar o acesso.');
      return;
    }
    ...
    try {
      const { data, error: tokenError } = await supabase.functions.invoke('open-finance-session', {
        body: { action: 'token', itemId: itemId || undefined },
      });
      if (tokenError) throw tokenError;
      if (data?.error) throw new Error(String(data.error));
      const connectToken = String(data?.connectToken || data?.accessToken || '');
      if (!connectToken) throw new Error('O servidor não retornou o token temporário do Open Finance.');
      ...
      instance = await createPluggyConnect({
        ...
        onEvent: (payload: PluggyConnectEvent) => {
          if (String(payload.event || '').toUpperCase() === 'SELECTED_INSTITUTION' && payload.connector) {
        ...
        onSuccess: async (payload) => {
          ...
          if (!item?.id) {
            setError('A instituição concluiu o fluxo, mas não retornou um Item válido.');
            return;
          }
          ...
          try {
            ...
            setError(successError instanceof Error ? successError.message : 'A conexão foi autorizada, mas não foi possível importar os dados.');
            await load();
          }
        },
        onError: async (widgetError) => {
          ...
          if (isAuthorizationPending(widgetError)) {
            const connector = item?.connector || selectedConnectorRef.current;
            setMessage(`A autorização ainda está pendente${connector?.name ? ` em ${connector.name}` : ''}. Você pode fechar esta tela; o webhook continuará acompanhando a conclusão.`);
          } else {
            setError(widgetError.message || 'A instituição não concluiu a conexão Open Finance.');
          }
          await load();
        },
        onClose: async () => {
      ...
      setPreparingConnection(false);
      setError(connectionError instanceof Error ? connectionError.message : 'Não foi possível abrir o Open Finance.');
    }
  }

  async function requestRevocation(connection: ProviderConnection) {
    if (!window.confirm(`Revogar o acesso de ${connection.institution_name}? A instituição deixará de sincronizar novos dados.`)) return;
    ...
    try {
      ...
      if (data?.error) throw new Error(String(data.error));
      setMessage('Acesso revogado no provedor. Os dados já importados permanecem no seu histórico financeiro.');
      await load();
    } catch (revokeError) {
      setError(revokeError instanceof Error ? revokeError.message : 'Não foi possível revogar a conexão.');
    } finally {
      setBusyConnectionId(null);
    }
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[10px] text-white/35">Regras determinísticas, consentimentos Open Finance e sincronização auditável.</p>
        </div>
      ...
      {(error || message) && (
        <div className={`flex items-start justify-between gap-3 rounded-xl border p-3 text-xs ${error ? 'border-red-500/25 bg-red-500/10 text-red-200' : 'border-green-500/25 bg-green-500/10 text-green-200'}`} role={error ? 'alert' : 'status'} aria-live="polite">
      ...
      <div className="mf-subnav" role="tablist" aria-label="Automação e Open Finance">
        <button type="button" role="tab" aria-selected={activeTab === 'rules'} className={activeTab === 'rules' ? 'active' : ''} onClick={() => setActiveTab('rules')}>Regras automáticas</button>
        <button type="button" role="tab" aria-selected={activeTab === 'connections'} className={activeTab === 'connections' ? 'active' : ''} onClick={() => setActiveTab('connections')}>Open Finance</button>
      </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1.25fr)_minmax(280px,.75fr)]">
          <section className="glass-card !p-0 overflow-hidden">
            <div className="flex items-center justify-between border-b border-white/10 p-4">
              <div><h3 className="text-sm font-bold">Regras de categorização</h3><p className="mt-1 text-[9px] text-white/35">Maior prioridade vence; regras manuais têm precedência sobre padrões aprendidos.</p></div>
        ...
        <div className="space-y-4">
          <section className="glass-card grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            <div className="flex gap-4">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-brand-primary/10 text-brand-primary"><Landmark size={21} /></div>
              <div>
                <h3 className="text-sm font-bold">Conectar instituição pelo Open Finance</h3>
                <p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-white/45">A seleção do banco, consentimento, OAuth e autenticação acontecem dentro do Pluggy Connect. O MF Financeiro recebe apenas o identificador da conexão e os dados autorizados; suas credenciais bancárias não passam pelo nosso frontend.</p>
                <div className="mt-2 flex flex-wrap gap-2 text-[9px] text-white/35"><span className="rounded-full bg-white/5 px-2 py-1">Contas</span><span className="rounded-full bg-white/5 px-2 py-1">Movimentações</span><span className="rounded-full bg-white/5 px-2 py-1">Sincronização por webhook</span>{includeSandbox && <span className="rounded-full bg-yellow-500/10 px-2 py-1 text-yellow-200">Sandbox habilitado no preview</span>}</div>
              </div>
            </div>
            <button type="button" disabled={preparingConnection || Boolean(busyConnectionId)} onClick={() => void launchOpenFinance()} className="flex items-center justify-center gap-2 rounded-xl bg-brand-primary px-5 py-3 text-xs font-black text-black disabled:opacity-40">{preparingConnection ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} Conectar instituição</button>
          </section>

          <div className="rounded-xl border border-green-500/15 bg-green-500/[0.04] p-3 text-[10px] text-white/50"><ShieldCheck size={14} className="mr-2 inline text-green-300" />Connect Tokens são temporários e gerados no servidor. `CLIENT_ID` e `CLIENT_SECRET` permanecem exclusivamente nos secrets do Supabase.</div>

          <section className="glass-card !p-0 overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 p-4"><div><h3 className="text-sm font-bold">Instituições conectadas</h3><p className="mt-1 text-[9px] text-white/35">Consentimento e sincronização são acompanhados separadamente.</p></div><span className="text-[10px] text-white/35">{connections.filter((item) => item.status === 'active').length} ativa(s)</span></div>
            <div className="divide-y divide-white/5">
              {connections.map((connection) => {
                ...
                const canUpdate = Boolean(itemId) && !['revoked', 'revocation_pending'].includes(connection.status);
                const canSync = connection.status === 'active' && Boolean(itemId);
                return (
                  <article key={connection.id} className="p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <div className="flex flex-wrap items-center gap-2"><strong className="text-xs">{connection.display_name || connection.institution_name}</strong><span className={`rounded-full px-2 py-0.5 text-[8px] font-bold ${connection.status === 'active' ? 'bg-green-500/10 text-green-300' : connection.status === 'error' ? 'bg-red-500/10 text-red-300' : 'bg-yellow-500/10 text-yellow-200'}`}>{statusLabels[connection.status]}</span></div>
                        <p className="mt-1 text-[9px] text-white/35">Provedor: {connection.provider} · última sincronização {formatDate(connection.last_synced_at)}</p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {canSync && <button type="button" disabled={busy || preparingConnection} onClick={() => void syncConnection(connection.id)} className="flex items-center gap-1.5 rounded-lg border border-brand-primary/20 px-3 py-2 text-[9px] text-brand-primary disabled:opacity-40">{busy ? <Loader2 size={12} className="animate-spin" /> : <RefreshCw size={12} />} Sincronizar</button>}
                        {canUpdate && <button type="button" disabled={busy || preparingConnection} onClick={() => void launchOpenFinance(connection)} className="flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-2 text-[9px] text-white/60 disabled:opacity-40"><RotateCw size={12} /> Atualizar acesso</button>}
                        {!['revoked', 'revocation_pending'].includes(connection.status) && <button type="button" disabled={busy || preparingConnection} onClick={() => void requestRevocation(connection)} className="flex items-center gap-1.5 rounded-lg border border-red-500/15 px-3 py-2 text-[9px] text-red-300 disabled:opacity-40"><Unplug size={12} /> Revogar</button>}
                      </div>
                    </div>
                    <div className="mt-3 grid grid-cols-2 gap-2 text-[9px] sm:grid-cols-4">
                      <div className="rounded-lg bg-white/5 p-2"><span className="text-white/30">Sincronização</span><strong className="mt-1 block">{syncLabels[connection.sync_status]}</strong></div>
                      ...
                      <div className="rounded-lg bg-white/5 p-2"><span className="text-white/30">Importados</span><strong className="mt-1 block">{lastRun?.imported_count || 0}</strong></div>
                      <div className="rounded-lg bg-white/5 p-2"><span className="text-white/30">Já existentes</span><strong className="mt-1 block">{lastRun?.duplicate_count || 0}</strong></div>
                    </div>
                    {lastRun && <p className="mt-2 flex items-center gap-1 text-[9px] text-white/30"><Clock3 size={11} /> Última execução: {lastRun.status} · {lastRun.trigger_source} · {formatDate(lastRun.finished_at || lastRun.created_at)}</p>}
                    {(connection.last_error || lastRun?.error_message) && <p className="mt-2 rounded-lg bg-red-500/5 p-2 text-[9px] text-red-300">{connection.last_error || lastRun?.error_message}</p>}
                  </article>
                );
              })}
              {connections.length === 0 && <div className="py-16 text-center text-xs text-white/30">Nenhuma instituição conectada. Use “Conectar instituição” para iniciar um consentimento.</div>}
            </div>
          </section>
        </div>
      )}
