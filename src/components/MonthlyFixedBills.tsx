...
export default function MonthlyFixedBills({
  userId,
  onDataChanged,
}: {
  userId: string;
  ...
  async function load() {
    ...
    try {
      ...
      const { data, error: queryError } = await supabase
        ...
        .select(
          'id,fixed_bill_id,competence,due_date,amount,status,paid_at,notes,fixed_bill:mf_fixed_bills(id,name,category,default_amount,default_due_day,active)',
        )
        .eq('user_id', userId)
      ...
      const normalized = (data || [])
        .map((item: any) => ({
          ...
          fixed_bill: Array.isArray(item.fixed_bill)
            ? item.fixed_bill[0]
            : item.fixed_bill,
        }))
      ...
      setError(
        loadError?.message || 'Não foi possível carregar as contas mensais.',
      );
    } finally {
      setLoading(false);
    }
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'mf_fixed_bill_occurrences',
  ...
  const selectedRows = rows.filter(
    (row) => row.competence === selectedMonth && row.status !== 'skipped',
  );
  const ignoredRows = rows.filter(
    (row) => row.competence === selectedMonth && row.status === 'skipped',
  );
  const selectedTotal = selectedRows.reduce((sum, row) => sum + row.amount, 0);
  const pendingTotal = selectedRows
    .filter((row) => row.status === 'pending')
    .reduce((sum, row) => sum + row.amount, 0);
  const paidTotal = selectedRows
  ...
  async function saveEdit(event: React.FormEvent) {
    ...
    try {
      ...
      if (editScope === 'month') {
        ...
        if (rpcError) throw rpcError;
        announce(
          `${monthLabel(editing.competence)} foi alterado sem modificar os outros meses.`,
        );
      } else {
        const fromCompetence =
          editing.status === 'paid'
  ...
  async function pay(row: Occurrence) {
    ...
    try {
      ...
      if (rpcError) throw rpcError;
      announce(
        `${row.fixed_bill.name} foi paga. A conta do mês seguinte já está disponível.`,
      );
      await load();
    } catch (payError: any) {
  ...
  async function reopen(row: Occurrence) {
    if (
      !window.confirm(
        `Desfazer o pagamento de ${row.fixed_bill.name} em ${monthLabel(row.competence)}?`,
      )
    )
      return;
    setSaving(true);
    try {
      ...
      setError(
        actionError?.message || 'Não foi possível desfazer o pagamento.',
      );
    } finally {
      setSaving(false);
    }
  ...
  async function skip(row: Occurrence) {
    if (
      !window.confirm(
        `Ignorar somente ${monthLabel(row.competence)} de ${row.fixed_bill.name}? Os próximos meses continuarão registrados.`,
      )
    )
      return;
    setSaving(true);
    try {
      ...
      setError(actionError?.message || 'Não foi possível ignorar este mês.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function endRecurring(row: Occurrence) {
    if (
      !window.confirm(
        `Encerrar a recorrência de ${row.fixed_bill.name}? O histórico pago será preservado e os próximos meses serão cancelados.`,
      )
    )
      return;
    setSaving(true);
    try {
      const { error: rpcError } = await supabase.rpc(
        ...
        { p_fixed_bill_id: row.fixed_bill_id },
      );
      if (rpcError) throw rpcError;
      ...
      setError(
        actionError?.message || 'Não foi possível encerrar a recorrência.',
      );
    } finally {
      setSaving(false);
    }
  ...
  function nextOccurrence(row: Occurrence) {
    return rows
      .filter(
        (item) =>
          item.fixed_bill_id === row.fixed_bill_id &&
          item.competence > row.competence &&
          item.status !== 'skipped',
      )
  ...
  return (
    <div className="mf-monthly-bills-page">
      ...
      <header className="mf-bills-heading">
        <div>
          <h2 className="flex items-center gap-2">
            <Repeat2 size={19} className="text-brand-primary" /> Contas fixas
            por mês
          </h2>
          <p>
            Uma conta recorrente gera automaticamente cada mês. O valor de um
            mês pode ser diferente sem alterar os anteriores.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="mf-small-action primary"
          >
            <Plus size={13} /> Nova conta fixa
          </button>
        </div>
      </header>
      ...
      {(error || message) && (
        <div
          className={`rounded-xl border px-4 py-2 text-xs ${error ? 'border-red-500/20 bg-red-500/10 text-red-300' : 'border-green-500/20 bg-green-500/10 text-green-300'}`}
        >
          {error || message}
          <button
            type="button"
      ...
      <div className="mf-bills-months">
        {months.map((month, index) => (
          <button
            key={month.key}
            type="button"
            className={`mf-month-chip ${selectedMonth === month.key ? 'active' : ''}`}
      ...
      <section className="mf-bills-metrics">
        <div className="mf-bills-metric">
          <span>Total do mês</span>
          <strong>{money(selectedTotal)}</strong>
        </div>
        <div className="mf-bills-metric">
          <span>Pendente</span>
          <strong className="text-yellow-300">{money(pendingTotal)}</strong>
        </div>
        <div className="mf-bills-metric">
          <span>Pago</span>
          <strong className="text-green-300">{money(paidTotal)}</strong>
        </div>
        <div className="mf-bills-metric">
      ...
      <section className="mf-bills-list">
        {loading ? (
          <div className="col-span-full flex min-h-44 items-center justify-center text-xs text-white/35">
            Carregando contas mensais...
          </div>
          ...
          selectedRows.map((row) => {
            ...
            return (
              <article
                key={row.id}
                className={`mf-occurrence-card ${row.status}`}
              >
                <div className="mf-occurrence-title">
                  <div>
                    <h3>{row.fixed_bill.name}</h3>
                    <p>
                      {row.fixed_bill.category || 'Conta fixa'} · vence{' '}
                      {format(parseISO(row.due_date), 'dd/MM/yyyy')}
                    </p>
                  </div>
                  <span className={`mf-status ${row.status}`}>
                    {row.status === 'paid' ? 'Paga' : 'Pendente'}
                  </span>
                </div>
                <div className="mf-occurrence-value">{money(row.amount)}</div>
                {row.notes && (
                  <p className="mt-2 text-[9px] text-white/35">{row.notes}</p>
                )}
                <div className="mf-occurrence-actions">
                  ...
                  {row.status === 'paid' ? (
                    ...
                    >
                      <RefreshCcw size={12} /> Desfazer pagamento
                    </button>
                  ) : (
                    <button
                      type="button"
                  ...
                  <button
                    type="button"
                    className="mf-small-action text-red-300"
                    onClick={() => void endRecurring(row)}
                  >
                    <Trash2 size={12} /> Encerrar recorrência
                  </button>
                </div>
                {row.status === 'paid' && next && (
                  <div className="mf-next-bill">
                    <strong className="text-brand-primary">
                      Próximo mês já registrado:
                    </strong>{' '}
                    {monthLabel(next.competence)} · {money(next.amount)} · vence{' '}
                    {format(parseISO(next.due_date), 'dd/MM')}
                  </div>
                )}
              </article>
            );
        ...
        {ignoredRows.length > 0 && (
          <div className="col-span-full text-[9px] text-white/25">
            {ignoredRows.length} conta(s) ignorada(s) somente neste mês.
          </div>
        )}
      </section>
      ...
      <div className="flex items-center justify-between text-[9px] text-white/30">
        ...
        >
          <ChevronLeft size={12} /> Mês anterior
        </button>
        <span>
          <CalendarDays size={12} className="mr-1 inline" /> Os meses futuros
        ...
        <button
          ...
          onClick={() => {
            ...
            if (index >= 0 && index < months.length - 1)
              setSelectedMonth(months[index + 1].key);
          }}
        >
          Próximo mês <ChevronRight size={12} />
        </button>
      </div>
            ...
            <header>
              <div>
                ...
                <p className="mt-1 text-[9px] text-white/35">
                  Ela será repetida automaticamente nos próximos meses.
                </p>
              </div>
            ...
            <form onSubmit={createRecurring}>
              ...
              <label className="mf-bill-field">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setNewForm({ ...newForm, amount: event.target.value })
                  }
                />
              </label>
              <label className="mf-bill-field">
                ...
                <input
                  ...
                  min="1"
                  max="31"
                  value={newForm.dueDay}
                  onChange={(event) =>
                    setNewForm({ ...newForm, dueDay: event.target.value })
                  }
                />
              </label>
              <label className="mf-bill-field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setNewForm({ ...newForm, category: event.target.value })
                  }
                >
              ...
              <div className="mf-dialog-actions">
                ...
                <button className="mf-small-action primary" disabled={saving}>
                  <Save size={13} />{' '}
      ...
      {editing && (
        <div className="mf-bill-dialog-backdrop">
          <div className="mf-bill-dialog">
            <header>
              <div>
                <strong>Editar {editing.fixed_bill.name}</strong>
                <p className="mt-1 text-[9px] text-white/35">
                  Escolha se a mudança vale somente para um mês ou para os
                  próximos.
                </p>
              </div>
            ...
            <form onSubmit={saveEdit}>
              <div className="mf-edit-scope">
                <button
                  ...
                  className={editScope === 'month' ? 'active' : ''}
                  onClick={() => setEditScope('month')}
                >
                  <strong>Somente {monthLabel(editing.competence)}</strong>
                  <span className="mt-1 block text-[9px] opacity-60">
                    Não altera os demais meses.
                  </span>
                </button>
                <button
                  type="button"
                  className={editScope === 'future' ? 'active' : ''}
                ...
                >
                  ...
                  <span className="mt-1 block text-[9px] opacity-60">
                    Atualiza o valor padrão futuro.
                  </span>
                </button>
              </div>
              <label className="mf-bill-field">
              ...
              {editScope === 'month' && (
                <label className="mf-bill-field col-span-full">
                  Observação
                  <textarea
                    ...
                    placeholder="Ex.: consumo maior neste mês"
                  />
                </label>
              )}
              <label className="mf-bill-field col-span-full">
                Forma usada ao marcar como paga
                <select
                  value={paymentMethod}
