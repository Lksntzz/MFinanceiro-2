  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col gap-4 overflow-hidden animate-fade-in">
      <section className="grid shrink-0 grid-cols-2 lg:grid-cols-4 gap-3">
        <Metric
          label="Objetivos"
          value={formatCurrency(totals.target, isPrivate)}
          icon={Target}
        />
        <Metric
          label="Já guardado"
          value={formatCurrency(totals.current, isPrivate)}
          icon={TrendingUp}
          positive
        />
        <Metric
          label="Concluídas"
          value={`${totals.completed} meta(s)`}
          icon={Trophy}
        />
      ...
      <div className="flex shrink-0 items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-black">Metas financeiras</h2>
          <p className="text-xs text-white/40">
            {totals.active} ativa(s), {totals.completed} concluída(s).
          </p>
        </div>
        <button
          onClick={openCreate}
          className="flex items-center gap-2 rounded-xl bg-brand-primary px-4 py-2 text-xs font-bold text-black"
        >
      ...
      <section className="flex-1 min-h-0 glass-card !p-4 flex flex-col overflow-hidden">
        {visible.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-white/10 text-center text-white/30">
            <Target size={30} />
            <p className="text-xs">Nenhuma meta cadastrada.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 content-start">
            {visible.map((goal) => {
              ...
              return (
                <article
                  key={goal.id}
                  className={`rounded-2xl border p-4 ${complete ? 'border-green-500/20 bg-green-500/5' : 'border-white/10 bg-white/[0.03]'}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 gap-3">
                      <div
                        className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${complete ? 'bg-green-500/15 text-green-400' : 'bg-brand-primary/10 text-brand-primary'}`}
                      >
                        {complete ? (
                  ...
                  <div className="mt-4 flex items-end justify-between gap-3">
                    <div>
                      <div className="text-[9px] text-white/30">Guardado</div>
                      <strong className={complete ? 'text-green-400' : ''}>
                        {formatCurrency(goal.current_amount, isPrivate)}
                      </strong>
                    </div>
                    <div className="text-right">
                      <div className="text-[9px] text-white/30">Objetivo</div>
                      <strong className="text-xs text-white/60">
                        {formatCurrency(goal.target_amount, isPrivate)}
                      </strong>
                    </div>
                  </div>
                  <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/5">
...
function Metric({
  ...
  return (
    <div className="glass-card !p-4">
      ...
      <div
        className={`mt-2 truncate text-sm font-black ${positive ? 'text-green-400' : ''}`}
      >
        {value}
      </div>
    </div>
