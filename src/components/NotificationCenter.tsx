...
function readReleaseState() {
  try { return window.localStorage.getItem(RELEASE_READ_KEY) === '1'; } catch { return false; }
}
...
export default function NotificationCenter({ notifications, onPay, onDismiss, onClose, isOpen, showReleaseUpdate = true, onNavigate }: NotificationCenterProps) {
  ...
  const quality = useMemo(() => notifications.filter((item) => item.type === 'quality' || item.status === 'attention'), [notifications]);
  const dueItems = useMemo(() => notifications.filter((item) => item.type !== 'quality' && item.status !== 'attention').map(normalizeDueNotification).sort((a, b) => a.daysUntilDue - b.daysUntilDue), [notifications]);
  ...
  function markReleaseRead() {
    setReleaseRead(true);
    try { window.localStorage.setItem(RELEASE_READ_KEY, '1'); } catch { /* optional */ }
  }

  async function handlePay(item: NotificationItem) {
    try { setPayingId(item.id); await onPay(item); } finally { setPayingId(null); }
  }
    ...
    {isOpen && <>
      ...
      <motion.aside initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed right-0 top-0 z-[70] flex h-full w-full max-w-sm flex-col border-l border-white/10 bg-[#0a0a0a] shadow-2xl" aria-label="Central de notificações">
        <div className="flex items-center justify-between border-b border-white/10 bg-white/5 p-6">
          <div className="flex items-center gap-3"><Bell className="text-brand-primary" size={20} /><div><h2 className="text-lg font-bold">Notificações</h2><p className="text-[10px] uppercase tracking-wider text-white/40">{notifications.length} atenção{notifications.length === 1 ? '' : 'ões'} financeira{notifications.length === 1 ? '' : 's'}</p></div></div>
          <button onClick={onClose} className="rounded-lg p-2 transition-colors hover:bg-white/10" aria-label="Fechar notificações"><X size={20} className="text-white/40" /></button>
        </div>
        ...
        <div className="no-scrollbar flex-1 space-y-6 overflow-y-auto p-4">
          ...
          {quality.length > 0 && <NotificationGroup title="Qualidade dos dados" icon={<AlertCircle size={12} />} className="text-amber-300">
            {quality.map((item) => <AttentionCard key={item.id} item={item} onDismiss={onDismiss} onNavigate={onNavigate} />)}
          </NotificationGroup>}

          {dueItems.length === 0 && quality.length === 0 ? <div className="rounded-2xl border border-white/8 bg-white/[0.025] p-6 text-center opacity-70"><CheckCircle2 size={34} className="mx-auto mb-3 text-green-500" /><p className="text-xs font-bold uppercase tracking-widest">Tudo em ordem</p><p className="mt-2 text-xs text-white/45">Nenhum alerta financeiro relevante agora.</p></div> : <>
...
function NotificationGroup({ title, icon, className, children }: { title: string; icon: React.ReactNode; className: string; children: React.ReactNode }) {
  return <section className="space-y-3"><h3 className={`flex items-center gap-2 px-2 text-[10px] font-bold uppercase tracking-widest ${className}`}>{icon} {title}</h3>{children}</section>;
}
...
function AttentionCard({ item, onDismiss, onNavigate }: { item: NotificationItem; onDismiss: (id: string) => void; onNavigate?: (path: string) => void }) {
  return <article className="rounded-2xl border border-amber-400/18 bg-amber-400/[0.045] p-4"><div className="mb-2 flex items-start justify-between gap-3"><div><strong className="text-xs text-white/85">{item.title}</strong><p className="mt-1 text-[10px] leading-relaxed text-white/45">{item.detail || 'Este ponto pode reduzir a confiança das suas análises.'}</p></div><button type="button" onClick={() => onDismiss(item.id)} className="text-white/25 hover:text-white/60" aria-label="Dispensar alerta"><X size={14} /></button></div>{item.actionPath && <button type="button" onClick={() => onNavigate?.(item.actionPath!)} className="inline-flex items-center gap-1.5 rounded-lg border border-amber-300/20 bg-amber-300/10 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-amber-200">{item.actionLabel || 'Revisar'}<ExternalLink size={11} /></button>}</article>;
}
...
function DueNotificationCard({ item, onPay, onDismiss, paying, onNavigate }: { item: DueNotification; onPay: (item: NotificationItem) => void; onDismiss: (id: string) => void; paying: boolean; onNavigate?: (path: string) => void }) {
  ...
  return <article className={`rounded-2xl border p-4 ${isOverdue ? 'border-red-500/20 bg-red-500/5' : isToday ? 'border-brand-primary/20 bg-brand-primary/5' : 'border-white/10 bg-white/5'}`}>
    <div className="mb-3 flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${item.type === 'card' ? 'bg-violet-500/10 text-violet-300' : 'bg-brand-primary/10 text-brand-primary'}`}><Wallet size={17} /></div><div className="min-w-0"><h4 className="truncate text-xs font-bold text-white/85">{item.title}</h4><p className="mt-1 text-[10px] text-white/40">{detail}</p>{Number(item.amount || 0) > 0 && <strong className="mt-1 block text-[11px] text-white/65">R$ {Number(item.amount || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</strong>}</div></div><button type="button" onClick={() => onDismiss(item.id)} className="text-white/25 hover:text-white/60" aria-label="Dispensar alerta"><X size={14} /></button></div>
    <div className="flex gap-2">{canPay && <button type="button" onClick={() => onPay(item)} disabled={paying} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-brand-primary px-3 py-2 text-[9px] font-black uppercase tracking-wider text-black disabled:opacity-50">{paying ? 'Registrando…' : 'Registrar pagamento'}</button>}{item.type === 'card' && <button type="button" onClick={() => onNavigate?.('/app/planejamento/cartoes')} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-white/60">Ver cartão<ExternalLink size={11} /></button>}</div>
  </article>;
}
