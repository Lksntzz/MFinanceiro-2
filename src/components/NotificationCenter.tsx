...
function readReleaseDismissed() {
  try {
    return window.localStorage.getItem(RELEASE_DISMISSED_KEY) === '1';
  } catch {
    return false;
  }
}
function clampDueDay(value: unknown): number {
  ...
  return Number.isFinite(parsed)
    ? Math.min(31, Math.max(1, Math.trunc(parsed)))
    : 1;
}
...
function isAttention(item: NotificationItem) {
  return (
    item.type === 'quality' ||
    item.status === 'attention' ||
    (item.type === 'card' && Boolean(item.detail))
  );
}

function normalizeDueNotification(item: NotificationItem): DueNotification {
  const dueDay = clampDueDay(
    item.dueDate ?? item.originalData?.due_day ?? item.originalData?.dueDate,
  );
  ...
  const status: DueNotification['status'] =
...
export default function NotificationCenter({
  ...
  const [releaseDismissed, setReleaseDismissed] =
    useState(readReleaseDismissed);
  const attention = useMemo(
    () => notifications.filter(isAttention),
    [notifications],
  );
  ...
  function dismissRelease() {
    ...
    try {
      window.localStorage.setItem(RELEASE_DISMISSED_KEY, '1');
    } catch {
      /* optional */
    }
  }
      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed right-0 top-0 z-[70] flex h-full w-full max-w-sm flex-col border-l border-white/10 bg-[#0a0a0a] shadow-2xl"
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] uppercase tracking-wider text-white/40">
                    {notifications.length} atenção
                    {notifications.length === 1 ? '' : 'ões'} financeira
            ...
            <div className="no-scrollbar flex-1 space-y-6 overflow-y-auto p-4">
              {showReleaseUpdate && !releaseDismissed && (
                <ReleaseUpdateCard onDismiss={dismissRelease} />
              )}
              {attention.length > 0 && (
                <NotificationGroup
                  title="Atenção"
              ...
              {dueItems.length === 0 && attention.length === 0 ? (
                <div className="rounded-2xl border border-white/8 bg-white/[0.025] p-6 text-center opacity-70">
                  ...
                  <p className="mt-2 text-xs text-white/45">
                    Nenhum alerta financeiro relevante agora.
                  </p>
                </div>
              ) : (
                <>
            ...
            <div className="border-t border-white/10 bg-white/5 p-4">
              <div className="rounded-xl border border-brand-primary/20 bg-brand-primary/10 p-3">
                <h4 className="mb-1 text-[10px] font-bold uppercase tracking-widest text-brand-primary">
                  Menos ruído
                </h4>
                <p className="text-[10px] leading-relaxed text-white/60">
                  Use Preferências para escolher os alertas que o MF deve
                  mostrar. Ao dispensar um alerta, ele não reaparece só porque
                  você mudou de ferramenta.
                </p>
              </div>
            </div>
          </motion.aside>
    ...
    <section className="overflow-hidden rounded-2xl border border-violet-400/20 bg-gradient-to-br from-violet-500/10 via-cyan-500/[0.045] to-transparent">
      ...
      <div className="space-y-2.5 p-4">
        {update.highlights.map((highlight) => (
          <div
            key={highlight}
            className="flex items-start gap-2.5 text-xs leading-relaxed text-white/60"
          >
...
function NotificationGroup({
  ...
  return (
    <section className="space-y-3">
      <h3
        className={`flex items-center gap-2 px-2 text-[10px] font-bold uppercase tracking-widest ${className}`}
      >
        {icon} {title}
      </h3>
      {children}
...
function AttentionCard({
  ...
  const actionPath =
    item.actionPath ||
    (item.type === 'card' ? '/app/planejamento/cartoes' : undefined);
  const actionLabel =
    item.actionLabel || (item.type === 'card' ? 'Ver cartão' : 'Revisar');
  return (
    <article className="rounded-2xl border border-amber-400/18 bg-amber-400/[0.045] p-4">
      <div className="mb-2 flex items-start justify-between gap-3">
        <div>
          <strong className="text-xs text-white/85">{item.title}</strong>
          <p className="mt-1 text-[10px] leading-relaxed text-white/45">
            {item.detail || 'Este ponto merece uma revisão rápida.'}
          </p>
        </div>
        <button
          type="button"
      ...
      {actionPath && (
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg border border-amber-300/20 bg-amber-300/10 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-amber-200"
        >
          {actionLabel}
...
function DueNotificationCard({
  ...
  const detail =
    item.detail ||
    (isOverdue
      ? `Venceu em ${item.dueDateLabel}`
      : isToday
        ? 'Vence hoje'
        : `Vence em ${item.dueDateLabel}`);
  return (
    <article
      className={`rounded-2xl border p-4 ${isOverdue ? 'border-red-500/20 bg-red-500/5' : isToday ? 'border-brand-primary/20 bg-brand-primary/5' : 'border-white/10 bg-white/5'}`}
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h4 className="truncate text-xs font-bold text-white/85">
              {item.title}
            </h4>
            <p className="mt-1 text-[10px] text-white/40">{detail}</p>
            {Number(item.amount || 0) > 0 && (
              <strong className="mt-1 block text-[11px] text-white/65">
                R${' '}
      ...
      <div className="flex gap-2">
        {canPay && (
          <button
            ...
            onClick={() => onPay(item)}
            disabled={paying}
            className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-brand-primary px-3 py-2 text-[9px] font-black uppercase tracking-wider text-black disabled:opacity-50"
          >
        ...
        {item.type === 'card' && (
          <button
            type="button"
            onClick={() => onNavigate?.('/app/planejamento/cartoes')}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-white/60"
          >
            Ver cartão
            <ExternalLink size={11} />
          </button>
