...
function normalizeInstallment(row: any): CardInstallment {
  return {
    ...
    current_installment: Number(
      row.current_installment ?? row.parcela_atual ?? 1,
    ),
    total_installments: Number(
      row.total_installments ?? row.total_parcelas ?? 1,
    ),
    due_day: Number(row.due_day ?? 1),
  } as CardInstallment;
}
...
function MobileHeader({
  ...
  return (
    <header className="mf-mobile-page-header">
      ...
      <div className="mf-mobile-brand-mark">
        <Wallet size={20} />
      </div>
    </header>
  );
...
function SafeToSpendCard({ summary }: { summary: FinanceSummary }) {
  ...
  const commitments = Math.max(
    0,
    summary.currentBalance - summary.projectedBalance,
  );
  const tone =
    summary.projectedBalance < 0
      ? 'danger'
      : summary.smartAlert?.type || 'success';
  ...
  return (
    <section className="mf-mobile-safe-card" data-tone={tone}>
      <div className="mf-mobile-safe-card__head">
        <div>
          <span>
            <ShieldCheck size={15} /> Disponível de verdade
          </span>
          <small>Depois dos compromissos cadastrados deste ciclo</small>
        </div>
        <div className="mf-mobile-safe-card__status">
          <Gauge size={18} />
        </div>
      </div>
      <strong className="mf-mobile-safe-card__value">
      ...
      <div className="mf-mobile-safe-card__foot">
        ...
        <span>
          ...
          {summary.daysRemaining === 1 ? 'dia restante' : 'dias restantes'}
        </span>
      </div>
      {summary.smartAlert?.message ? (
        <p className="mf-mobile-safe-card__insight">
          {summary.smartAlert.message}
        </p>
      ) : null}
      <button
        type="button"
        className="mf-mobile-safe-card__simulate"
        onClick={() => navigate(MOBILE_ROUTES.canSpend)}
      >
        ...
        return (
          <div className="mf-mobile-row" key={item.id}>
            ...
            <div className="mf-mobile-row-main">
              <strong>
                {item.description || item.category || 'Movimentação'}
              </strong>
              <small>
                {item.category || 'Geral'} •{' '}
                {new Date(
            ...
            <b className={income ? 'mf-mobile-positive' : ''}>
              {income ? '+' : '−'} {formatCurrency(Math.abs(item.amount))}
            </b>
          </div>
        );
      })}
...
function HomePage({ data }: { data: MobileData }) {
  ...
  const balance =
    ...
    data.accounts.reduce((sum, account) => sum + account.current_balance, 0);
  const monthExpense = data.monthTransactions
    .filter(
      (item) =>
        item.type === 'expense' &&
        !['pending', 'duplicate', 'error'].includes(
  ...
  return (
    <div className="mf-mobile-page">
      <MobileHeader
        title={firstName ? `Olá, ${firstName}` : 'Seu dinheiro hoje'}
      ...
      <section className="mf-mobile-balance-card">
        ...
        <div className="mf-mobile-balance-meta">
          <div>
            ...
            <b>{formatCurrency(monthIncome)}</b>
          </div>
          <div>
            <small>Saídas no mês</small>
            <b>{formatCurrency(monthExpense)}</b>
          </div>
        </div>
      </section>
      {data.summary ? <SafeToSpendCard summary={data.summary} /> : null}
      <section className="mf-mobile-quick-actions" aria-label="Ações rápidas">
        <button type="button" onClick={() => navigate(MOBILE_ROUTES.quick)}>
          <ReceiptText size={20} />
          <span>Lançar</span>
        </button>
        <button type="button" onClick={() => navigate(MOBILE_ROUTES.scan)}>
          <ScanLine size={20} />
          <span>Escanear</span>
        </button>
        <button type="button" onClick={() => navigate(MOBILE_ROUTES.cards)}>
      ...
      <section className="mf-mobile-section">
        <div className="mf-mobile-section-title">
          <div>
            ...
            <small>O que merece atenção</small>
          </div>
        </div>
        <div className="mf-mobile-list-card">
          {data.pending.length ? (
            data.pending.slice(0, 3).map((item) => (
              <div className="mf-mobile-row" key={item.id}>
                <div className="mf-mobile-row-icon">
                  <ReceiptText size={17} />
                </div>
                <div className="mf-mobile-row-main">
                  <strong>
                    {item.description || item.category || 'Compromisso'}
                  </strong>
                  <small>
                    {item.due_date
            ...
            <div className="mf-mobile-empty">
              Nenhum compromisso pendente próximo.
            </div>
          )}
        </div>
      </section>
      <section className="mf-mobile-section">
        <div className="mf-mobile-section-title">
          <div>
            ...
            <small>Seu dia a dia</small>
          </div>
          <button
            type="button"
            onClick={() => navigate(MOBILE_ROUTES.transactions)}
          >
...
function CardsPage({ cards }: { cards: CreditCard[] }) {
  return (
    <div className="mf-mobile-page">
      <MobileHeader
        title="Cartões"
        subtitle="Faturas e limites sem excesso de informação"
      />
      <div className="mf-mobile-card-stack">
        {cards.length ? (
          cards.map((card) => {
            ...
            return (
              <article className="mf-mobile-credit-card" key={card.id}>
                <div className="mf-mobile-credit-card__top">
                  <div>
                ...
                <div className="mf-mobile-credit-card__amount">
                  <span>Utilizado</span>
                  <strong>{formatCurrency(used)}</strong>
                </div>
                <div className="mf-mobile-progress">
                  <i style={{ width: `${usage}%` }} />
                </div>
                <div className="mf-mobile-credit-card__meta">
                  <div>
                    <small>Disponível</small>
                    <b>{formatCurrency(limit - used)}</b>
                  </div>
                  <div>
          ...
          <div className="mf-mobile-list-card">
            <div className="mf-mobile-empty">Nenhum cartão cadastrado.</div>
          </div>
        )}
      </div>
    </div>
...
function MorePage() {
  ...
  return (
    <div className="mf-mobile-page">
      <MobileHeader
        title="Mais"
        subtitle="Poucas ferramentas, só as que ajudam no celular"
      />
      <div className="mf-mobile-menu-list">
        <button type="button" onClick={() => navigate(MOBILE_ROUTES.profile)}>
          <span className="mf-mobile-menu-icon">
            <UserRound size={19} />
          </span>
          <div>
            <strong>Perfil</strong>
            <small>Conta, preferências e segurança</small>
          </div>
          <ChevronRight size={18} />
        </button>
        <button type="button" onClick={() => navigate(MOBILE_ROUTES.inbox)}>
          <span className="mf-mobile-menu-icon">
            <Inbox size={19} />
          </span>
          <div>
            <strong>MF Inbox</strong>
            <small>Revisar extratos e lançamentos extraídos</small>
          </div>
          <ChevronRight size={18} />
        </button>
        <button type="button" onClick={() => navigate(MOBILE_ROUTES.scan)}>
          ...
          <div>
            <strong>MF Scan</strong>
            <small>Capturar boleto, conta, QR ou documento</small>
          </div>
          <ChevronRight size={18} />
        </button>
        <button type="button" onClick={() => navigate(MOBILE_ROUTES.quick)}>
          ...
          <div>
            <strong>MF Quick</strong>
            <small>Lançamento rápido em poucos toques</small>
          </div>
          <ChevronRight size={18} />
        </button>
        <button type="button" onClick={openDesktopExperience}>
          <span className="mf-mobile-menu-icon">
            <ExternalLink size={19} />
          </span>
          <div>
            <strong>Abrir versão completa</strong>
            <small>
              Planejamento, análises, importações e ferramentas avançadas
            </small>
          </div>
          <ChevronRight size={18} />
        </button>
...
export default function MobileApp({ user }: { user: User }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const cycleHistoryStart = format(
      ...
      const [
        ...
        supabase
          ...
          .select(
            'id,user_id,account_id,category_id,amount,category,description,date,type,status,source,affects_balance',
          )
          .eq('user_id', user.id)
        ...
        supabase
          .from('mf_finance_ledger_entries')
          .select(
            'id,user_id,amount,category,description,date,type,status,affects_balance',
          )
          .eq('user_id', user.id)
          .gte('date', monthStart)
          .lte('date', monthEnd),
      ...
      const monthTransactions = (monthResult.data || []).map(
        normalizeTransaction,
      );
      const cycleTransactions = (cycleResult.data || []).map(
        normalizeTransaction,
      );
      const categorizationHistory = (learningResult.data || []).map(
        normalizeTransaction,
      );
      const derivedBalance = accounts.reduce(
      ...
      const settings = settingsResult.data
        ? ({
            ...settingsResult.data,
            current_balance: derivedBalance,
          } as UserSettings)
        : null;
      ...
      setData({
        ...
        cycleTransactions,
        categorizationHistory,
        pending: (pendingResult.data || []).map((row: any) => ({
          ...row,
          amount: Number(row.amount || 0),
        })) as PendingItem[],
        summary,
  ...
  if (loading)
    return (
      <div className="mf-mobile-loading">
        <Loader2 className="animate-spin" size={30} />
        <span>Carregando seu MF</span>
      </div>
    );
  if (error)
    return (
  ...
  if (page === 'quick')
    return (
      <MobileQuickAdd
        ...
        history={data.categorizationHistory}
        onSaved={refresh}
      />
    );
  if (page === 'scan')
  ...
  if (page === 'canSpend') {
    ...
    return (
      <div className="mf-mobile-loading">
        <strong>Simulação indisponível</strong>
        <span>
          Complete suas configurações financeiras para calcular o impacto de uma
          compra.
        </span>
        <button
          type="button"
  ...
  if (page === 'purchaseImpact')
    return <MobilePurchaseImpact cards={data.cards} summary={data.summary} />;
  if (page === 'pulse')
    return <MobilePulse summary={data.summary} commitments={data.pending} />;
  ...
  return (
    <MobileAppShell onQuickAction={() => navigate(MOBILE_ROUTES.quick)}>
      {content}
    </MobileAppShell>
  );
}
