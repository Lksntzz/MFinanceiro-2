  ...
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(
        request.error ||
          new Error(
            'Não foi possível abrir o armazenamento de compartilhamentos.',
          ),
      );
  });
}
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      transaction.onerror = () =>
        reject(
          transaction.error ||
            new Error('Não foi possível atualizar o compartilhamento.'),
        );
      transaction.onabort = () =>
        reject(
          transaction.error ||
...
export async function getMobileSharedPayload(id: string) {
  ...
  try {
    return await new Promise<MobileSharedPayload | null>((resolve, reject) => {
      ...
      request.onsuccess = () =>
        resolve((request.result as MobileSharedPayload | undefined) || null);
      request.onerror = () =>
        reject(
          request.error ||
            new Error('Não foi possível ler o conteúdo compartilhado.'),
        );
    });
  } finally {
    db.close();
...
export function unreviewedSharedFiles(
  files: MobileSharedFile[],
  reviewedIndex: number,
) {
  if (
    !Number.isInteger(reviewedIndex) ||
    reviewedIndex < 0 ||
    reviewedIndex >= files.length
  )
    return [...files];
  return files.filter((_, index) => index !== reviewedIndex);
}

export async function keepOnlyUnreviewedSharedFiles(
  payload: MobileSharedPayload,
  reviewedIndex: number,
) {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      transaction.onerror = () =>
        reject(
          transaction.error ||
            new Error('Não foi possível limpar o compartilhamento.'),
        );
      transaction.onabort = () =>
        reject(
          transaction.error ||
