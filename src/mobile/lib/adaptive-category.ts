...
function similarity(left: string, right: string) {
  ...
  const containment = overlap / Math.max(1, Math.min(leftSet.size, rightSet.size));
  const union = new Set([...leftSet, ...rightSet]).size;
  const jaccard = overlap / Math.max(1, union);
  return Math.min(1, containment * 0.72 + jaccard * 0.18 + 0.1);
}

function compatibleCategory(category: TransactionCategory, type: TransactionType) {
  if (!category.is_active) return false;
  if (type === 'transfer') return false;
  return category.category_type === 'both' || category.category_type === type;
}

function resolveHistoryCategory(item: Transaction, categories: TransactionCategory[]) {
  if (item.category_id) {
    const byId = categories.find((category) => category.id === item.category_id);
    if (byId) return byId;
  }
  const key = normalizeText(item.category || '');
  return categories.find((category) => normalizeText(category.name) === key) || null;
}
...
export function inferAdaptiveCategory(options: {
  ...
  const candidates = history
    .filter((item) => item.type === type && !EXCLUDED_STATUSES.has(String(item.status || 'paid')))
    .map((item) => {
      const category = resolveHistoryCategory(item, categories);
      const score = similarity(merchantText, item.description || item.source || '');
      return { item, category, score };
    })
    .filter((candidate) => candidate.category && compatibleCategory(candidate.category, type) && candidate.score >= 0.7) as Array<{
      item: Transaction;
  ...
  const grouped = new Map<string, { category: TransactionCategory; count: number; similarityTotal: number }>();
  for (const candidate of candidates) {
    const current = grouped.get(candidate.category.id) || { category: candidate.category, count: 0, similarityTotal: 0 };
    current.count += 1;
    current.similarityTotal += candidate.score;
    grouped.set(candidate.category.id, current);
  }

  const ranked = [...grouped.values()].sort((a, b) => b.count - a.count || b.similarityTotal - a.similarityTotal);
  const winner = ranked[0];
  if (!winner) return null;

  const consistency = winner.count / candidates.length;
  const averageSimilarity = winner.similarityTotal / winner.count;
  const high = winner.count >= 3 && consistency >= 0.8 && averageSimilarity >= 0.72;
  const medium = winner.count >= 2 && consistency >= 0.67 && averageSimilarity >= 0.7;
  if (!high && !medium) return null;

  const confidenceScore = Math.min(0.98, 0.42 + Math.min(0.24, winner.count * 0.06) + consistency * 0.2 + averageSimilarity * 0.12);
  return {
