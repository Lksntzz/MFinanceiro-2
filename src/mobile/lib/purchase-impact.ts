...
export function projectInvoiceForPurchase(
  purchaseDate: Date,
  closingDay: number,
  dueDay: number,
): InvoiceProjection {
  const normalizedClosingDay = Math.min(
    31,
    Math.max(1, Math.trunc(Number(closingDay) || 1)),
  );
  const normalizedDueDay = Math.min(
    31,
    Math.max(1, Math.trunc(Number(dueDay) || 1)),
  );
  const closeMonthOffset =
    purchaseDate.getDate() <= normalizedClosingDay ? 0 : 1;
