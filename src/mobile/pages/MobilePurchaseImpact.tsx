...
function impactTone(availableAfter: number, cycleFreeAfter: number | null) {
  if (availableAfter < 0 || (cycleFreeAfter !== null && cycleFreeAfter < 0))
    return 'danger';
  if (cycleFreeAfter !== null && cycleFreeAfter < 300) return 'warning';
  return 'success';
}

export default function MobilePurchaseImpact({
  cards,
  summary,
}: MobilePurchaseImpactProps) {
  ...
  const projection = useMemo(() => {
    ...
    const firstInvoice = projectInvoiceForPurchase(
      purchase,
    ...
    const usageAfter =
      Number(card.limit || 0) > 0
        ? (usedAfter / Number(card.limit || 0)) * 100
        : 0;
    ...
    if (summary?.nextPaydayDate) {
      ...
      impactsCurrentCycle =
        firstInvoice.dueDate.getTime() <= nextPayday.getTime();
    ...
    const invoices = Array.from(
      { length: Math.min(installmentCount, 6) },
      (_, index) => {
        const closeDate = addMonths(firstInvoice.closeDate, index);
        const dueMonthOffset =
          Number(card.due_day || 1) > Number(card.closing_day || 1) ? 0 : 1;
    ...
    <div className="mf-mobile-focus-page mf-purchase-impact">
      <header className="mf-mobile-focus-header">
        <button
          ...
          onClick={() => navigate(MOBILE_ROUTES.cards)}
      ...
      {!cards.length ? (
        <div className="mf-mobile-list-card">
          <div className="mf-mobile-empty">
            Cadastre um cartão para simular o impacto de uma compra.
          </div>
        </div>
      ) : (
        <div className="mf-purchase-impact__form">
          ...
          {projection ? (
            <section
              className="mf-purchase-impact__result"
              data-tone={projection.tone}
            >
              <div className="mf-purchase-impact__headline">
                <div>
                  <small>Primeira fatura</small>
                  <strong>
                    {format(projection.firstInvoice.dueDate, "MMMM 'de' yyyy", {
                      locale: ptBR,
                    })}
                  </strong>
                </div>
              ...
              <div className="mf-purchase-impact__metrics">
                <div>
                  <small>Parcela</small>
                  <b>{formatCurrency(projection.monthlyAmount)}</b>
                </div>
                <div>
                  <small>Limite depois</small>
                  <b>{formatCurrency(projection.availableAfter)}</b>
                </div>
                <div>
                  <small>Uso do limite</small>
                  <b>{projection.usageAfter.toFixed(0)}%</b>
                </div>
                <div>
                  <small>Total comprometido</small>
                  <b>{formatCurrency(parsedAmount)}</b>
                </div>
              </div>
              ...
              {summary ? (
                <div className="mf-purchase-impact__cycle">
                  ...
                  {projection.impactsCurrentCycle ? (
                    <div>
                      <strong>
                        Essa parcela entra antes do próximo recebimento.
                      </strong>
                      <span>
                        Valor livre: {formatCurrency(summary.projectedBalance)}{' '}
                        → {formatCurrency(projection.cycleFreeAfter || 0)}
                      </span>
                      <span>
                        Margem diária: {formatCurrency(summary.dailyLimit)} →{' '}
                        {formatCurrency(projection.dailyAfter || 0)}
                      </span>
                    </div>
                  ) : (
                    <div>
                      ...
                      <span>
                        ...
                        {format(projection.firstInvoice.dueDate, 'dd/MM/yyyy')}.
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="mf-purchase-impact__timeline">
                <div className="mf-purchase-impact__timeline-title">
                  <Layers3 size={17} />
                  <span>Próximas parcelas</span>
                </div>
                {projection.invoices.map((invoice) => (
                  <div
                    className="mf-purchase-impact__invoice"
                    key={invoice.index}
                  >
                    <span>
                      {invoice.index}/{installmentCount} •{' '}
                ...
                {installmentCount > 6 ? (
                  <small>
                    + {installmentCount - 6} parcelas futuras no mesmo valor
                    estimado.
                  </small>
                ) : null}
              </div>

              <p className="mf-purchase-impact__note">
                Simulação informativa. O fechamento real pode variar por
                emissor, horário de processamento, feriados e ajustes da
                operadora.
              </p>
            </section>
          ) : (
            <div className="mf-mobile-scan-safety">
              <Gauge size={18} />
              <span>
                Informe o valor para ver em qual fatura a compra cai e como ela
                afeta seu limite e ciclo.
              </span>
            </div>
          )}
        </div>
