...
export default function MobileVoice({ userId }: { userId: string }) {
  ...
  const selectedCategory =
    compatibleCategories.find((category) => category.id === categoryId) || null;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const [accountsResult, categoriesResult, historyResult] =
          await Promise.all([
            supabase
        ...
        setAccountId(
          nextAccounts.find((account) => account.is_default)?.id ||
            nextAccounts[0]?.id ||
            '',
        );
      } catch (loadError: any) {
        if (active)
          setError(
            loadError?.message || 'Não foi possível preparar o MF Voice.',
          );
      } finally {
  ...
  function applyTranscript(text: string) {
    ...
    const learnedCategoryId =
      adaptive?.confidence === 'high' ? adaptive.categoryId : parsed.categoryId;
    setType(parsed.type);
    ...
    setInterpretation([
      ...
      ...parsed.warnings.filter(
        (warning) =>
          !(adaptive?.confidence === 'high' && warning.includes('categoria')),
      ),
    ]);
  }
  ...
  function startListening() {
    ...
    if (!Constructor) {
      setError(
        'Este navegador não oferece reconhecimento de voz direto. Use o ditado do teclado ou digite a frase abaixo.',
      );
      return;
    }
    ...
    recognition.onerror = (event) => {
      ...
      if (reason === 'not-allowed' || reason === 'service-not-allowed') {
        setError(
          'Permissão de microfone/voz não concedida. Você ainda pode usar o ditado do teclado ou digitar a frase.',
        );
      } else if (reason === 'no-speech') {
        setError(
          'Não ouvi uma frase completa. Tente novamente falando perto do microfone.',
        );
      } else {
        setError(
          'O reconhecimento de voz não concluiu. Você pode tentar novamente ou editar a frase manualmente.',
        );
      }
    };
    ...
    try {
      const { error: rpcError } = await supabase.rpc(
        'mf_create_finance_entry_v3',
        {
          ...
          p_due_date: null,
          p_notes: transcript.trim()
            ? `Frase revisada no MF Voice: ${transcript.trim().slice(0, 240)}`
            : 'Lançamento revisado no MF Voice',
      ...
      setError(
        saveError?.message || 'Não foi possível salvar o lançamento por voz.',
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="mf-mobile-focus-page mf-mobile-voice-page">
      <header className="mf-mobile-focus-header">
        <button
          ...
          onClick={() => navigate(MOBILE_ROUTES.quick)}
          aria-label="Voltar para o MF Quick"
        >
          <ArrowLeft size={21} />
        </button>
        <div>
        ...
        <div className="mf-mobile-voice-header-mark">
          <Sparkles size={19} />
        </div>
      </header>
      ...
      <section className="mf-mobile-voice-capture">
        ...
        <strong>
          {listening
            ? 'Ouvindo…'
            : speechSupported
              ? 'Toque e fale uma frase'
              : 'Use o ditado do teclado'}
        </strong>
        <p>
          Exemplo: “Gastei 48 reais de gasolina.” O MF nunca salva apenas pela
          fala; a revisão abaixo é obrigatória.
        </p>
      </section>
      ...
      <form className="mf-mobile-voice-form" onSubmit={saveEntry}>
        ...
        <div
          ...
          role="group"
          aria-label="Tipo do lançamento por voz"
        >
          <button
            type="button"
        ...
        <div className="mf-mobile-scan-safety">
          <ShieldCheck size={18} />
          <span>
            O MF não armazena áudio nesta versão. O serviço de reconhecimento
            pode ser fornecido pelo próprio navegador/sistema.
          </span>
        </div>
        ...
        {saved ? (
          <div className="mf-mobile-feedback success">
            <Check size={16} />
            Lançamento salvo pelo MF Voice.
          </div>
        ) : null}
