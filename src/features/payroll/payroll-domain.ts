...
import type { PayrollItem, PayrollItemCategory, PayrollItemKind } from '../../lib/payroll-pdf-parser';
...
export function categoryFromDescription(description: string): PayrollItemCategory {
...
export function normalizeSettings(row: unknown): SettingsRow {
  ...
  return {
    ...
    payday_1_percentage: numberValue(value.payday_1_percentage as number ?? 50),
    payday_2_percentage: numberValue(value.payday_2_percentage as number ?? 50),
  } as SettingsRow;
}
...
export function sanitizeItems(raw: unknown, gross = 0): PayrollItem[] {
  ...
  return parsed.map((rawItem, index) => {
    const value = rawItem && typeof rawItem === 'object' ? rawItem as Record<string, unknown> : {};
    ...
    const kind: PayrollItemKind = ['earning', 'deduction', 'benefit'].includes(String(value.kind))
    ...
    return {
      ...
      amount,
      percentage: gross > 0 ? roundMoney((amount / gross) * 100) : numberValue(value.percentage as number),
...
export function legacyItems(row: PayrollRow, idFactory = createId): PayrollItem[] {
  const result: PayrollItem[] = [];
  const add = (description: string, kind: PayrollItemKind, category: PayrollItemCategory, amount: number) => {
    if (amount <= 0) return;
    result.push({
      id: idFactory('legacy'), description, kind, category, amount,
      percentage: percentageNumber(amount, row.gross_salary), source: 'manual', confidence: 1,
    });
  };
...
export function emptyForm(settings: SettingsRow | null, competence = monthKey()): EditorForm {
...
export function derivePayrollSummary(form: EditorForm, items: PayrollItem[]) {
  ...
  const normalizedItems = items.map((item) => ({ ...item, percentage: percentageNumber(item.amount, gross) }));
  ...
  const firstPayment = form.paydayCycle === 'biweekly' ? roundMoney(cycleBase * firstPercentage / 100) : cycleBase;
  const secondPayment = form.paydayCycle === 'biweekly' ? roundMoney(cycleBase - firstPayment) : 0;
  const estimatedTaxes = calculatePayrollFromGross(gross, dateForMonth(form.competence));

  return {
    gross, normalizedItems, deductionItems, benefitItems, earningItems, actualInss, actualIrrf,
    totalDeductions, otherDeductions, benefits, actualNet, cycleBase, firstPercentage,
    secondPercentage, firstPayment, secondPayment, estimatedTaxes,
  };
}

export function buildPayrollSaveCommand(form: EditorForm, items: PayrollItem[], sourceFileName: string | null) {
  ...
  if (summary.totalDeductions > summary.gross) throw new Error('Os descontos não podem superar o total bruto.');
  if (summary.normalizedItems.some((item) => !item.description.trim())) throw new Error('Preencha ou exclua as rubricas sem descrição.');
  ...
  if (form.paydayCycle === 'biweekly') {
    ...
    if (Math.abs(summary.firstPercentage + summary.secondPercentage - 100) > 0.01) throw new Error('Os percentuais precisam somar 100%.');
  }

  const sourceKind: 'manual' | 'pdf' | 'mixed' = sourceFileName
    ? summary.normalizedItems.some((item) => item.source === 'manual') ? 'mixed' : 'pdf'
    : 'manual';
  ...
  return {
    ...
    params: {
      ...
      p_payday_1_percentage: form.paydayCycle === 'biweekly' ? summary.firstPercentage : 100,
      p_payday_2_percentage: form.paydayCycle === 'biweekly' ? summary.secondPercentage : 0,
