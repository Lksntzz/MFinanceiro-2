...
async function invokeBetaMarketProxy(
  action: 'quote' | 'income',
  symbol: string,
...
async function fetchPublicSandbox(url: string, errorMessage: string) {
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw new Error(
        'A fonte completa de mercado ainda não está conectada neste beta.',
      );
    }
...
function stockQuoteFromPayload(
  payload: any,
  symbol: string,
  ...
  if (price <= 0)
    throw new Error('A fonte não retornou uma cotação válida para este ativo.');
  return {
    ...
    changePercent: finite(
      item.regularMarketChangePercent ?? item.changePercent,
    ),
...
function cryptoQuoteFromPayload(
  payload: any,
  symbol: string,
  source: InvestmentBetaMarketSource,
): InvestmentBetaQuote {
  ...
  if (!item) throw new Error('Ativo não encontrado na fonte de mercado.');
  const price = finite(
    item.regularMarketPrice ?? item.price ?? item.marketPrice,
  );
  if (price <= 0)
    throw new Error('A fonte não retornou uma cotação válida para este ativo.');
  return {
    ...
    price,
    change: finite(
      item.regularMarketChange ?? item.change ?? item.priceChange24h,
    ),
    changePercent: finite(
...
export async function fetchBetaMarketQuote(
  symbolInput: string,
  assetClass: InvestmentAssetClass,
): Promise<InvestmentBetaQuote> {
  ...
  if (
    ...
    throw new Error(
      'Esta classe ainda não está conectada à fonte de mercado no beta.',
    );
  }

  const proxiedPayload = await invokeBetaMarketProxy(
    'quote',
    symbol,
    assetClass,
  );
...
function incomeEventsFromPayload(
  ...
  return extractDividendRows(payload)
    .map((row: any, index: number) => {
      ...
      const recordDate = normalizeEventDate(
        row?.lastDatePrior ?? row?.recordDate ?? row?.exDate,
      );
      const paymentDate = normalizeEventDate(row?.paymentDate ?? row?.payDate);
      const approvedOn = normalizeEventDate(
        row?.approvedOn ?? row?.declaredDate,
      );
      const label = String(row?.label || row?.type || 'PROVENTO')
        .trim()
        .toUpperCase();
      return {
        id: [
    ...
    .filter(
      (event: InvestmentBetaIncomeEvent) => event.symbol && event.rate > 0,
    )
    .sort((a: InvestmentBetaIncomeEvent, b: InvestmentBetaIncomeEvent) =>
      String(b.paymentDate || b.recordDate || '').localeCompare(
        String(a.paymentDate || a.recordDate || ''),
      ),
    );
}
...
export async function fetchBetaIncomeEvents(
  ...
  if (!['stock', 'fii', 'etf', 'bdr'].includes(assetClass)) {
    throw new Error(
      'Proventos automáticos estão conectados apenas a ações, FIIs, ETFs e BDRs neste beta.',
    );
  }
  ...
  if (proxiedPayload)
    return incomeEventsFromPayload(
      proxiedPayload,
      symbol,
      incomeClass,
      'brapi-backend',
  ...
  const payload = await fetchPublicSandbox(
    url,
    'Não foi possível consultar os proventos deste ativo agora.',
  );
