...
export function parseOfxTransactions(
  ...
  const ledgerBalanceMatch = content.match(
    /<LEDGERBAL>[\s\S]*?<BALAMT>([\d.,-]+)/i,
  );
  const finalLedgerBalance = ledgerBalanceMatch
    ? parseStatementAmount(ledgerBalanceMatch[1])
    : undefined;

  return blocks.map((block, index) => {
    const fitId = readOfxTag(block, 'FITID');
    const transactionType = normalizeStatementHeader(
      readOfxTag(block, 'TRNTYPE'),
    );
    ...
    const rawDate = postedAt
      ? `${postedAt.slice(6, 8)}/${postedAt.slice(4, 6)}/${postedAt.slice(0, 4)}`
      : '';
    const type: ImportedTransaction['type'] =
      /(credit|dep|income)/.test(transactionType) || signedAmount > 0
        ? 'income'
        : 'expense';
    const status: ImportedTransaction['status'] =
      signedAmount !== 0 ? 'ready' : 'error';
    ...
    return {
      ...
      running_balance:
        index === blocks.length - 1 ? finalLedgerBalance : undefined,
    };
  });
}
