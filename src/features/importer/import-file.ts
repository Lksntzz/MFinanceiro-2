...
export function analyzeDelimitedContent(content: string): DelimitedAnalysis {
  ...
  const dateRegex =
    /\b\d{2}[./-]\d{2}(?:[./-]\d{2,4})?\b|\b\d{4}-\d{2}-\d{2}\b/;
  const amountRegex = /(?:R\$\s*)?[+-]?\s*\d[\d.\s]*[,.]\d{2}-?/;
  const headerKeywords =
    /(data|date|descricao|hist[óo]rico|description|valor|amount|debito|credito|saldo|lan[çc]amento)/i;

  return {
    nonEmptyLines: lines.length,
    hasHeaderKeywords: lines.some((line) => headerKeywords.test(line)),
    hasTransactionSignals: lines.some(
      (line) => dateRegex.test(line) && amountRegex.test(line),
    ),
  };
}
...
export function inferEmptyResultReason(params: {
  ...
  const normalizedFileName = String(fileName || '')
    ...
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '');
  const isMfinanceiroExport = normalizedFileName.includes(
    'mfinanceirorelatoriotransacoes',
  );
  ...
  if (format === 'csv') {
    if (
      isMfinanceiroExport &&
      (!csvAnalysis || csvAnalysis.nonEmptyLines === 0)
    ) {
      return 'arquivo do MFinanceiro sem transacoes para importar';
    }
    if (!csvAnalysis || csvAnalysis.nonEmptyLines === 0)
      return 'planilha vazia';
    if (csvAnalysis.nonEmptyLines <= 1 && csvAnalysis.hasHeaderKeywords)
      return 'arquivo sem movimentacoes';
    if (!csvAnalysis.hasTransactionSignals)
      return 'arquivo nao e um extrato bancario';
  ...
  if (format === 'xls' || format === 'xlsx') {
    if (
      isMfinanceiroExport &&
      (!sheetAnalysis || sheetAnalysis.nonEmptyRows === 0)
    ) {
      return 'arquivo do MFinanceiro sem transacoes para importar';
    }
    if (!sheetAnalysis || sheetAnalysis.nonEmptyRows === 0)
      return 'planilha vazia';
    if (sheetAnalysis.nonEmptyRows <= 1 && sheetAnalysis.hasHeaderKeywords)
      return 'arquivo sem movimentacoes';
    if (!sheetAnalysis.hasTransactionSignals)
      return 'arquivo nao e um extrato bancario';
    return 'nenhum registro de transacao encontrado';
  }
  ...
  if (format === 'ofx') {
    ...
    if (!/<OFX|<BANKTRANLIST|<STMTTRN>/i.test(content))
      return 'arquivo nao e um extrato bancario';
...
export function getInvalidLineReasons(items: ImportedTransaction[]): string[] {
  ...
  for (const item of items) {
    ...
    if (!item.description || item.description === 'Sem descricao') {
      ...
      reasons.push(
        `Linha rejeitada: nao reconhecida como transacao (${item.id})`,
      );
    }
  }
  return reasons;
...
export function ensureMinimumTransactionFields(
...
export function detectFileFormat(
  ...
  if (ext === 'xls') {
    return {
      ...
      supported: false,
      reason:
        'O formato XLS legado não é processado com segurança. Converta o arquivo para XLSX, CSV ou OFX.',
    };
  }
  if (
