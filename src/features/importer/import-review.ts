...
export function calculateImportBalanceValidation(
  ...
  const withRunningBalance = items
    ...
    .filter(
      ({ item }) =>
        ...
        Number.isFinite(item.running_balance),
    );

  if (withRunningBalance.length) {
    const ascending = [...withRunningBalance].sort((a, b) => {
      const byDate =
        importedTimestamp(a.item.date) - importedTimestamp(b.item.date);
      return byDate || a.index - b.index;
    });
    const descending = [...withRunningBalance].sort((a, b) => {
      const byDate =
        importedTimestamp(b.item.date) - importedTimestamp(a.item.date);
    ...
    const openingBalance =
      Number(first.running_balance ?? 0) - signedAmount(first);
  ...
  if (
    !items.length ||
    documentStatementBalance === undefined ||
    !Number.isFinite(documentStatementBalance)
  ) {
    return null;
  }

  const statementFinal = Number(documentStatementBalance);
  const fullStatementNet = items.reduce(
    (sum, item) => sum + signedAmount(item),
    0,
  );
  const selectedNet = items
    .filter((item) => item.status === 'ready')
...
export function toggleImportedTransaction(
  items: ImportedTransaction[],
  id: string,
): ImportedTransaction[] {
  return items.map((item) => {
    ...
    if (
      item.status === 'error' &&
      (item.amount <= 0 || item.description === 'Sem descricao')
    )
      return item;
    ...
    return {
      ...
      review_status: item.extraction_item_id
        ? selected
          ? 'accepted'
          : 'pending'
        : item.review_status,
...
export function removeImportedTransaction(
  items: ImportedTransaction[],
  id: string,
) {
