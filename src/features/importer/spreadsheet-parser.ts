...
import { standardizeBankSheet, type SpreadsheetRow } from '../../lib/bank-excel-normalizer';
...
export async function analyzeSpreadsheetContent(file: File): Promise<SpreadsheetAnalysis> {
  ...
  if (!sheets.length) return { csv: '', nonEmptyRows: 0, hasHeaderKeywords: false, hasTransactionSignals: false };
  ...
  for (const { sheet: sheetName, data: rows } of sheets) {
    ...
    const fallbackCsv = normalizedRows.map((row) => row.map((cell) => {
