...
const PROFILES: CsvProfile[] = [
  {
    id: 'nubank',
    label: 'Nubank',
    hints: ['nubank', 'nupagamentos', 'identificador'],
    date: ['data', 'date'],
    description: [
      'descricao',
  ...
  },
  {
    id: 'inter',
    label: 'Banco Inter',
    hints: ['bancointer', 'intermedium', 'contadigitalinter'],
...
function parseMoney(rawValue: string | undefined): number {
  ...
  const negative =
    /^\(.*\)$/.test(raw) || /-$/.test(raw) || normalizedText.endsWith('debito');
  let cleaned = raw
    .replace(/R\$/gi, '')
    .replace(/\s+/g, '')
    .replace(/[A-Za-z]/g, '')
...
function scoreHeader(
  ...
  const context = normalize(
    lines.slice(Math.max(0, rowIndex - 10), rowIndex + 3).join(' '),
  );
  let score =
    20 +
    (credit >= 0 ? 4 : 0) +
...
export function standardizeBankCsv(
  content: string,
  fileName: string,
): string | null {
  if (normalize(fileName).includes('mfinanceirorelatoriotransacoes'))
    return null;
  const lines = content
    .split(/\r?\n/)
  ...
  for (let index = header.headerIndex + 1; index < lines.length; index += 1) {
    ...
    const description = String(row[header.description] ?? '')
      .replace(/\s+/g, ' ')
      .trim();
