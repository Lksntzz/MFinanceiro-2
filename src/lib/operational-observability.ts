...
const BLOCKED_KEYS = /(amount|balance|card|account|email|name|description|document|file|token|secret|password|payload|statement|merchant|salary|income|expense|raw|preview|text)/i;
...
function currentBuildId() {
  return typeof __MF_BUILD_ID__ !== 'undefined' ? String(__MF_BUILD_ID__) : 'unknown';
}

function environment(): 'production' | 'preview' | 'development' | 'unknown' {
  const explicit = String(import.meta.env.VITE_MF_DIAG_ENVIRONMENT || '').trim().toLowerCase();
...
async function sendEvent(payload: Record<string, unknown>) {
  const ingestUrl = String(import.meta.env.VITE_MF_ADMIN_INGEST_URL || '').trim();
  ...
  try {
    ...
    try {
      ...
      if (!response.ok && response.status !== 429) throw new Error('diagnostic_ingest_failed');
...
export function reportOperationalEvent(
  ...
  const module = cleanToken(options.module || descriptor?.module || normalizedArea, 80);
  const operation = cleanToken(options.operation || descriptor?.operation || event, 100);
  const errorCode = cleanErrorCode(options.errorCode || descriptor?.errorCode || event.replace(/[.-]/g, '_'));
...
export function installGlobalOperationalObservers() {
  ...
  const onRejection = () => {
    reportOperationalEvent('runtime.unhandled_rejection', 'web-runtime', 'error');
  };
