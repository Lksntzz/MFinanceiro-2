...
function captureDailyBase(canvas: HTMLCanvasElement, chart: Chart): BaseSeries {
  ...
  const datasets = chart.data.datasets.map((dataset) =>
    toNumberArray((dataset.data || []) as unknown[]),
  );

  const existing = baseByCanvas.get(canvas);

  // The React component supplies 30 daily points. Capture them whenever they
  // are available so realtime refreshes replace stale cached values.
...
function aggregateWeeks(base: BaseSeries): BaseSeries {
  ...
  return {
    ...
    datasets: base.datasets.map((series) =>
      groups.map(({ start, end: groupEnd }) =>
...
function applyFilter(button: HTMLButtonElement, filter: RhythmFilter) {
  ...
  const next = filter === 'week'
    ? aggregateWeeks(base)
    : filter === 'month'
      ? aggregateMonths(base)
      : base;

  chart.data.labels = [...next.labels];
  chart.data.datasets.forEach((dataset, index) => {
    dataset.data = [...(next.datasets[index] || [])];
  });
  chart.update();
}
...
export function installRhythmChartFilter() {
  document.addEventListener('click', (event) => {
    ...
    // React first updates the selected button. Apply the chart aggregation just
    // after that render, and repeat once to survive Chart.js/react-chartjs updates.
