...
const PLUGGY_CONNECT_SCRIPT_URL = 'https://cdn.pluggy.ai/pluggy-connect/v2.13.0/pluggy-connect.js';
...
function resolveConstructor(): PluggyConnectConstructor | null {
  return typeof window !== 'undefined' && typeof window.PluggyConnect === 'function'
...
export async function loadPluggyConnect(): Promise<PluggyConnectConstructor> {
  ...
  loader = new Promise<PluggyConnectConstructor>((resolve, reject) => {
    ...
    const finish = () => {
      ...
      else reject(new Error('O Pluggy Connect foi carregado, mas o SDK não ficou disponível.'));
    };
    ...
    script.addEventListener('error', () => reject(new Error('Não foi possível carregar o Pluggy Connect.')), { once: true });
...
export async function createPluggyConnect(options: PluggyConnectOptions): Promise<PluggyConnectInstance> {
  const Constructor = await loadPluggyConnect();
  return new Constructor(options);
}

export function extractPluggyItem(payload: PluggyConnectSuccess | PluggyConnectError | null | undefined): PluggyItem | null {
