...
export interface UserPreferences { version: 1; homeWidgets: HomeWidgetId[]; notifications: Record<NotificationPreferenceKey, boolean>; toursAutoStart: boolean; privacyDefault: boolean; reducedMotion: boolean; highContrast: boolean; compactHome: boolean; }
...
export const DEFAULT_USER_PREFERENCES: UserPreferences = {
  ...
  notifications: { commitments: true, cards: true, quality: true, release: true },
  toursAutoStart: true,
...
export function userPreferencesKey(userId: string) { return `mf-preferences:v1:${userId}`; }
export function allToursSkipKey(userId: string) { return `mf-tour:all-skipped:${userId}`; }
function sanitizeWidgets(value: unknown): HomeWidgetId[] {
  ...
  const unique = [...new Set(widgets)].slice(0, 5);
  return unique.length >= 3 ? unique : DEFAULT_USER_PREFERENCES.homeWidgets;
}
export function loadUserPreferences(userId: string): UserPreferences {
  ...
  try {
    ...
    return { ...DEFAULT_USER_PREFERENCES, ...parsed, version: 1, homeWidgets: sanitizeWidgets(parsed.homeWidgets), notifications: { ...DEFAULT_USER_PREFERENCES.notifications, ...(parsed.notifications || {}) }, toursAutoStart: globalToursSkipped ? false : parsed.toursAutoStart ?? DEFAULT_USER_PREFERENCES.toursAutoStart };
  } catch { return DEFAULT_USER_PREFERENCES; }
}
export function saveUserPreferences(userId: string, preferences: UserPreferences) {
  if (typeof window === 'undefined' || !userId) return;
  const normalized: UserPreferences = { ...DEFAULT_USER_PREFERENCES, ...preferences, version: 1, homeWidgets: sanitizeWidgets(preferences.homeWidgets), notifications: { ...DEFAULT_USER_PREFERENCES.notifications, ...preferences.notifications } };
  try {
    window.localStorage.setItem(userPreferencesKey(userId), JSON.stringify(normalized));
    if (normalized.toursAutoStart) window.localStorage.removeItem(allToursSkipKey(userId)); else window.localStorage.setItem(allToursSkipKey(userId), 'skipped');
  } catch { /* Preferences must never block the financial experience. */ }
...
export function updateUserPreferences(userId: string, patch: Partial<UserPreferences>) {
  ...
  saveUserPreferences(userId, { ...current, ...patch, notifications: patch.notifications ? { ...current.notifications, ...patch.notifications } : current.notifications });
}
export function resetUserPreferences(userId: string) { saveUserPreferences(userId, DEFAULT_USER_PREFERENCES); }
export function subscribeUserPreferences(userId: string, listener: (preferences: UserPreferences) => void) {
  if (typeof window === 'undefined') return () => undefined;
  const handle = (event: Event) => { const detail = (event as CustomEvent<{ userId?: string; preferences?: UserPreferences }>).detail; if (detail?.userId === userId && detail.preferences) listener(detail.preferences); };
  const storage = (event: StorageEvent) => { if (event.key === userPreferencesKey(userId) || event.key === allToursSkipKey(userId)) listener(loadUserPreferences(userId)); };
  window.addEventListener(EVENT_NAME, handle); window.addEventListener('storage', storage);
  return () => { window.removeEventListener(EVENT_NAME, handle); window.removeEventListener('storage', storage); };
}
