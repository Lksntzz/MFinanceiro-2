...
type StoredUserPreferences = Omit<Partial<UserPreferences>, 'version'> & {
  version?: number;
};
...
export const HOME_WIDGET_OPTIONS: Array<{
  id: HomeWidgetId;
  label: string;
  description: string;
}> = [
  {
    ...
    description: 'Saldo, limite diário, ciclo e gasto de hoje.',
  },
  {
    id: 'alerts',
    label: 'Atenção e leitura',
    description: 'Alertas do ciclo e leitura financeira.',
  },
  {
    id: 'quality',
    label: 'Qualidade dos dados',
    description: 'Pendências que podem reduzir a confiança das análises.',
  },
  {
    id: 'balance_chart',
    label: 'Evolução do saldo',
    description: 'Gráfico da evolução histórica do saldo.',
  },
  {
    id: 'rhythm_chart',
    label: 'Ritmo de gastos',
    description: 'Entradas e saídas por dia, semana ou mês.',
  },
  {
    id: 'categories',
    label: 'Categorias principais',
    description: 'Categorias com maior peso nas despesas.',
  },
  {
    id: 'recent',
  ...
  },
  {
    ...
    description: 'Limite utilizado e restante disponível.',
  },
];
...
function sanitizeWidgets(value: unknown): HomeWidgetId[] {
  ...
  const widgets = [
    ...new Set(
      value.filter(
        (item): item is HomeWidgetId =>
          typeof item === 'string' && valid.has(item as HomeWidgetId),
      ),
    ),
  ].slice(0, MAX_HOME_WIDGETS);
  return widgets.length >= MIN_HOME_WIDGETS
    ? widgets
    : DEFAULT_USER_PREFERENCES.homeWidgets;
}
...
export function loadUserPreferences(userId: string): UserPreferences {
  ...
  try {
    const parsed = JSON.parse(
      window.localStorage.getItem(userPreferencesKey(userId)) || '{}',
    ) as StoredUserPreferences;
    const globalToursSkipped =
      window.localStorage.getItem(allToursSkipKey(userId)) === 'skipped';
    const homeWidgets =
      parsed.version === 2
        ? sanitizeWidgets(parsed.homeWidgets)
        : DEFAULT_USER_PREFERENCES.homeWidgets;
    return {
      ...
      toursAutoStart: globalToursSkipped
        ? false
        : (parsed.toursAutoStart ?? DEFAULT_USER_PREFERENCES.toursAutoStart),
    };
  } catch {
...
export function saveUserPreferences(
  ...
  try {
    ...
    if (normalized.toursAutoStart)
      window.localStorage.removeItem(allToursSkipKey(userId));
...
export function subscribeUserPreferences(
  ...
  const handle = (event: Event) => {
    const detail = (
      event as CustomEvent<{ userId?: string; preferences?: UserPreferences }>
    ).detail;
    if (detail?.userId === userId && detail.preferences)
      listener(detail.preferences);
  };
  const storage = (event: StorageEvent) => {
    if (
      event.key === userPreferencesKey(userId) ||
      event.key === allToursSkipKey(userId)
    )
      listener(loadUserPreferences(userId));
  };
