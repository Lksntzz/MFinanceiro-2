...
export function assessDataQuality({
  ...
  if (!activeAccounts.length) {
    issues.push({
      id: 'no-account',
      severity: 'high',
      title: 'Nenhuma conta financeira ativa',
      description:
        'Sem uma conta ativa, saldo e disponibilidade perdem confiabilidade.',
      actionLabel: 'Configurar conta',
      actionPath: '/app/planejamento/contas',
    });
  } else if (!activeAccounts.some((account) => account.is_default)) {
    issues.push({
      id: 'no-default-account',
      severity: 'medium',
      title: 'Conta principal não definida',
      description:
        'Defina qual conta representa o ponto de entrada padrão dos lançamentos.',
      actionLabel: 'Definir principal',
      actionPath: '/app/planejamento/contas',
    });
  }
  ...
  if (!activeCategories.length) {
    issues.push({
      ...
      description:
        'Categorias melhoram orçamento, busca e interpretação dos gastos.',
      actionLabel: 'Configurar categorias',
      actionPath: '/app/planejamento/categorias',
    });
  }
  ...
  if (recent.length >= 8) {
    ...
    if (genericCount / recent.length >= 0.25) {
      issues.push({
        ...
        severity: 'medium',
        title: 'Muitos lançamentos genéricos',
        description: `${genericCount} dos ${recent.length} lançamentos recentes estão em categorias genéricas.`,
        actionLabel: 'Organizar genéricos',
        actionPath: '/app/movimentacoes?assist=categorias',
      });
    }
  }
  ...
  if (duplicateGroups > 0) {
    issues.push({
      ...
      actionLabel: 'Conferir histórico',
      actionPath: '/app/movimentacoes',
    });
  }
  ...
  const incompleteCards = cards.filter(
    (card) =>
      ...
      Number((card as any).due_day || 0) <= 0 ||
      Number((card as any).closing_day || 0) <= 0,
  );
  if (incompleteCards.length) {
    issues.push({
      id: 'card-dates',
      severity: 'low',
      title: 'Cartão com configuração incompleta',
      description:
        'Limite, fechamento e vencimento completos melhoram Agenda e projeções.',
      actionLabel: 'Revisar cartões',
      actionPath: '/app/planejamento/cartoes',
    });
  }

  const invalidBills = fixedBills.filter(
    (bill) =>
      Number(bill.amount || 0) <= 0 ||
      Number((bill as any).due_day || 0) < 1 ||
      Number((bill as any).due_day || 0) > 31,
  );
  if (invalidBills.length) {
    issues.push({
      id: 'invalid-recurrence',
      severity: 'low',
      title: 'Recorrência precisa de revisão',
      description: 'Há compromisso sem valor ou dia de vencimento válido.',
      actionLabel: 'Revisar recorrências',
      actionPath: '/app/agenda/recorrencias',
    });
  }

  const rank: Record<DataQualitySeverity, number> = {
    high: 0,
    medium: 1,
    low: 2,
  };
