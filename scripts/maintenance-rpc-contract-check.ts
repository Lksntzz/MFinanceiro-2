...
assert.match(
  edgeFunction,
  /safeEqual\(suppliedControlSecret, expectedControlSecret\)/,
);
assert.match(edgeFunction, /['"]mobile['"],\s*['"]desktop['"],\s*['"]ios['"]/);
assert.match(edgeFunction, /body\.targets/);
assert.ok(
  !edgeFunction.includes('mf_set_maintenance_scope') &&
    !edgeFunction.includes('mf_set_maintenance_mode'),
  'Server control endpoint must not depend on legacy browser-admin RPCs',
);

assert.ok(
  !dashboard.includes('AdminMaintenanceControl'),
  'Financeiro desktop must not render maintenance administration',
);
assert.ok(
  !mobileProfile.includes('mf_set_maintenance_scope'),
  'Financeiro mobile must not mutate maintenance state',
);
assert.ok(
  !mobileProfile.includes('Manutenção'),
  'Financeiro mobile profile must not expose a maintenance administration card',
);
assert.ok(
  !app.includes('isMaintenanceBypass='),
  'Financeiro must not pass maintenance bypass into the product shell',
);
assert.match(
  app,
...
console.log(
  'Maintenance control boundary: one Admin control plane targets desktop, mobile/Android and iOS.',
);
