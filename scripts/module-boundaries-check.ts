...
import {
  ...
  parseStatementDate,
} from '../src/features/importer/statement-parser-utils';
import {
...
// Dashboard: import payload keeps only valid selections and normalizes the RPC summary.
{
  const command = buildStatementImportCommand(
    ...
    980,
    { accountId: 'account-1', fileName: 'extrato.csv' },
  );
  assert.equal(command.balanceMode, 'statement');
  assert.equal(
    command.params.p_entries[0].external_id,
    'statement:Banco Teste:bank-1',
  );
  ...
  assert.throws(
    () =>
      buildStatementImportCommand(
        [imported({ status: 'error', amount: 0 })],
        undefined,
        { accountId: 'a' },
      ),
...
// Dashboard: notification rules and local dismissed-alert retention are deterministic.
{
  ...
  assert.equal(
    sanitizeDismissedAlerts([
      ...Array.from({ length: 260 }, (_, index) => `id-${index}`),
    ]).length,
    250,
  );
  ...
  assert.deepEqual(
    sortTransactionsByDateDesc(transactions).map((item) => item.id),
    ['new', 'old'],
  );
}
...
{
  assert.equal(
    detectFileFormat({ name: 'conta.XLSX', type: '' }).format,
    'xlsx',
  );
  assert.equal(
    detectFileFormat({ name: 'foto.png', type: 'image/png' }).format,
    'image',
  );
  assert.equal(
    detectFileFormat({ name: 'arquivo.zip', type: 'application/zip' })
      .supported,
    false,
  );
  ...
  const analysis = analyzeDelimitedContent(
  ...
  assert.equal(
    toggleImportedTransaction(balancedRows, 'one')[0].status,
    'pending',
  );
  assert.equal(
    updateImportedTransaction(balancedRows, 'one', { description: '' })[0]
      .status,
    'error',
  );
  assert.equal(
    removeImportedTransaction(
      [imported({ extraction_item_id: 'ocr-1' })],
      'import-1',
    ).rejectedExtractionItemId,
    'ocr-1',
  );
  assert.equal(
    normalizeImportedTransactions([imported({ amount: -20 })])[0].amount,
    20,
  );
}
...
{
  assert.equal(
    categoryFromDescription('Desconto assistência médica'),
    'health',
  );
  ...
  const settings = normalizeSettings({
    user_id: 'u',
    gross_salary: '5000',
    payday_cycle: 'biweekly',
    payday_1_percentage: '60',
  });
  ...
  const form = {
    ...emptyForm(settings, '2026-08'),
    grossSalary: '5000',
    payday1Percentage: '60',
    payday2Percentage: '40',
  };
  const items = [
  ...
  assert.throws(
    () =>
      buildPayrollSaveCommand(
        { ...form, grossSalary: '100' },
        [payrollItem({ amount: 200 })],
        null,
      ),
