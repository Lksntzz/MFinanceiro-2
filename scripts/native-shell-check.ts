...
expectContains(androidManifest, 'android:scheme="mfinanceiro"', 'Android deep link');
expectContains(androidManifest, 'android.intent.action.SEND', 'Android share target');
expectContains(androidManifest, 'android:mimeType="application/pdf"', 'Android PDF share target');
expectContains(androidManifest, 'android:mimeType="image/*"', 'Android image share target');
expectContains(androidManifest, 'android:mimeType="text/plain"', 'Android text share target');
...
assert.equal(
  occurrenceCount(androidActivity, 'NativeShareStore.capture(this, getIntent())'),
  0,
  'Android onCreate must not duplicate the initial share capture',
);
expectContains(androidActivity, 'registerPlugin(NativeShareReceiverPlugin.class);', 'Android custom plugin registration');

// Native file receiver: private cache, supported document types and the same 20 MB safety cap as MF Share.
expectContains(androidShareStore, '20L * 1024L * 1024L', 'Android native share size cap');
expectContains(androidShareStore, 'application/pdf', 'Android native PDF support');
...
expectContains(androidSharePlugin, '@CapacitorPlugin(name = "NativeShareReceiver")', 'Android Capacitor share plugin');
expectContains(androidSharePlugin, 'getPendingShare', 'Android share read method');
expectContains(androidSharePlugin, 'clearPendingShare', 'Android share cleanup method');

// Web bridge must hydrate the existing MF Share review queue instead of bypassing human review.
expectContains(nativeShareBridge, 'saveMobileSharedPayload', 'Native share queue reuse');
expectContains(nativeShareBridge, '/share?id=', 'Native share review route');
expectContains(nativeShareBridge, 'clearPendingShare', 'Native share native cleanup');
expectContains(mainEntry, 'installNativeShareBridge', 'Native share bridge bootstrap');
...
expectContains(iosInfo, '<string>mfinanceiro</string>', 'iOS custom URL scheme');
expectContains(iosDelegate, 'launchOptions?[.shortcutItem]', 'iOS quick action cold launch');
expectContains(iosDelegate, 'pendingShortcutURL', 'iOS pending quick action handoff');
expectContains(iosDelegate, 'performActionFor shortcutItem', 'iOS quick action warm launch');
for (const destination of ['mfinanceiro://quick', 'mfinanceiro://scan', 'mfinanceiro://pulse']) {
