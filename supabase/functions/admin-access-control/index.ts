...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers':
    'authorization, x-client-info, apikey, content-type, x-mf-access-approval-control-secret',
...
async function listRequests(supabaseUrl: string, serviceRole: string) {
  ...
  if (!response.ok)
    throw new Error(`Falha ao listar solicitações (${response.status}).`);
...
async function decideRequest(
  ...
  if (normalizeStatus(current.status) !== 'pending') {
    return {
      status: 409,
      body: { error: 'A solicitação já possui decisão.', request: current },
    };
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Falha ao registrar decisão (${response.status}): ${detail}`,
    );
  }
  ...
  if (!updated) {
    ...
    return {
      status: 409,
      body: {
        error: 'A solicitação foi alterada por outra operação.',
        request: latest,
      },
    };
  }
...
Deno.serve(async (request: Request) => {
  ...
  if (!expectedControlSecret)
    return json(
      { error: 'Canal de aprovação administrativa não configurado.' },
      503,
    );

  const suppliedControlSecret =
    request.headers.get('x-mf-access-approval-control-secret') || '';
  ...
  try {
    if (action === 'list') {
      return json({
        requests: await listRequests(supabaseUrl, serviceRole),
    ...
    if (identity.role !== 'admin')
      return json(
        { error: 'Somente administradores podem aprovar ou negar usuários.' },
        403,
      );
    if (identity.aal !== 'aal2')
      return json(
        { error: 'A decisão exige uma sessão administrativa AAL2/MFA.' },
        403,
      );
    ...
    if (!isUuid(requestId) || !isUuid(correlationId)) {
      return json(
        { error: 'request_id e correlation_id devem ser UUIDs válidos.' },
        400,
      );
    }
    ...
    if (!decision)
      return json({ error: 'Decisão inválida. Use approved ou denied.' }, 400);

    const reason = sanitizeText(body.reason, 500);
    if (reason.length < 4)
      return json(
        { error: 'Informe uma justificativa com pelo menos 4 caracteres.' },
        400,
      );
    ...
    const message =
      error instanceof Error
        ? error.message
        : 'Falha inesperada no controle de aprovação.';
