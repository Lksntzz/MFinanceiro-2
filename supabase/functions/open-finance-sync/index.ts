...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers':
    'authorization, x-client-info, apikey, content-type, x-open-finance-internal-secret',
  'Access-Control-Allow-Methods': 'POST, OPTIONS',
};

const allowedTriggerSources = new Set([
  'initial',
  'manual',
  'scheduled',
  'webhook',
...
function transactionType(
  ...
  if (account.type === 'CREDIT')
    return Number(transaction.amount || 0) >= 0 ? 'expense' : 'income';
...
Deno.serve(async (request: Request) => {
  ...
  const suppliedInternal =
    request.headers.get('x-open-finance-internal-secret') || '';
  ...
  try {
    ...
    if (callerUserId)
      connectionQuery = connectionQuery.eq('user_id', callerUserId);
    ...
    if (!connection.provider_connection_ref)
      throw new Error('Conexão sem item Pluggy associado.');
    if (
      connection.status === 'revoked' ||
      connection.status === 'revocation_pending'
    )
      throw new Error('Essa conexão já foi revogada.');
    ...
    for (const providerAccount of accounts) {
      ...
      const safeBalance = Number.isFinite(providerBalance)
        ? providerBalance
        : 0;
      const accountPayload = {
        user_id: connection.user_id,
        name:
          providerAccount.marketingName ||
          providerAccount.name ||
          `${connection.institution_name} · Open Finance`,
      ...
      if (localAccountId) {
        ...
        if (error || !data)
          throw (
            error || new Error('Não foi possível criar a conta Open Finance.')
          );
      ...
      for (const transaction of transactions) {
        ...
        const providerCategory =
          String(transaction.category || 'Outros').trim() || 'Outros';
        const category =
          categories.find(
            (item: any) =>
              item.name?.toLowerCase() === providerCategory.toLowerCase() &&
              (item.category_type === 'both' || item.category_type === type),
          ) ||
          categories.find(
            (item: any) =>
              item.name?.toLowerCase() === 'outros' &&
              (item.category_type === 'both' || item.category_type === type),
          ) ||
          categories.find(
            (item: any) =>
        ...
        if (!category)
          throw new Error(
            'Nenhuma categoria financeira ativa disponível para a sincronização.',
          );
        ...
        const payload = {
          ...
          payment_method:
            providerAccount.type === 'CREDIT' ? 'credit_card' : 'unspecified',
          status: transaction.status === 'PENDING' ? 'pending' : 'paid',
          status_importacao: 'imported',
          affects_balance:
            providerAccount.type !== 'CREDIT' &&
    ...
    const { error: finishConnectionError } = await admin
      .from('mf_bank_connections')
      .update({
        status: 'active',
        sync_status: 'idle',
    ...
    const message =
      error instanceof Error
        ? error.message
        : 'Falha ao sincronizar Open Finance.';
    const finishedAt = new Date().toISOString();
    if (syncRunId) {
      await admin
        .from('mf_bank_sync_runs')
