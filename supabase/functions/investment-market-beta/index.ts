...
Deno.serve(async (request: Request) => {
  ...
  if (!authorization || !token)
    return json(request, { error: 'Autenticação necessária.' }, 401);
  ...
  if (!supabaseUrl || !publishableKey)
    return json(request, { error: 'Supabase não configurado na função.' }, 500);
