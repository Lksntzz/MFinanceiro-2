...
const responseSchema = {
  ...
  properties: {
    ...
    amount: {
      type: ['number', 'null'],
      description: 'Positive monetary amount in BRL when visible',
    },
...
Deno.serve(async (request: Request) => {
  ...
  const model =
    Deno.env.get('GEMINI_DOCUMENT_OCR_MODEL') ||
  ...
  try {
    ...
    const { data: extraction, error: extractionError } = await supabase
      ...
      .select(
        'id,user_id,account_id,source_file_path,source_file_name,source_mime_type,source_file_size,document_type,status,result_metadata',
      )
      .eq('id', extractionId)
    ...
    if (file.size < 1 || file.size > MAX_OCR_FILE_SIZE)
      throw new Error(
        'O arquivo privado não está dentro do limite permitido para OCR visual.',
      );
    ...
    const categoryMap = new Map(
      categoryNames.map((name) => [normalizeLearningKey(name), name]),
    );
    ...
    const matchedCategory = rawCategory
      ? categoryMap.get(normalizeLearningKey(rawCategory)) || null
      : null;
    ...
    if (rawCategory && !matchedCategory)
      warnings.push(
        'A categoria sugerida pelo OCR não corresponde a uma categoria ativa e foi descartada.',
      );
    ...
    const documentKind = [
      ...
      'pix_charge',
      'other',
    ].includes(String(parsed.document_kind))
      ? String(parsed.document_kind)
      : 'other';
    const paymentStatus = ['pending', 'paid', 'unknown'].includes(
      String(parsed.payment_status),
    )
      ? String(parsed.payment_status)
      : 'unknown';
    const metadata = {
      ...(extraction.result_metadata &&
      typeof extraction.result_metadata === 'object'
        ? extraction.result_metadata
        : {}),
