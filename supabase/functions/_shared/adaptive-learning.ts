...
export function calibrateConfidence(rawConfidence: unknown, reviewThreshold: unknown) {
  const raw = clampConfidence(rawConfidence);
  const threshold = Math.min(0.98, Math.max(0.75, Number(reviewThreshold) || BASE_REVIEW_THRESHOLD));
...
export function buildAdaptivePatternMap(rows: Array<Record<string, unknown>>) {
  ...
  for (const row of rows) {
    ...
    map.set(`${type}:${key}`, {
      ...
      confirmation_count: Math.max(0, Math.trunc(Number(row.confirmation_count) || 0)),
    });
  }
  return map;
}
