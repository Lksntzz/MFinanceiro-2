...
export function pluggyConfigured() {
  return Boolean(
    Deno.env.get('PLUGGY_CLIENT_ID')?.trim() &&
      Deno.env.get('PLUGGY_CLIENT_SECRET')?.trim(),
  );
}
...
export async function createApiKey() {
  const response = await fetch(`${PLUGGY_API}/auth`, {
    ...
    body: JSON.stringify({
      clientId: required('PLUGGY_CLIENT_ID'),
      clientSecret: required('PLUGGY_CLIENT_SECRET'),
    }),
  });
  if (!response.ok)
  ...
  const apiKey = String(
    payload.apiKey || payload.api_key || payload.accessToken || '',
  );
...
async function pluggyFetch(path: string, apiKey: string, init?: RequestInit) {
  const response = await fetch(
    path.startsWith('http') ? path : `${PLUGGY_API}${path}`,
    {
      ...init,
      headers: {
        'X-API-KEY': apiKey,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Pluggy recusou a operação (${response.status})${text ? `: ${text.slice(0, 300)}` : ''}`,
    );
  }
...
export async function createConnectToken(
  ...
  const payload = (await pluggyFetch('/connect_token', apiKey, {
    method: 'POST',
    body: JSON.stringify(body),
  })) as Record<string, unknown>;
  const accessToken = String(
    payload.accessToken || payload.connectToken || payload.token || '',
  );
...
export async function fetchItem(
  ...
  const payload = (await pluggyFetch(
...
export async function deleteItem(apiKey: string, itemId: string) {
  await pluggyFetch(`/items/${encodeURIComponent(itemId)}`, apiKey, {
    method: 'DELETE',
  });
}
...
export async function fetchTransactions(
  ...
  do {
    ...
    const payload = (await pluggyFetch(
      ...
      apiKey,
    )) as { results?: PluggyTransaction[]; next?: string | null };
